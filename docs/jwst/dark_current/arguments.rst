...
Step Arguments
==============
The ``dark_current`` step has these step-specific arguments:

``--dark_output`` (str, default=None)
...
``--average_dark_current`` (float, default=None)
  ...
  current. This parameter is a scalar quantity; if a 2D array is desired to
  describe the dark current pixel-to-pixel, this must be specified in the
  :ref:`dark_reffile`.
