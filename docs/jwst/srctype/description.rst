...
#. TSO exposures default to a source type of "POINT". TSO exposures are
   identified by ``EXP_TYPE="NRC_TSGRISM"``, ``EXP_TYPE="NRS_BRIGHTOBJ"``,
   or ``TSOVISIT=True``.
...
.. note::
    NIRSpec fixed-slit (``EXP_TYPE="NRS_FIXEDSLIT"``) exposures are
...
metadata file are loaded for each source/slitlet by the :ref:`assign_wcs <assign_wcs_step>` step of
