...
``--fit_threshold`` (float, default=10.0)
  ...
  Higher values will create spline models for fewer regions; lower values will attempt
  to fit more regions. If set to 0, all regions will be fit.
...
``--maximum_cores`` (str, default="none")
  ...
  Note that this option applies only to IFU data, and that multiprocessing is applied
  only to the spline fitting portion of the processing. The oversampling portion is
  not affected by this flag. Thus the speed up will be less than linear with the number
  of cores, since only a portion of the code is affected. The relative fractional speedup
  will be largest for cases in which the ``fit_threshold`` and ``slope_limit`` parameter
  values, and/or the ``psf_optimal`` flag, result in a larger fraction of the image
  having the spline fit performed.
