...
to the way functions are called. For example:
    ...
    >>> poly_x = astmodels.Polynomial2D(
    ...     degree=2, c0_0=.2, c1_0=.11, c2_0=2.3,
    ...     c0_1=.43, c0_2=.1, c1_1=.5)
...
in ``x`` and ``y``, preceded by a shift in both axes:
    ...
    >>> poly_y = astmodels.Polynomial2D(
    ...     degree=2, c0_0=.2, c1_0=1.1, c2_0=.023,
    ...     c0_1=3, c0_2=.01, c1_1=2.2)
...
The `~stdatamodels.jwst.datamodels.DistortionModel` is used as an example of how to create a reference file.
...
.. testcleanup::
   >>> import os
   >>> for f in ("new_distortion.asdf", "reffile.asdf"):
   ...     if os.path.exists(f):
   ...         os.remove(f)
