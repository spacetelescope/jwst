...
containing a 2D boolean array in its ``.mask`` attribute. The output of a previous
run of the background subtraction step, saved with suffix "bsub", can be used
as such a custom mask, after editing the ``.mask`` attribute as desired. To generate
a valid model from scratch, use something like::
   
    from stdatamodels.jwst.datamodels import ImageModel
    import numpy as np

    mask_data = np.ones((2048, 2048), dtype=bool)  # same shape as science data
