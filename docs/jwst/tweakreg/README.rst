...
string and ``ignore_input_catalogs`` is `False`, then it will be interpretted
...
``ignore_input_catalogs`` parameter of the ``tweakreg`` step must be set to
`False`.
...
* ``ignore_input_catalogs``: A boolean that indicates whether
  to ignore source catalog in the input data model's ``meta.tweakreg_catalog``
  attribute. If `True`, new catalogs will be generated by the ``tweakreg``
  step. (Default=`True`)
