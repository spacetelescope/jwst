...
Images taken at the same time (i.e., NIRCAM images from all short-wave
...
guide star acquision will be identical in all these images and it is assumed
that relative positions of (e.g., NIRCAM) detectors do not change.
...
setting ``image_model.meta.group_id`` attribute to a unique string or integer
value or by adding ``group_id`` attribute to the ``members`` of the input ASN
table - see `~jwst.datamodels.ModelContainer` for more details.
...
.. note::
    ...
    If it is desirable to apply different grouping strategies to ``tweakreg``
