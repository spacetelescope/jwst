...
* ``TNNNN``: Candidate Identifier. Can be one of the following:
    ...
    * ``c1NNN``: Association candidate, specified by the letter 'c',
      followed by a number starting at 1001.
...
* ``ATYPE``: The type of association. See :ref:`asn-jwst-association-types`.
...
* Files containing target-centric data:
  ...
  The following regex matches file names produced by Stage 3 calibration
  and containing target-centric data::
