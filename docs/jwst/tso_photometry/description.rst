...
:Class: `jwst.tso_photometry.tso_photometry_step.TSOPhotometryStep`
...
The output is a table (:ref:`astropy:ecsv_format`) containing the time at the
...
integration instead (set ``moving_centroid`` to `True` in :ref:`tso_phot_step_arguments`).

Aperture Photometry
-------------------
Aperture photometry is done by :ref:`photutils <photutils:user_guide>`.

If the input file is *not* averaged over integrations (i.e., a ``_calints``
...
- ``aperture_ra_icrs``: RA position (ICRS frame) at the aperture center
- ``aperture_dec_icrs``: Dec position (ICRS frame) at the aperture center
