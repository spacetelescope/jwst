...
number of direct images  provided. The ``model.wavetable`` attribute specifies the corresponding
wavelengths.
...
When the step encounters a `~stdatamodels.jwst.datamodels.WFSSMultiBandModel` file as the direct image,
...
``model.wavetable``, a flat extrapolation is used. If NaNs are encountered in a given pixel at some
