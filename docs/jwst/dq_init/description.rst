...
#. Propagate any DO_NOT_USE flags from the reference file DQ array to the science data
   ...
   :ref:`ramp_fit <ramp_fitting_step>` for pixels marked as unusable by the reference file.

FGS guide star data
-------------------
Note that when applying the ``dq_init`` step to FGS guide star data, as is done in
the :ref:`calwebb_guider <calwebb_guider>` pipeline, the flags from the :ref:`mask_reffile`
are propagated into the guide star dataset "DQ" array, instead of the "PIXELDQ" array.
...
readout pattern, because matching IRS2 :ref:`mask_reffile` are supplied in CRDS.
