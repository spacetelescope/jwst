...
#. A :class:`~jwst.datamodels.container.ModelContainer` with several 2-D IFU data models
...
The NIRSpec IFU has a 3 x 3 arcsecond field of view that is sliced into 30 0.1-arcsecond regions. Each slice is
...
there will be an added binary extension table to the output FITS file of the IFU cube. This extension has
...
Output Product Name
-------------------
If the input data is passed in as an `~stdatamodels.jwst.datamodels.ImageModel`, then the IFU cube will be passed back as an `~stdatamodels.jwst.datamodels.IFUCubeModel`. The cube
...
- For MIRI, the output string name is determined from the channels and sub-channels used.
  The  IFU string for MIRI is 'ch' plus the channel numbers used plus a string for the subchannel. For example, if the IFU cube
  contains channels 1 and 2 data for the short sub-channel, the output name would be ``rootname_ch1-2_SHORT_s3d.fits``.
  If all the sub-channels were used, then the output name would be ``rootname_ch1-2_ALL_s3d.fits``.
...
- For NIRSpec the output string is determined from the gratings and filters used. The gratings are grouped together in a dash (-)
  ...
  gratings G140M and G235M and from filters F070LP and F100LP, the output name would be
...
is defined by a :ref:`cubepar_reffile` as a function of wavelength, and can also be changed by the user.
The :ref:`cubepar_reffile` contains a predefined scale to use
...
The JWST pipeline includes two methods for building IFU data cubes:

* :ref:`cube_build_3d_drizzle` (default)
* :ref:`cube_build_shepard`

The core principle of both algorithms is to resample the 2-D detector
data into a 3-D rectified data cube in a single step while conserving flux.
The differences in the the techniques are how the detector pixels are weighted in the final 3-D data cube.

.. _cube_build_3d_drizzle:
...
In the 3-D drizzling, we project the 2-D detector pixels to
their corresponding 3-D volume elements and allocate their intensities to the individual voxels of the final data cube according
...
rotated in a complex manner with respect to the cube voxels, see :ref:`Figure 1 <cube_build_algo_fig_1>`. The iso-α and iso-λ directions are not perfectly orthogonal
...
can be computed independently (see :ref:`Figure 2 <cube_build_algo_fig_2>`).
...
Law, D. R., et al. 2023, AJ, 166, 45 <https://iopscience.iop.org/article/10.3847/1538-3881/acdddc>`_.

.. _cube_build_algo_fig_1:
...
.. _cube_build_algo_fig_2:

.. figure:: cube_build_overlap2.png
   :scale: 50%
   :align: left

   Same as :ref:`Figure 1 <cube_build_algo_fig_1>` but representing the simplified case in which the spectral dispersion is assumed to be aligned with detector columns and
...
variant of Shepard's method, which is an alternative based on an exponential
modified-Shepard method (EMSM) weighting function.
In this technique we ignore the overlap between the detector pixel and cube voxel and
...
A schematic of this process is shown in :ref:`Figure 3 <cube_build_algo_fig_3>`.

.. _cube_build_algo_fig_3:

.. figure:: pointcloud.png
   :scale: 50%
   :align: left

   Schematic of two dithered exposures mapped to the IFU output coordinate system (black regular grid).
...
xdistance
    Distance between point in the cloud and voxel center in units of arc seconds along the x axis.
ydistance
    Distance between point in the cloud and voxel center in units of arc seconds along the y axis.
zdistance
    Distance between point in the cloud and voxel center in the lambda dimension in units of microns along the wavelength axis.

These distances are then normalized by the IFU cube voxel size for the appropriate axis:

xnormalized
    ``xdistance`` / (cube voxel size in x dimension [``cdelt1``])
ynormalized
    ``ydistance`` / (cube voxel size in y dimension [``cdelt2``])
znormalized
    ``zdistance`` / (cube voxel size in z dimension [``cdelt3``])
...
The default size of the region of influence is defined in the :ref:`cubepar_reffile`, but can be changed by the
user with the options: ``rois`` and ``roiw``.

If *n* point cloud members are located within the ROI of a voxel, the voxel flux:

:math:`K = \frac{ \sum_{i=1}^n Flux_i w_i}{\sum_{i=1}^n w_i}`

where the weighting ``weighting=emsm`` is:

:math:`w_i = e^{\frac{ -({xnormalized}_i^2 + {ynormalized}_i^2 + {znormalized}_i^2)} {scale\_factor}}`

and:

:math:`scale\_factor = scale\_rad / cdelt1`

where *scale_rad* is read in from the reference file and varies with wavelength.

If the alternative weighting function (set by ``weighting=msm``) is selected then:

:math:`w_i = (\sqrt{({xnormalized}_i^2 + {ynormalized}_i^2 + {znormalized}_i^2)^{p} })^{-1}`

In this weighting function, the default value for *p* is read in from the :ref:`cubepar_reffile`. It can also be set
