...
Step Arguments
==============
The default values for the step arguments are found in the :attr:`~jwst.cube_build.cube_build_step.CubeBuildStep.spec` attribute.
...
``debug_spaxel [string]``
  ...
  The string is the x, y, and z values of the cube spaxel that is being investigated. The numbering starts counting at 0.
...
The offset file is an `ASDF formatted file <https://asdf-standard.readthedocs.io/>`_.
For each input file in the ``spec3`` association used to build the IFU cubes, the offset file needs to have a
corresponding right ascension and declination offset given in arcseconds.
