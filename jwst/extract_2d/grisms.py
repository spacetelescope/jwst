...
def extract_tso_object(
    input_model, reference_files=None, tsgrism_extract_height=None, extract_orders=None, compute_wavelength=True
):
    """
    ...
    if extract_orders is None:
        ...
        if not isinstance(extract_orders, list) or not all(isinstance(item, int) for item in extract_orders):
    ...
    if input_model.meta.wcsinfo.siaf_xref_sci is None or input_model.meta.wcsinfo.siaf_yref_sci is None:
    ...
    for order in available_orders:
        ...
        if output_model.meta.model_type == "SlitModel":
            ...
            output_model.meta.wcsinfo.dispersion_direction = input_model.meta.wcsinfo.dispersion_direction
            output_model.name = "1"
...
def extract_grism_objects(
    ...
    if grism_objects is None:
        ...
        if "wavelengthrange" not in reference_files or reference_files["wavelengthrange"] in ["N/A", ""]:
            ...
            log.info("Grism object list created from source catalog: {0:s}".format(input_model.meta.source_catalog))
    ...
    for obj in grism_objects:
        for order in obj.order_bounding.keys():
            ...
            if ymax - ymin > 0 and xmax - xmin > 0:
                ...
                tr = Mapping((0, 1, 0, 0, 0)) | (Shift(xmin) & Shift(ymin) & xcenter_model & ycenter_model & order_model) | tr
                ...
                new_slit = datamodels.SlitModel(
                    data=ext_data, err=ext_err, dq=ext_dq, var_poisson=var_poisson, var_rnoise=var_rnoise, var_flat=var_flat
                )
                new_slit.meta.wcsinfo.spectral_order = order
                new_slit.meta.wcsinfo.dispersion_direction = input_model.meta.wcsinfo.dispersion_direction
