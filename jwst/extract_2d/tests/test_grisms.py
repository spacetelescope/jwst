...
def create_tso_wcsimage(filtername="F277W", subarray=False):
    ...
    if subarray:
        ...
        subarray = "FULL"
    hdul = create_hdul(
        exptype="NRC_TSGRISM", pupil="GRISMR", filtername=filtername, detector="NRCALONG", subarray=subarray, wcskeys=wcs_tso_kw
    )
...
def test_wfss_extract_custom_height():
    ...
    extract_orders = [1]  # just extract the first order
    test_boxes = create_grism_bbox(imwcs, refs, mmag_extract=99.0, extract_orders=extract_orders, wfss_extract_half_height=5)
