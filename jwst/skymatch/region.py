...
class Polygon(Region):
    ...
    def __init__(self, rid, vertices, coord_system="Cartesian"):
        assert len(vertices) >= 4, "Expected vertices to be a list of minimum 4 tuples (x,y)"
    ...
    def scan(self, data):
        ...
        while y <= scline:
            ...
            scan_line = Edge(
                "scan_line", start=[self._bbox[0], y], stop=[self._bbox[0] + self._bbox[2], y]
            )
...
class Edge:
    ...
    def is_parallel(self, edge):
        ...
        return np.allclose(np.linalg.det([u, v]), 0, rtol=0, atol=1e2 * np.finfo(float).eps)
