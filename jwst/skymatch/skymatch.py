...
drizzlepac/astrodrizzle.html>`_.
    ...
    **Limitations and Discussions:**
    ...
    if skymethod not in ["local", "global", "match", "global+match"]:
        raise ValueError("Unsupported 'skymethod'. Valid values are: " "'local', 'global', 'match', or 'global+match'")
    ...
    for img in images:
        ...
        if img_type == "Group":
            ...
            for im in img:
                log.debug("      - Image ID={}. Conversion factor = {:G}".format(im.id, im.convf))
        else:
            log.debug("   *  Image ID={}. Conversion factor = {:G}".format(img.id, img.convf))
    ...
    log.info("***** {:s}.{:s}() TOTAL RUN TIME: {}".format(__name__, function_name, runtime_end - runtime_begin))
    log.info(" ")
...
def _apply_sky(images, sky_deltas, do_global, do_skysub, show_old):
    for img, sky in zip(images, sky_deltas):
        ...
        if do_global:
            ...
            if not valid:
                log.warning("   *  {:s} ID={}: Unable to compute sky value".format("Group" if is_group else "Image", img.id))
                sky = 0.0
        ...
        if is_group:
            ...
            # log sky values:
            log.info("   *  Group ID={}. Sky background of " "component images:".format(img.id))
            ...
            for im, old_sky, new_sky in zip(img, old_img_sky, new_img_sky):
                ...
                if show_old:
                    ...
                    log.info("      - Image ID={}. Sky background: {:G}".format(im.id, c * new_sky))
            ...
            if show_old:
                ...
                log.info("   *  Image ID={}. Sky background: {:G}".format(img.id, c * new_sky))
