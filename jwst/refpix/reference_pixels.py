...
class NIRDataset(Dataset):
    ...
    def __init__(self, input_model,
        ...
        if self.is_irs2:
            ...
            self.irs2_odd_mask = self.make_irs2_odd_mask()
        else:
            self.reference_sections = NIR_reference_sections
            self.irs2_odd_mask = None

    def make_irs2_odd_mask(self):
        """
        ...
        # Assume standard size and readout mode: reference sections
        # are coded to match.
        nx, ny = (3200, 2048)
        x_mask = make_irs2_mask(nx, ny, 16, 4)
