...
def correct_model(input_model, irs2_model, scipix_n_default=16, refpix_r_default=4, pad=8, preserve_refpix=False):
    ...
    if nrows != expected_nrows:
        log.error("Number of rows in reference file = {}," " but it should be {}.".format(nrows, expected_nrows))
    ...
    if hasattr(irs2_model, "dq_table") and len(irs2_model.dq_table) > 0:
        ...
        clobber_ref(data, output, odd_even, mask, ref_flags, is_irs2, scipix_n=scipix_n, refpix_r=refpix_r)
    else:
    ...
    for integ in range(n_int):
        ...
        data0 = data[integ, :, :, :]
        data0 = subtract_reference(data0, alpha, beta, irs2_mask, scipix_n, refpix_r, pad, preserve_refpix=preserve_refpix)
...
def clobber_ref(data, output, odd_even, mask, ref_flags, is_irs2, scipix_n=16, refpix_r=4):
    ...
    for row in range(nrows):
        ...
        log.debug("output {}  odd_even {}  mask {}  DQ bits {}".format(output[row], odd_even[row], mask[row], bits))
        new_bad_pix = []
        for k in bits:
        ...
        for bad_pix in new_bad_pix:
            replace_refpix(bad_pix, data, ref_flags, is_irs2, offset, offset + nx // 5, scipix_n, refpix_r, axis=-1)
...
def replace_refpix(bad_pix, data, bad_mask, is_irs2, low_limit, high_limit, scipix_n, refpix_r, axis=-2):
    """
    ...
    for k in range(5):
        ...
        for j in range(nints):
            ...
            if not replace_only:
                ...
                log.debug(
                    f"High diffs={np.sum(high_diffs)}, " f"high means={np.sum(high_means)}, " f"high stds={np.sum(high_stds)}"
                )
        ...
        if not flag_only:
            ...
            for bad_pix in all_bad:
                replace_refpix(bad_pix, data, mask_bad, is_irs2, offset, offset + amplifier, scipix_n, refpix_r)
...
def subtract_reference(data0, alpha, beta, irs2_mask, scipix_n, refpix_r, pad, preserve_refpix=False):
    ...
    hs[:, scipix_n // 2 + 1 - refpix_r // 2 : scipix_n // 2 + 1 + refpix_r // 2 + refpix_r] = hs[:, two_indr_t]
    mask = hs >= 0
...
def remove_slopes(data0, ngroups, ny, row):
    ...
    for i in range(5):
        for k in range(ngroups):
            ...
            (intercept, slope) = ols_line(time_arr[row4plus4, :][mask], data0[i, k, row4plus4, :][mask])
...
def fill_bad_regions(data0, ngroups, ny, nx, row, scipix_n, refpix_r, pad, hnorm, hnorm1):
    ...
    blen = (512 + 512 // scipix_n * (refpix_r + 2) + pad) // (scipix_n + refpix_r + 2) * ny // 2 - elen // 2
    ...
    fft_interp_norm(dd0, np.ones((ny, nx // 4), dtype=np.int64), row, hnorm, hnorm1, ny, ngroups, aa, n_iter_norm)
