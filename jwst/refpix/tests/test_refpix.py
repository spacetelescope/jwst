...
def test_do_corrections_subarray_no_oddEven(setup_subarray_cube):
    ...
    input_model = setup_subarray_cube(
        "SUB160", "NRCB1", xstart, ystart, ngroups, nrows, ncols
    )
...
def test_do_corrections_subarray_4amp(setup_subarray_cube):
    ...
    input_model = setup_subarray_cube(
        "SUBGRISM64", "NRCA1", xstart, ystart, ngroups, nrows, ncols
    )
    input_model.meta.exposure.noutputs = 4
    input_model.data[0, 0, 4:-4, 4:512:2] = dataval + bottom_rpix_a_odd + side_rpix_mean
    input_model.data[0, 0, 4:-4, 5:512:2] = (
        dataval + bottom_rpix_a_even + side_rpix_mean
    )
    input_model.data[0, 0, 4:-4, 512:1024:2] = (
        dataval + bottom_rpix_b_odd + side_rpix_mean
    )
    input_model.data[0, 0, 4:-4, 513:1024:2] = (
        dataval + bottom_rpix_b_even + side_rpix_mean
    )
    input_model.data[0, 0, 4:-4, 1024:1536:2] = (
        dataval + bottom_rpix_c_odd + side_rpix_mean
    )
    input_model.data[0, 0, 4:-4, 1025:1536:2] = (
        dataval + bottom_rpix_c_even + side_rpix_mean
    )
    input_model.data[0, 0, 4:-4, 1536:2044:2] = (
        dataval + bottom_rpix_d_odd + side_rpix_mean
    )
    input_model.data[0, 0, 4:-4, 1537:2044:2] = (
        dataval + bottom_rpix_d_even + side_rpix_mean
    )
    ...
    np.testing.assert_almost_equal(
        np.mean(input_model.data[0, 0, 4:-4, 4:-4]), dataval, decimal=4
    )
...
def test_get_restore_group_subarray(setup_subarray_cube):
    ...
    assert np.all(
        init_dataset.group[
            ystart - 1 : ystart - 1 + nrows, xstart - 1 : xstart - 1 + ncols
        ]
        == 150
    )
...
def test_do_top_bottom_correction(setup_cube):
    ...
    for i in np.arange(0, len(abounds) - 1):
        ...
        # bottom, odd
        input_model.data[0, 0, :4, abounds[i] : abounds[i + 1] : 2] = bottom_even_amps[
            i
        ]

        # bottom, even
        input_model.data[
            0, 0, :4, abounds[i] + 1 : abounds[i + 1] - 1 : 2
        ] = bottom_odd_amps[i]
...
def test_correct_model(setup_cube, instr, det):
    ...
    np.testing.assert_almost_equal(
        np.mean(input_model.data[0, 0, :4, 4:-4]), 0, decimal=0
    )
    np.testing.assert_almost_equal(
