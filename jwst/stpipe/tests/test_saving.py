...
def test_save_step_withoutput(mk_tmp_dirs):
    ...
    args = ["jwst.stpipe.tests.steps.StepWithModel", data_fn_path, "--output_file=" + output_file]
...
def test_save_step_withdir(mk_tmp_dirs):
    ...
    args = ["jwst.stpipe.tests.steps.StepWithModel", data_fn_path, "--output_dir=" + tmp_data_path]
...
def test_save_step_withdir_environment(mk_tmp_dirs):
    ...
    args = ["jwst.stpipe.tests.steps.StepWithModel", data_fn_path, "--output_dir=$TSSWE_OUTPATH"]
...
def test_save_step_withdir_withoutput(mk_tmp_dirs):
    ...
    output_path, output_ext = path.splitext(output_file)
    output_fn_path = path.join(tmp_data_path, output_path + "_stepwithmodel" + output_ext)
    assert path.isfile(output_fn_path)
...
def test_save_pipeline_default(mk_tmp_dirs):
    ...
    args = [step_fn_path, tmp_data_fn_path, "--steps.savestep.skip=False"]
...
def test_save_pipeline_withdir(mk_tmp_dirs):
    ...
    output_pipeline_fn_path = path.join(tmp_data_path, data_name + "_savepipeline" + data_ext)
...
def test_save_substep_withdir(mk_tmp_dirs):
    ...
    args = [step_fn_path, data_fn_path, "--steps.savestep.skip=False", "--steps.savestep.output_dir=" + tmp_data_path]

    Step.from_cmdline(args)

    # Output from the explicit `SaveStep.save_model`
    desired = path.join(tmp_data_path, data_name + "_processed" + data_ext)
    assert path.isfile(desired)

    # Output from the Step's default saving of `SaveStep`
    desired = path.join(tmp_data_path, data_name + "_savestep" + data_ext)
    assert path.isfile(desired)
...
def test_save_proper_pipeline_container_withdir_withoutput(mk_tmp_dirs):
    ...
    output_path, output_ext = path.splitext(output_name)
    assert path.isfile(path.join(tmp_data_path, output_path + "_0_pp" + output_ext))
