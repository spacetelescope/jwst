...
from jwst.stpipe.tests.steps import EmptyPipeline, MakeListPipeline, MakeListStep, ProperPipeline, AnotherDummyStep
...
@pytest.mark.parametrize(
    ...
    [
        ...
        #
        (
            MakeListPipeline(par1="Instantiated", steps={"make_list": {"par1": 0.0, "par2": "sub-instantiated"}}),
            True,
            {
                "pre_hooks": [],
                "post_hooks": [],
        ...
        # ######################################
        # Mix of required and optional parameters
        (MakeListStep(par1=0.0, par2="from args"), False, {"par1": 0.0, "par2": "from args", "par3": False}),
        # Pipeline with all parameters set
        (
            MakeListPipeline(par1="Instantiated", steps={"make_list": {"par1": 0.0, "par2": "sub-instantiated"}}),
            False,
            {"par1": "Instantiated", "steps": {"make_list": {"par1": 0.0, "par2": "sub-instantiated", "par3": False}}},
        ),
        # Pipeline without any sub-steps
...
def test_step_from_commandline():
    args = [abspath(join(dirname(__file__), "steps", "some_other_step.cfg")), "--par1=58", "--par2=hij klm"]
...
def test_step_from_commandline_config_class_alias(mock_stpipe_entry_points):
    args = [abspath(join(dirname(__file__), "steps", "dummy_with_alias.cfg")), "--par1=58", "--par2=hij klm"]
...
def test_step_from_commandline_par_precedence(
    command_line_pars, command_line_config_pars, reference_pars, expected_pars, tmp_path, monkeypatch
):
    args = []
...
def test_crds_override():
    step = AnotherDummyStep(
        "SomeOtherStepOriginal", par1=42.0, par2="abc def", override_flat_field=join(dirname(__file__), "data", "flat.fits")
    )
