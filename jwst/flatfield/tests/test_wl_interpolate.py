...
def test_wl_interpolate():
    ...
    tab_neg_wl = np.array(
        [-1.0, -0.418, 0.128, 0.638, 1.112, 1.55, 1.952, 2.318, 2.648, 2.942],
        dtype=np.float64,
    )
    ...
    tab_wl = np.array(
        [5.0, 5.582, 6.128, 6.638, 7.112, 7.55, 7.952, 8.318, 8.648, 8.942],
        dtype=np.float64,
    )
    wavelength = 4.0  # out of bounds
    value = wl_interpolate(wavelength, tab_wl, tab_flat)
    assert value is None

    wavelength = 9.0  # out of bounds
    value = wl_interpolate(wavelength, tab_wl, tab_flat)
    assert value is None
    ...
    value = wl_interpolate(wavelength, tab_wl, tab_flat)
    assert math.isclose(value, expected_value, rel_tol=1.0e-12, abs_tol=1.0e-12)
