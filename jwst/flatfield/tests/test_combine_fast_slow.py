...
def test_combine_fast_slow(flat_err_1, flat_err_2):
    ...
    coeff = {"c0": -41.9, "c1": 30.7, "c2": -8.3, "c3": 1.15, "c4": -7.8e-2, "c5": 2.0e-3}
    ...
    dispaxis = 1
    value, new_dq, new_err = combine_fast_slow(
        wl, flat_2d, flat_dq, flat_err, tab_wl, tab_flat, tab_flat_err, dispaxis
    )
    ...
    assert_allclose(value[:, 5], correct_value, rtol=1.0e-8, atol=1.0e-8)
    assert_array_equal(new_dq[:, 5], 0)
    assert_allclose(new_err[:, 5], err_result, rtol=1.0e-8, atol=1.0e-8)
    ...
    if flat_err is None or np.isnan(flat_err_2):
        ...
        assert_allclose(new_err[:, (3, 4, 6, 7, 8, 9)], flat_err[:, (3, 4, 6, 7, 8, 9)])
