...
def test_combine_fast_slow(flat_err_1, flat_err_2):
    ...
    assert np.allclose(value[:, 5], correct_value, rtol=1.0e-8, atol=1.0e-8)
    assert np.all(new_dq[:, 5] == 0)
    assert np.allclose(new_err[:, 5], err_result, rtol=1.0e-8, atol=1.0e-8)
    ...
    if flat_err is None or np.isnan(flat_err_2):
        ...
        assert np.allclose(new_err[:, (3, 4, 6, 7, 8, 9)], flat_err[:, (3, 4, 6, 7, 8, 9)])
