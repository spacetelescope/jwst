...
def create_nirspec_flats(shape, msa=False):
    ...
    for flat_name in ['f', 's', 'd']:
        ...
        if flat_name == 's' and not msa:
            ...
            fflat_hdul.append(fits.table_to_hdu(
                Table({'wavelength': np.arange(1, shape[0] + 1)})))
        ...
        if flat_name == 'f' and msa:
            for quadrant in range(4):
                fflat_hdul.append(fits.ImageHDU(data=np.full(shape, 1.0),
                                                name='SCI', ver=(quadrant + 1)))
                fflat_hdul.append(fits.ImageHDU(data=np.full(shape, 0),
                                                name='DQ', ver=(quadrant + 1)))
                fflat_hdul.append(fits.ImageHDU(data=np.full(shape, 0.0),
                                                name='ERR', ver=(quadrant + 1)))
