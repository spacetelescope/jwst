    ...
    for flat_name in ["f", "s", "d"]:
        ...
        if flat_name == "f" and msa:
            for quadrant in range(4):
                fflat_hdul.append(fits.ImageHDU(data=np.full(shape, 1.0), name="SCI", ver=(quadrant + 1)))
                fflat_hdul.append(fits.ImageHDU(data=np.full(shape, 0), name="DQ", ver=(quadrant + 1)))
                fflat_hdul.append(fits.ImageHDU(data=np.full(shape, 0.0), name="ERR", ver=(quadrant + 1)))
...
def test_nirspec_bots_flat():
    ...
    result = FlatFieldStep.call(
        data, override_fflat=flats[0], override_sflat=flats[1], override_dflat=flats[2], override_flat="N/A"
    )
...
def test_nirspec_ifu_flat():
    ...
    hdul = create_nirspec_ifu_file(grating="PRISM", filter="CLEAR", gwa_xtil=0.35986012, gwa_ytil=0.13448857, gwa_tilt=37.1)
