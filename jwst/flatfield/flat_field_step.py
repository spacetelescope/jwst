...
class FlatFieldStep(Step):
    ...
    def process(self, input):
        ...
        self.log.debug("Input is {} of exposure type {}".format(input_model.__class__.__name__, exposure_type))
        ...
        if input_model.meta.cal_step.extract_2d == "COMPLETE" and exposure_type not in EXTRACT_2D_IS_OK:
            self.log.warning(
                "The extract_2d step has been run, but for " "%s data it should not have been run, so ...", exposure_type
            )
        ...
        if self.user_supplied_flat is not None:
            self.log.info(
                f"User-supplied flat {self.user_supplied_flat} given." " Ignoring all flat reference files and flat creation."
            )
            ...
            reference_file_models = {"user_supplied_flat": datamodels.open(self.correction_pars["flat"])}

            # Record the flat as the FLAT reference type for recording
            # in the result header.
            self._reference_files_used.append(("flat", reference_file_models["user_supplied_flat"].meta.filename))
        else:
            reference_file_models = self._get_references(input_model, exposure_type)

        # Do the flat-field correction
        output_model, flat_applied = flat_field.do_correction(input_model, **reference_file_models, inverse=self.inverse)
