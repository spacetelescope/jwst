...
def do_correction(input_model, flat=None, fflat=None, sflat=None, dflat=None, user_supplied_flat=None, inverse=False):
    ...
    if (input_model.meta.exposure.type in NIRSPEC_SPECTRAL_EXPOSURES) and (input_model.meta.instrument.lamp_mode != "IMAGE"):
        flat_applied = do_nirspec_flat_field(
            output_model, fflat, sflat, dflat, user_supplied_flat=user_supplied_flat, inverse=inverse
        )
    else:
...
def do_flat_field(output_model, flat_model, inverse=False):
    ...
    if (output_model.data.shape[-1] > flat_model.data.shape[-1]) or (output_model.data.shape[-2] > flat_model.data.shape[-2]):
...
def do_nirspec_flat_field(output_model, f_flat_model, s_flat_model, d_flat_model, user_supplied_flat=None, inverse=False):
    ...
    if exposure_type == "NRS_BRIGHTOBJ":
        if not isinstance(output_model, datamodels.SlitModel):
            ...
            raise RuntimeError("Input is {}; expected SlitModel".format(type(output_model)))
    ...
    if not hasattr(output_model, "slits"):
        if exposure_type == "NRS_IFU" or (
            ...
            if not isinstance(output_model, datamodels.IFUImageModel):
                ...
                raise RuntimeError("Input is {}; expected IFUImageModel".format(type(output_model)))
            return nirspec_ifu(
                output_model,
                f_flat_model,
                s_flat_model,
...
def nirspec_fs_msa(output_model, f_flat_model, s_flat_model, d_flat_model, dispaxis, user_supplied_flat=None, inverse=False):
...
def nirspec_brightobj(output_model, f_flat_model, s_flat_model, d_flat_model, dispaxis, user_supplied_flat=None, inverse=False):
    ...
    if user_supplied_flat is not None:
        ...
        interpolated_flat = flat_for_nirspec_brightobj(output_model, f_flat_model, s_flat_model, d_flat_model, dispaxis)
    ...
    output_model.err = np.sqrt(output_model.var_poisson + output_model.var_rnoise + output_model.var_flat)
...
def nirspec_ifu(output_model, f_flat_model, s_flat_model, d_flat_model, dispaxis, user_supplied_flat=None, inverse=False):
...
def create_flat_field(
    wl, f_flat_model, s_flat_model, d_flat_model, xstart, xstop, ystart, ystop, exposure_type, dispaxis, slit_name, slit_nt=None
):
    ...
    f_flat, f_flat_dq, f_flat_err = fore_optics_flat(wl, f_flat_model, exposure_type, dispaxis, slit_name, slit_nt)
...
def spectrograph_flat(wl, s_flat_model, xstart, xstop, ystart, ystop, exposure_type, dispaxis, slit_name):
    ...
    (tab_wl, tab_flat, tab_flat_err) = read_flat_table(s_flat_model, exposure_type, slit_name, quadrant)
    if tab_wl.max() < MICRONS_100:
    ...
    if len(full_array_flat.shape) == 3:
        ...
        flat_2d, s_flat_dq, s_flat_err = interpolate_flat(image_flat, image_dq, image_err, image_wl, wl)
    else:
...
def detector_flat(wl, d_flat_model, xstart, xstop, ystart, ystop, exposure_type, dispaxis, slit_name):
    ...
    (tab_wl, tab_flat, tab_flat_err) = read_flat_table(d_flat_model, exposure_type, slit_name, quadrant)
    if tab_wl.max() < MICRONS_100:
...
def read_image_wl(flat_model, quadrant=None):
    ...
    if len(wavelength.shape) > 1:
        ...
        try:
            ...
            log.error("Image wavelength array has shape %s; " "don't know how to interpret that.", str(wavelength.shape))
...
def read_flat_table(flat_model, exposure_type, slit_name=None, quadrant=None):
    ...
    if exposure_type in FIXED_SLIT_TYPES and slit_col is not None and slit_name is not None:
        ...
        if row is None:
            ...
            raise ValueError("{} not found in SLIT_NAME column (nor was 'ANY')".format(slit_name))
    ...
    if nelem is not None:
        if len(tab_wl) < nelem:
            log.error(
                "The fast_variation array size %d in " "the data model is too small, and table data were " "truncated.",
                len(tab_wl),
            )
    ...
    if n1 != nelem:
        log.debug("The table wavelength or flat-field data array contained " "%d NaNs; these have been skipped.", nelem - n1)
    ...
    if n2 != n1:
        log.debug(
            "The table wavelength or flat-field data array contained " "%d zero or negative values; these have been skipped.",
            n1 - n2,
        )
        tab_wl = tab_wl[filter]
        tab_flat = tab_flat[filter]
...
def interpolate_flat(image_flat, image_dq, image_err, image_wl, wl):
    ...
    if nz == 1:
        if len(image_dq.shape) == 2:
            ...
            return (image_flat.reshape((ysize, xsize)), image_dq.reshape((ysize, xsize)), image_err.reshape((ysize, xsize)))
...
def flat_for_nirspec_ifu(output_model, f_flat_model, s_flat_model, d_flat_model, dispaxis):
    ...
    for k, ifu_wcs in enumerate(list_of_wcs):
        ...
        if ystop > 2047.5:
            ...
            ystop = 2047.0
        if truncated:
            log.info("WCS bounding_box for stripe %d extended beyond image " "edges, has been truncated to ...", k)
        ...
        flat_2d, flat_dq_2d, flat_err_2d = create_flat_field(
            wl, f_flat_model, s_flat_model, d_flat_model, xstart, xstop, ystart, ystop, exposure_type, dispaxis, None, None
        )
        ...
        if flat_dq.dtype == flat_dq_2d.dtype:
            ...
            log.warning("flat_dq.dtype = {}  flat_dq_2d.dtype = {}".format(flat_dq.dtype, flat_dq_2d.dtype))
...
def flat_for_nirspec_brightobj(output_model, f_flat_model, s_flat_model, d_flat_model, dispaxis):
    ...
    if sum_nan_mask > 0:
        log.debug("Number of NaNs in wavelength array = %d out of %d", sum_nan_mask, sum_nan_mask + sum_good_mask)
        if sum_good_mask < 1:
    ...
    flat_2d, flat_dq_2d, flat_err_2d = create_flat_field(
        wl, f_flat_model, s_flat_model, d_flat_model, xstart, xstop, ystart, ystop, exposure_type, dispaxis, slit_name, None
    )
    mask = flat_2d <= 0.0
    nbad = mask.sum(dtype=np.intp)
    if nbad > 0:
...
def flat_for_nirspec_slit(
    slit, f_flat_model, s_flat_model, d_flat_model, dispaxis, exposure_type, slit_nt, subarray, use_wavecorr
):
    ...
    sum_good_mask = good_mask.sum(dtype=np.intp)
    if sum_nan_mask > 0:
        log.debug(f"Number of NaNs in sci wavelength array = {sum_nan_mask} " f"out of {sum_nan_mask + sum_good_mask}")
        if sum_good_mask < 1:
            log.warning("(all are NaN)")
        # Replace NaNs with a relatively harmless but out-of-bounds value.
        wl[nan_mask] = 0.0
    max_wavelength = np.nanmax(wl)
    if max_wavelength > 0.0 and max_wavelength < MICRONS_100:
        log.warning("Wavelengths in science data appear to be in meters.")

    # Combine the three flat fields for the current subarray.
    flat_2d, flat_dq_2d, flat_err_2d = create_flat_field(
        wl, f_flat_model, s_flat_model, d_flat_model, xstart, xstop, ystart, ystop, exposure_type, dispaxis, slit.name, slit_nt
    )
