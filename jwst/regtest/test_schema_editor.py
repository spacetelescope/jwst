...
def run_editor_full(tmp_cwd_module, keyword_db):
    ...
    editor = se.Schema_editor(input=str(keyword_db), output=FIXED_SCHEMA, add=True, delete=True, edit=True, rename=True)
    editor.change()
...
def test_limit_datamodels_from_file(tmp_cwd_module, model_db, keyword_db, rtdata_module):
    ...
    # Run the editor
    editor = se.Schema_editor(input=str(keyword_db), exclude_file=exclude_path, list=True, rename=True)
    editor.change()
