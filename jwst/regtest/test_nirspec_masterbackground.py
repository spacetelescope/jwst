...
@pytest.mark.bigdata
class TestNIRSpecMasterBackground:

    def test_nirspec_fs_mbkg_user(self, run_nirspec_mbkg, fitsdiff_default_kwargs):
        ...
        rtdata.output = "nrs_sci+bkg_masterbackgroundstep.fits"

        # Get the truth file
        rtdata.get_truth(os.path.join("truth/test_nirspec_fs_mbkg_user", "nrs_sci+bkg_masterbackgroundstep.fits"))

        # Compare the results
        diff = FITSDiff(rtdata.output, rtdata.truth, **fitsdiff_default_kwargs)
        assert diff.identical, diff.report()

    def test_nirspec_ifu_mbkg_user(self, run_nirspec_mbkg, fitsdiff_default_kwargs):
        rtdata, step = run_nirspec_mbkg(
            'nirspec/ifu/prism_sci_bkg_cal.fits',
            user_files=[('--user_background', 'prism_bkg_x1d.fits')]
        )

        rtdata.output = 'prism_sci_bkg_masterbackgroundstep.fits'

        # Get the truth file
        rtdata.get_truth(os.path.join("truth/test_nirspec_ifu_mbkg_user", 'prism_sci_bkg_masterbackgroundstep.fits'))
    ...
    def test_nirspec_mos_mbkg_user(self, run_nirspec_mbkg, fitsdiff_default_kwargs):
        rtdata, _ = run_nirspec_mbkg(
            'nirspec/mos/nrs_mos_sci+bkg_cal.fits',
            user_files=[('--user_background', 'nirspec/mos/nrs_mos_bkg_x1d.fits')]
        )

        rtdata.output = "nrs_mos_sci+bkg_masterbackgroundstep.fits"

        # Get the truth file
        rtdata.get_truth(os.path.join("truth/test_nirspec_mos_mbkg_user", "nrs_mos_sci+bkg_masterbackgroundstep.fits"))

        # Compare the results
        diff = FITSDiff(rtdata.output, rtdata.truth, **fitsdiff_default_kwargs)
        assert diff.identical, diff.report()

    def test_nirspec_ifu_nod_mbkg(self, run_nirspec_mbkg, fitsdiff_default_kwargs):
        rtdata, step = run_nirspec_mbkg(
        ...
        rtdata.output = 'ifu_prism_source_off_fix_NRS1_o001_masterbg.fits'
