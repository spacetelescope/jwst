...
def test_nis_wfss_spec2(
    run_nis_wfss_spec2, rtdata_module, fitsdiff_default_kwargs, suffix
):
...
def test_nis_wfss_spec3(
    run_nis_wfss_spec3, rtdata_module, suffix, source_id, fitsdiff_default_kwargs
):
    ...
    output = (
        "jw01324-o001_" + source_id + "_niriss_f115w-gr150c-gr150r_" + suffix + ".fits"
    )
    rtdata.output = output
    rtdata.get_truth(f"truth/test_niriss_wfss/{output}")
