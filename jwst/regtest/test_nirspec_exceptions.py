...
@pytest.mark.bigdata
def test_nirspec_missing_msa_fail(function_jail, rtdata, fitsdiff_default_kwargs, caplog):
    """
...
@pytest.mark.bigdata
def test_nirspec_missing_msa_nofail(function_jail, rtdata, fitsdiff_default_kwargs, caplog):
    """
        Test of calwebb_spec2 pipeline performed on NIRSpec MSA exposure
...
@pytest.mark.bigdata
def test_nirspec_assignwcs_skip(function_jail, rtdata, fitsdiff_default_kwargs, caplog):
    """
        Test of calwebb_spec2 pipeline performed on NIRSpec MSA exposure
...
@pytest.mark.bigdata
def test_nirspec_nrs2_nodata_strun(function_jail, rtdata, fitsdiff_default_kwargs, caplog):
    """
        Test of calwebb_spec2 pipeline performed on NIRSpec IFU exposure
        that has a filter/grating combination that produces no data on
