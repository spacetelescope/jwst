...
def test_miri_spectral_leak(output, _jail, rtdata, fitsdiff_default_kwargs):
    ...
    # also test that the input file for CH1 is unchanged. To set up the regression test the ch1 input
    # file was copied from the input location to the truth directory
    
    ch1_file = 'jw01538-o001_t019_miri_test_ch1-shortmedium_x1d.fits'  
    rtdata.get_truth(f"truth/test_miri_spectral_leak/{ch1_file}")
    output_second_test = 'test_spectral_leak_asn_0_spectralleakstep.fits'
    rtdata.output= f"{output_second_test}"
    diff = FITSDiff(rtdata.output, rtdata.truth, **fitsdiff_default_kwargs)
    assert diff.identical, diff.report()
