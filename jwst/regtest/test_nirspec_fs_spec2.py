...
def run_pipeline_pixel_replace(rtdata_module):
    ...
    # Run the calwebb_spec2 pipeline; save results from intermediate steps
    args = ["calwebb_spec2", rtdata.input, "--steps.pixel_replace.save_results=true", "--steps.pixel_replace.skip=false"]
    Step.from_cmdline(args)
...
@pytest.mark.parametrize(
    "suffix", ["assign_wcs", "nsclean", "extract_2d", "wavecorr", "flat_field", "pathloss", "srctype", "cal", "s2d", "x1d"]
)
def test_nirspec_fs_spec2(run_pipeline, fitsdiff_default_kwargs, suffix):
    ...
    rtdata = run_pipeline
    output = replace_suffix(os.path.splitext(asn_memberdict[os.path.basename(rtdata.input)][0])[0], suffix) + ".fits"
