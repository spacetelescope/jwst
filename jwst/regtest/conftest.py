...
def generate_artifactory_json(request, artifactory_repos):
    ...
    rtdata = postmortem(request, "rtdata") or postmortem(request, "rtdata_module") or postmortem(request, "sdpdata_module")
    if rtdata:
        try:
        ...
        if rtdata.output and os.path.exists(rtdata.output):
            ...
            if rtdata.okify_op == "file_copy":
                upload_schema = generate_upload_schema(rtdata.output, rtdata.remote_results_path, schema=upload_schema)
