...
class MIRI_AB2Slice(Model):
    ...
    def __init__(
        self, beta_zero=beta_zero, beta_del=beta_del, channel=channel, **kwargs
    ):
        super().__init__(
            beta_zero=beta_zero, beta_del=beta_del, channel=channel, **kwargs
        )
        self.inputs = ("beta",)
        """ "beta": the beta angle """
        self.outputs = ("slice",)
...
class RefractionIndexFromPrism(Model):
    ...
    def __init__(self, prism_angle, name=None):
        super(RefractionIndexFromPrism, self).__init__(
            prism_angle=prism_angle, name=name
        )
        self.inputs = (
            "alpha_in",
            "beta_in",
    ...
    def evaluate(self, alpha_in, beta_in, alpha_out, prism_angle):
        ...
        nsq = (
            ((alpha_out + alpha_in * (1 - 2 * sangle**2)) / (2 * sangle * cangle))
            ** 2
            + alpha_in**2
            + beta_in**2
        )
...
class NirissSOSSModel(Model):
    ...
    def evaluate(self, x, y, spectral_order):
        ...
        try:
            ...
            raise ValueError(
                "Spectral order is not between 1 and 3, {}".format(spectral_order)
            )
...
class Logical(Model):
    ...
    def __repr__(self):
        ...
        return txt.format(
            self.__class__.__name__, self.condition, self.compareto, self.value
        )
...
class V2V3ToIdeal(Model):
    ...
    def evaluate(v2, v3, v3idlyangle, v2ref, v3ref, vparity):
        ...
        xidl = vparity * (
            (v2 - v2ref) * np.cos(v3idlyangle) - (v3 - v3ref) * np.sin(v3idlyangle)
        )
...
class NIRCAMForwardRowGrismDispersion(Model):
    ...
    def __init__(
        self, orders, lmodels=None, xmodels=None, ymodels=None, name=None, meta=None
    ):
        self.orders = orders
...
class NIRCAMForwardColumnGrismDispersion(Model):
    ...
    def evaluate(self, x, y, x0, y0, order):
        ...
        model = (
            Mapping((1, 3, 1, 3, 2, 3, 4))
            | dx & wavelength & Identity(3)
            | Mapping((0, 2, 3, 1, 4))
            | astmath.AddUfunc() & Identity(3)
        )
...
class NIRISSForwardRowGrismDispersion(Model):
    ...
    def evaluate(self, x, y, x0, y0, order):
        ...
        dx = (
            xmodel[0](x00, y00) + t * xmodel[1](x00, y00) + t**2 * xmodel[2](x00, y00)
        )
        dy = (
            ymodel[0](x00, y00) + t * ymodel[1](x00, y00) + t**2 * ymodel[2](x00, y00)
        )
        ...
        model = Mapping((2, 3, 0, 2, 4)) | Const1D(x00) & Const1D(
            y00
...
class NIRISSForwardColumnGrismDispersion(Model):
    ...
    def evaluate(self, x, y, x0, y0, order):
        ...
        model = Mapping((2, 3, 1, 3, 4)) | Const1D(x00) & Const1D(
            y00
        ) & wavelength & Const1D(order)
        return model(x, y, x0, y0, order)
...
class Snell(Model):
    ...
    def __init__(
        self, angle, kcoef, lcoef, tcoef, tref, pref, temperature, pressure, name=None
    ):
