...
class FringeFitter:
    ...
    def fit_fringes_single_integration(self, slc):
        ...
        nrm.make_model(
            fov=self.ctrd.shape[0], bandpass=nrm.bandpass, over=self.oversample, psf_offset=nrm.psf_offset, pixscale=nrm.pixel
        )

        nrm.fit_image(self.ctrd, modelin=nrm.model, psf_offset=nrm.psf_offset, dqm=self.dqslice, weighted=self.weighted)
