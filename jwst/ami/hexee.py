...
def hex_eeAG(s=(121, 121), c=None, d=0.80, lam=4.3e-6, pitch=mas2rad(65)):
    ...
    for index, val in enumerate(xnan):
        h1 = glimit(xnan[index], ynan[index], d=d, c=c, lam=lam, pixel=pitch, minus=False)
        h2 = glimit(xnan[index], ynan[index], d=d, c=c, lam=lam, pixel=pitch, minus=True)
    ...
    if log.getEffectiveLevel() <= logging.DEBUG:
        hr = hex_complex.real
        hi = hex_complex.imag
        log.debug("hex_eeAG: hr.min: %s, hr.mean: %s, hr.max: %s", hr.min(), hr.mean(), hr.max())
        log.debug("hex_eeAG: hi.min: %s, hi.mean: %s, hi.max: %s", hi.min(), hi.mean(), hi.max())
