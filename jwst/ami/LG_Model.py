...
# 07820 - DG; this module was previous nrm_model.py
...
class Nrm_Model:
    ...
    def __init__(self, mask=None, holeshape="circ", pixscale=None,
        ...
        # WARNING! JWST CHOOSEHOLES CODE NOW DUPLICATED IN mask_definitions.py
        ...
        if mask.lower() == 'jwst':
            ...
            # as designed: segments C5 open, C2 closed, meters V2V3 per Paul
            #   Lightsey
            # as built C5 closed, C2 open
            #
            # undistorted pupil coords on PM.  These numbers are considered
            #   immutable.
            # As designed seg -> as built seg in comments each ctr entry
            #   (NO distortion)
    ...
    def simulate(self, fov=None, bandpass=None, over=None, psf_offset=(0, 0)):
        ...
        # next 2 are user-specified
        self.simhdr["psfoff0"] = (psf_offset[0],
                                  "psf ctr at arrayctr + off[0], detpix")

        self.simhdr["psfoff1"] = (psf_offset[1],
                                  "psf ctr at arrayctr + off[1], detpix")
    ...
    def make_model(self, fov=None, bandpass=None, over=1, psf_offset=(0, 0),
        ...
        for w, l in bandpass:  # w: weight, l: lambda (wavelength)
            ...
            self.logger.debug("Passed to model_array: psf_offset: {0}".
                              format(psf_offset))
            self.logger.debug("Primary beam in the model created: {0}".
                              format(pb))
            self.model_beam += pb
            self.fringes += ff
    ...
    def plot_model(self, fits_true=0):
        """
        >>>>> {{ I should change function name if all this does is plot }}
        Short Summary
        -------------
        ...
        Parameters
        ----------
        fits_true: integer {{ >> change to Bool }}
    ...
    def fit_image(self, image, reference=None, pixguess=None, rotguess=0,
        ...
        if modelin is None:  # No model provided
            ...
            self.new_and_better_scaling_routine(self.reference,
                                                scaleguess=self.pixel,
                                                rotstart=rotguess,
                                                centering=self.bestcenter,
                                                fitswrite=self.saveval)
