    ...
    def process(self, images):
        ...
        for ch in chm.keys():
            for c in map(int, list(ch)):
                if c in single_ch:
                    raise ValueError("Channel '{:d}' appears in multiple " "combinations of channels.".format(c))
                single_ch[c] = chm[ch]
        ...
        if hasattr(self.bkg_degree, "__iter__"):
            if len(self.bkg_degree) != 3:
                raise ValueError("The length of 'bkg_degree' parameter must " "be 3 or 'bkg_degree' must be an integer.")
        ...
        for m in models:
            if m.meta.background.subtracted is None:
                if m.meta.background.level is not None:
                    ...
                    raise ValueError("Background level was set but the " "'subtracted' property is undefined " "(None).")

            elif m.meta.background.subtracted:
                raise ValueError(
                    "'cube_skymatch' step cannot be run on " "data whose background has been previously " "subtracted."
                )
...
def apply_background_2d(model2d, channel=None, subtract=True):
    ...
    if isinstance(channel, int):
        ...
        if len(diff) > 0:
            ...
            raise ValueError(
                "Background data for channel(s) '{}' not present " "in 2D model '{}'".format(
                    missing_channels, model2d.meta.filename
                )
            )
        for ch in channel:
            apply_background_2d(model2d, channel=ch, subtract=subtract)
    ...
    if index is None:
        raise ValueError(
            "Background data for channel '{}' not present in " "2D model '{}'".format(channel, model2d.meta.filename)
        )
...
def _match_models(models, channel, degree, center=None, center_cs="image"):
    ...
    if len(cube_models) != len(models):
        raise RuntimeError("The number of generated cube models does not " "match the number of input 2D images.")
    ...
    if wcsinfo is not None and (wcsinfo.crval1 is None or wcsinfo.crval2 is None or wcsinfo.crval3 is None):
    ...
    if np.isnan(bkg_poly_coef).any():
        ...
        for im, poly in zip(models, bkg_poly_coef):
            ...
            im.meta.background.polynomial_info.append(
                {"degree": degree, "refpoint": center, "coefficients": poly.ravel().tolist(), "channel": channel}
            )
