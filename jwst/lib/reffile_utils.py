...
def ref_matches_sci(sci_model, ref_model):
    ...
    log.debug("ref substrt1=%d, subsize1=%d, substrt2=%d, subsize2=%d" % (xstart_ref, xsize_ref, ystart_ref, ysize_ref))
    ...
    if (xstart_sci is None) or (xsize_sci is None) or (ystart_sci is None) or (ysize_sci is None):
        ...
        log.debug("sci substrt1=%d, subsize1=%d, substrt2=%d, subsize2=%d" % (xstart_sci, xsize_sci, ystart_sci, ysize_sci))
    ...
    if xstart_ref == xstart_sci and xsize_ref == xsize_sci and ystart_ref == ystart_sci and ysize_ref == ysize_sci:
        return True
    else:
        return False
...
def get_subarray_data(sci_model, ref_model):
    ...
    ystart_sci = sci_model.meta.subarray.ystart
    ysize_sci = sci_model.meta.subarray.ysize
    log.debug("science xstart=%d, xsize=%d, ystart=%d, ysize=%d", xstart_sci, xsize_sci, ystart_sci, ysize_sci)
    ...
    ystart_ref = ref_model.meta.subarray.ystart
    ysize_ref = ref_model.meta.subarray.ysize
    log.debug("reference xstart=%d, xsize=%d, ystart=%d, ysize=%d", xstart_ref, xsize_ref, ystart_ref, ysize_ref)
    ...
    if xstart < 0 or ystart < 0 or xstop > ref_model.meta.subarray.xsize or ystop > ref_model.meta.subarray.ysize:
        log.error("Computed reference file slice indexes are " + "incompatible with size of reference data array")
        log.error("Science: SUBSTRT1=%d, SUBSTRT2=%d, SUBSIZE1=%d, SUBSIZE2=%d", xstart_sci, ystart_sci, xsize_sci, ysize_sci)
        log.error("Reference: SUBSTRT1=%d, SUBSTRT2=%d, SUBSIZE1=%d, SUBSIZE2=%d", xstart_ref, ystart_ref, xsize_ref, ysize_ref)
