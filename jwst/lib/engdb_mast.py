...
class EngdbMast(EngdbABC):
    ...
    def __init__(self, base_url=None, token=None, **service_kwargs):
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"MAST url: {self.base_url} is not available. Returned HTTPS status {resp.status_code}"
            )
    ...
    def configure(self, base_url=None, token=None):
        ...
        if token is None:
            raise RuntimeError(
                "No MAST token provided but is required. See https://auth.mast.stsci.edu/ for more information."
            )
        self.token = token
