...
class EngdbMast(EngdbABC):
    ...
    def get_values(
        ...
        records = self._get_records(mnemonic=mnemonic, starttime=starttime, endtime=endtime, time_format=time_format)

        # If desired, remove bracket or outside of timeframe entries.
        if not include_bracket_values:
            selection = np.logical_and(records["MJD"] >= starttime.mjd, records["MJD"] <= endtime.mjd)
    ...
    def set_session(self):
        """Setup HTTP session"""
        self._req = requests.Request(method="GET", url=self.base_url + API_URI, headers={"Authorization": f"token {self.token}"})
    ...
    def _get_records(self, mnemonic, starttime, endtime, time_format=None, **other_kwargs):
        """
        Retrieve all results for a mnemonic in the requested time range.
