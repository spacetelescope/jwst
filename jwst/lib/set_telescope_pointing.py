...
def update_wcs_from_fgs_guiding(
    model, t_pars, default_roll_ref=0.0, default_vparity=1, default_v3yangle=0.0
):
    """
...
def calc_transforms_ops_tr_202111(t_pars: TransformParameters):
    ...
    This implements the ECI-to-SIAF transformation from
    Technical Report JWST-STScI-003222, SM-12, Rev. C, 2021-11
    The actual implementation depends on the guide star mode,
    represented by the header keyword PCS_MODE.
...
def calc_j2fgs1_matrix(j2fgs_matrix, transpose=True):
    ...
    if np.isclose(j2fgs_matrix, 0.0).all():
        ...
        logger.info(
            "Using J-Frame to FGS1 engineering parameters for the J-Frame to FGS1 transformation."
        )
...
def calc_sifov_fsm_delta_matrix(fsmcorr, fsmcorr_version="latest", fsmcorr_units="arcsec"):
    ...
    if version == "v1":
        transform = np.array(
            [[1.0, x / 22.01, y / 21.68], [-x / 22.01, 1.0, 0.0], [-y / 21.68, 0.0, 1.0]]
        )
...
def gs_position_acq(mnemonics_to_read, mnemonics, exp_type="fgs_acq1"):  # noqa: ARG001
    """
    ...
    if len(valid) < FGS_ACQ_MINVALUES[exp_type]:
        raise ValueError(
            f"exp_type {exp_type} IFGS_ACQ_XPOSG only has {len(valid)} locations. "
            f"Requires {FGS_ACQ_MINVALUES[exp_type]}"
        )
...
def gs_position_fgtrack(mnemonics_to_read, mnemonics):  # noqa: ARG001
    """
...
def gs_position_id(mnemonics_to_read, mnemonics):  # noqa: ARG001
    """
    ...
    ``IFGS_ID_[X|Y]`` in arcseconds (Ideal system). This mode is when
    the desired guide star is identified.
    As such, there is no position to report. However, when completed,
    the position is then reported for approximately
    five seconds after the end of the exposure. The input mnemonic array needs to have this.
    The first non-zero report is used.
...
def gs_ideal_to_subarray(gs_position, aperture, flip=False):
    ...
    position_subarray = (
        position_pixel[0] - gs_position.corner[0],
...
def calc_wcs_guiding(
    model,
    t_pars: TransformParameters,
    default_roll_ref=0.0,
    default_vparity=1,
    default_v3yangle=0.0,
):
    """
    ...
    try:
        ...
        logger.warning(
            "Cannot determine guide star position from engineering. Defaulting to CRPIX = 0"
        )
