...
__all__ = ["ENGDB_PATH", "cache_engdb"]

# Setup for local testing cache
ENGDB_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data", "engdb")
MNEMONICS_TO_CACHE = [
    "IFGS_ACQ_DETXCOR",
    "IFGS_ACQ_DETYCOR",
    "IFGS_ACQ_DETXSIZ",
...
class EngDB_Mocker(requests_mock.Mocker):
    ...
    def __init__(self, *args, real_http=True, **kwargs):
        ...
        meta_query = re.compile("".join([engdb_direct.ENGDB_METADATA, ".*"]))
        self.get(meta_query, json=self.response_meta)

        # Setup to return a general data query
        data_query = re.compile("".join([engdb_direct.ENGDB_DATA, r".+\?sTime=.+\&eTime=.+"]))
    ...
    def response_data(self, request, context):
        ...
        """
        mnemonic = os.path.basename(request.path)
        data = self.cache.fetch_data(mnemonic, request.qs["stime"][0], request.qs["etime"][0])
        context.status_code = 200
        return data
...
class EngDB_Local:
    ...
    def fetch_meta(self, mnemonic_substr=""):
        ...
        RESTful interface
        """
        with open(os.path.join(self.db_path, META), "r") as fp:
            meta = json.load(fp)
        ...
        mnemonics = [
            tlmmnemonics[idx] for idx, mnemonic in enumerate(tlmmnemonics) if mnemonic_substr in mnemonic["TlmMnemonic"].lower()
        ]
    ...
    def json_load(self, mnemonic):
        with open(os.path.join(self.db_path, mnemonic_data_fname(mnemonic)), "r") as fp:
...
# Utilities
# #########
def cache_engdb(mnemonics=MNEMONICS_TO_CACHE, starttime="2016-01-18T15:40:00", endtime="2016-01-18T15:40:05", db_path=ENGDB_PATH):
    ...
    for mnemonic in mnemonics:
        ...
        with open(os.path.join(db_path, mnemonic_data_fname(mnemonic)), "w") as fp:
