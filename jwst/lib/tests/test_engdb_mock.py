...
def test_cache_engdb(engdb):
    ...
    with TemporaryDirectory() as db_path:
        engdb_mock.cache_engdb(mnemonics=[GOOD_MNEMONIC], starttime=GOOD_STARTTIME, endtime=GOOD_ENDTIME, db_path=db_path)

        assert os.path.isfile(os.path.join(db_path, engdb_mock.META))
        assert os.path.isfile(os.path.join(db_path, engdb_mock.mnemonic_data_fname(GOOD_MNEMONIC)))
...
def test_cache_partial_data(db_cache, engdb):
    ...
    endtime = Time(engdb_direct.extract_db_time(data["Data"][1]["ObsTime"]) / 1000.0, format="unix")
...
def test_mocker_data(db_cache, engdb):
    with engdb_mock.EngDB_Mocker(db_path=db_cache.db_path):
        query = "".join(
            [ENGDB_MOCK_URL, engdb_direct.ENGDB_DATA, GOOD_MNEMONIC, "?sTime=", GOOD_STARTTIME, "&eTime=", GOOD_ENDTIME]
        )
        response = requests.get(query)
