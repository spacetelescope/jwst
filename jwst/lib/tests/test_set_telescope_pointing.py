...
@pytest.fixture(
    params=[("good_model", True), ("bad_model", False), ("fits_nomodel", False)]
)
...
def test_override_calc_wcs():
    ...
    assert all(
        np.isclose(
            vinfo_new,
            stp.WCSRef(
                ra=32.504066294908846, dec=17.16116653015435, pa=352.2757851954435
            ),
        )
    )
...
def test_override(attribute, expected):
    ...
    to_override = stp.Transforms(
        m_eci2j="original", m_j2fgs1="untouched", override=overrides
    )
...
def test_get_pointing_fail():
    with pytest.raises(Exception):
        (q, j2fgs_matrix, fmscorr, obstime, gs_commanded) = stp.get_pointing(
            47892.0, 48256.0
        )
...
def test_get_pointing(eng_db_ngas):
    (
        ...
        fgsid,
        gs_position,
    ) = stp.get_pointing(STARTTIME.mjd, ENDTIME.mjd, engdb_url="http://localhost")
    ...
    with datamodels.Level1bModel(data_file) as model:
        ...
        with datamodels.open(DATA_PATH / expected_name) as expected:
            ...
            assert word_precision_check(
                model.meta.wcsinfo.s_region, expected.meta.wcsinfo.s_region
            )
...
def test_add_wcs_method_full_nosiafdb(eng_db_ngas, data_file, tmp_path):
    ...
    # Calculate
    stp.add_wcs(
        data_file, method=stp.Methods.OPS_TR_202111, engdb_url="http://localhost"
    )
...
def make_t_pars(detector="any", fgsid_telem=1, fgsid_user=None):
    ...
    t_pars.guide_star_wcs = stp.WCSRef(
        ra=241.24294932221, dec=70.66165389073196, pa=None
    )
...
def calc_coarse_202111_fgsid(request, tmp_path_factory):
    ...
    transforms.write_to_asdf(
        tmp_path_factory.mktemp("transforms")
        / f"tforms_{t_pars.method}{truth_ext}.asdf"
    )
...
def calc_transforms(request, tmp_path_factory):
    ...
    # Save transforms for later examination
    transforms.write_to_asdf(
        tmp_path_factory.mktemp("transforms") / f"tforms_{request.param}.asdf"
    )
