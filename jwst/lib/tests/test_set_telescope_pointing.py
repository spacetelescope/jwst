...
Q_EXPECTED = np.asarray([-0.36915286, 0.33763282, 0.05758533, 0.86395264])
...
def test_override_calc_wcs():
    ...
    override = stp.Transforms(
        m_eci2j=np.array(
            [[0.80583682, 0.51339893, 0.29503999], [-0.56953229, 0.8083677, 0.14891175], [-0.16204967, -0.28803339, 0.94380971]]
        )
    )
    ...
    assert all(np.isclose(vinfo_new, stp.WCSRef(ra=32.504066294908846, dec=17.16116653015435, pa=352.2757851954435)))


@pytest.mark.parametrize("attribute, expected", [("m_eci2j", "overridden"), ("m_j2fgs1", "untouched")])
...
def test_change_engdb_url():
    ...
    with pytest.raises(ValueError):
        stp.get_pointing(STARTTIME.mjd, ENDTIME.mjd, engdb_url=engdb_mast.MAST_BASE_URL)
...
def test_get_pointing_with_zeros(eng_db_ngas):
    (q, j2fgs_matrix, fsmcorr, obstime, gs_commanded, fgsid, gs_position) = stp.get_pointing(
        ZEROTIME_START.mjd, ENDTIME.mjd, reduce_func=stp.first_pointing, engdb_url="http://localhost"
    )
    assert j2fgs_matrix.any()
    (q_desired, j2fgs_matrix_desired, fsmcorr_desired, obstime, gs_commanded, fgsid, gs_position) = stp.get_pointing(
        STARTTIME.mjd, ENDTIME.mjd, engdb_url="http://localhost"
    )
...
def test_add_wcs_default(data_file, tmp_path):
    ...
    try:
        stp.add_wcs(data_file, siaf_path=siaf_path, tolerance=0, allow_default=True)
    except ValueError:
...
def test_add_wcs_default_fgsacq(tmp_path):
    ...
    with datamodels.Level1bModel(DATA_PATH / "add_wcs_default_acq1.fits") as model:
        ...
        try:
            stp.update_wcs(model, siaf_path=siaf_path, tolerance=0, allow_default=True)
        except ValueError:
            pass  # This is what we want for the test.
        except Exception as e:
            pytest.skip("Live ENGDB service is not accessible." "\nException={}".format(e))
...
def test_add_wcs_default_nosiaf(data_file_nosiaf, caplog):
    ...
    with pytest.raises(ValueError):
        stp.add_wcs(data_file_nosiaf, siaf_path=siaf_path, tolerance=0, allow_default=True)
...
def test_add_wcs_with_mast(data_file_fromsim, fgsid, tmp_path):
    ...
    try:
        stp.add_wcs(data_file_fromsim, siaf_path=siaf_path, engdb_url=engdb_mast.MAST_BASE_URL, fgsid=fgsid)
