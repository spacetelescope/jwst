...
IMAGING_TYPES = set(
    tuple(ACQ_EXP_TYPES) + tuple(IMAGE2_SCIENCE_EXP_TYPES) + tuple(IMAGE2_NONSCIENCE_EXP_TYPES) + ("fgs_image", "fgs_focus")
)
...
def is_moving_target(input_models):
    ...
    if hasattr(model.meta.target, "type") and model.meta.target.type is not None and model.meta.target.type.lower() == "moving":
        return True
    return False
