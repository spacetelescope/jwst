    ...
    if (PyArray_CheckExact(obj) && PyArray_IS_C_CONTIGUOUS((PyArrayObject *)obj) &&
        ...
        *is_copy = 1;
        return (PyArrayObject *)PyArray_FromAny(
            obj, PyArray_DescrFromType(npy_dbl), 0, 0, NPY_ARRAY_CARRAY | NPY_ARRAY_FORCECAST, NULL);
    }
}
...
get_clipped_pixels(PyObject *module, PyObject *args)
{
    ...
    status = clip_pixels(
        n, (dbl_type *)PyArray_DATA(xc), (dbl_type *)PyArray_DATA(yc), padding, nx, ny, w, h, &npix, &x, &y, &areas,
        &idx);
