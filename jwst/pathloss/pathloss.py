...
def get_center(exp_type, input, offsets=False):
    ...
    if exp_type == "NRS_IFU":
        ...
        xcenter, ycenter = sky_to_det(input.meta.target.ra, input.meta.target.dec, ref_wave)
...
def calculate_pathloss_vector(pathloss_refdata, pathloss_wcs, xcenter, ycenter, calc_wave=True):
    ...
    # at the specified location.  We do this using bilinear interpolation
        ...
        if object_colindex < 0 or object_colindex >= (ncols - 1) or object_rowindex < 0 or object_rowindex >= (nrows - 1):
...
def do_correction(input_model, pathloss_model=None, inverse=False, source_type=None, correction_pars=None, user_slit_loc=None):
    ...
    if not pathloss_model and not correction_pars:
        raise RuntimeError("Neither a PathLossModel nor PathLossStep correction parameters given." " One needs to be specified.")
    exp_type = input_model.meta.exposure.type
    log.info(f"Input exposure type is {exp_type}")
    output_model = input_model.copy()

    if exp_type == "NRS_MSASPEC":
        corrections = do_correction_mos(output_model, pathloss_model, inverse, source_type, correction_pars)
    elif exp_type in ["NRS_FIXEDSLIT", "NRS_BRIGHTOBJ"]:
        corrections = do_correction_fixedslit(output_model, pathloss_model, inverse, source_type, correction_pars)
    elif exp_type == "NRS_IFU":
        corrections = do_correction_ifu(output_model, pathloss_model, inverse, source_type, correction_pars)
...
def interpolate_onto_grid(wavelength_grid, wavelength_vector, pathloss_vector):
    ...
    denominator = extended_wavelength_vector[upper_indices] - extended_wavelength_vector[lower_indices]
...
def do_correction_soss(data, pathloss):
    ...
    if pupil_wheel_position is None:
        log.warning("Unable to get pupil wheel position from PWCPOS keyword " f"for {data.meta.filename}")
    ...
    if aperture is None:
        log.warning("Unable to get Aperture from reference file " f"for subarray {subarray}")
        log.warning("Pathloss correction skipped")
        data.meta.cal_step.pathloss = "SKIPPED"
        return
    ...
    if pupil_wheel_index < 0 or pupil_wheel_index > (ncols - 2):
        ...
        for row in range(data_ncols):
            ...
            if refrow_index < 0 or refrow_index > (nrows - 1):
                ...
                correction[row] = (1.0 - dx) * pathloss_array[refrow_index, ix] + dx * pathloss_array[refrow_index, ix + 1]
...
def _corrections_for_mos(slit, pathloss, exp_type, source_type=None):
    ...
    if size > 0:
        ...
        if aperture is not None:
            (wavelength_pointsource, pathloss_pointsource_vector, is_inside_slitlet) = calculate_pathloss_vector(
                aperture.pointsource_data, aperture.pointsource_wcs, xcenter, ycenter
            )
            if two_shutters:
                (wavelength_uniformsource, pathloss_uniform_vector) = calculate_two_shutter_uniform_pathloss(pathloss)
            else:
            ...
            if is_inside_slitlet:
                ...
                pathloss_2d_ps = interpolate_onto_grid(wavelength_array, wavelength_pointsource, pathloss_pointsource_vector)

                # Compute the uniform source pathloss 2D correction
                pathloss_2d_un = interpolate_onto_grid(wavelength_array, wavelength_uniformsource, pathloss_uniform_vector)
            ...
            log.warning("Cannot find matching pathloss model for slit with" f"{slitlength} shutters")
    else:
...
def _corrections_for_fixedslit(slit, pathloss, exp_type, source_type):
    ...
    if aperture is not None:
        ...
        (wavelength_pointsource, pathloss_pointsource_vector, is_inside_slit) = calculate_pathloss_vector(
            aperture.pointsource_data, aperture.pointsource_wcs, xcenter, ycenter
        )
        (wavelength_uniformsource, pathloss_uniform_vector, dummy) = calculate_pathloss_vector(
            aperture.uniform_data, aperture.uniform_wcs, xcenter, ycenter
        ...
        if is_inside_slit:
            ...
            if is_pointsource(source_type or slit.source_type):
                ...
                # Compute the point source pathloss 2D correction
                pathloss_2d_ps = interpolate_onto_grid(wavelength_array_corr, wavelength_pointsource, pathloss_pointsource_vector)

                # Compute the uniform source pathloss 2D correction
                pathloss_2d_un = interpolate_onto_grid(wavelength_array_uncorr, wavelength_uniformsource, pathloss_uniform_vector)
            ...
            log.warning("Source is outside slit. Skipping " f"pathloss correction for slit {slit.name}")
    else:
...
def _corrections_for_lrs(data, pathloss, user_slit_loc):
    ...
    if user_slit_loc is None:
        ...
        location = (ra, dec, wav)
        scale_degrees = util.compute_scale(data.meta.wcs, location, disp_axis=data.meta.wcsinfo.dispersion_direction)
