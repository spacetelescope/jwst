...
def flag_pixels(data, gdq, signal_threshold):
    ...
    for ii_int in range(n_ints):
        for ii_grp in range(n_grps):
            wh_set_flag = np.argwhere(lowest_exc_2d == ii_grp)
            ...
            if len(wh_set_flag) > 0 and len(xx) > 0:  # there are exceedances
                ...
                xx_m1 = xx[xx > 0] - 1   # WEST
                if len(xx_m1) > 0:
                    gdq[ii_int, ii_grp:, yy, xx_m1] = \
                        np.bitwise_or(gdq[ii_int, ii_grp:, yy, xx_m1],
                                      dqflags.group['UNDERSAMP'] |
                                      dqflags.group['DO_NOT_USE'])

                xx_max_p1 = xx[xx < (ncols-1)] + 1   # EAST
                if len(xx_max_p1) > 0:
                    gdq[ii_int, ii_grp:, yy, xx_max_p1] = \
                        np.bitwise_or(gdq[ii_int, ii_grp:, yy, xx_max_p1],
                                      dqflags.group['UNDERSAMP'] |
                                      dqflags.group['DO_NOT_USE'])

                yy_m1 = yy[yy > 0] - 1   # NORTH
                if len(yy_m1) > 0:
                    gdq[ii_int, ii_grp:, yy_m1, xx] = \
                        np.bitwise_or(gdq[ii_int, ii_grp:, yy_m1, xx],
                                      dqflags.group['UNDERSAMP'] |
                                      dqflags.group['DO_NOT_USE'])

                yy_max_p1 = yy[yy < (nrows-1)] + 1  # SOUTH
                if len(yy_max_p1) > 0:
                    gdq[ii_int, ii_grp:, yy_max_p1, xx] = \
                        np.bitwise_or(gdq[ii_int, ii_grp:, yy_max_p1, xx],
                                      dqflags.group['UNDERSAMP'] |
                                      dqflags.group['DO_NOT_USE'])
