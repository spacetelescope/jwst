...
def no_NaN(input_model, fill_value, zap_nan=False, zap_zero=False):
...
class DataSet:
    ...
    def __init__(
        self, output_obj, input_traps_filled, flag_pers_cutoff, save_persistence, trap_density_model, trappars_model, persat_model
    ):
    ...
    def do_all(self):
        ...
        if have_traps_filled:  # was there an actual traps_filled file?
            ...
            # over integrations.
            to_start = (self.output_obj.meta.exposure.start_time - self.traps_filled.meta.exposure.end_time) * 86400.0
        ...
        if hasattr(self.trap_density, "dq"):
            mask = np.bitwise_and(self.trap_density.dq, dqflags.pixel["DO_NOT_USE"]) > 0
        ...
        for integ in range(nints):
            ...
            for group in range(ngroups):
                ...
                for k in range(nfamilies):
                    ...
                    if integ > 0 and group == 0 and self.nresets > 0:
                        ...
                        decay_during_reset = self.compute_decay(self.traps_filled.data[k], decay_param_k, reset_time)
                        self.traps_filled.data[k, :, :] -= decay_during_reset
                    # Decays during current group, for current trap family.
                    decayed_in_group = self.compute_decay(self.traps_filled.data[k], decay_param_k, t_group)
            ...
            for k in range(nfamilies):
                ...
                filled = self.predict_capture(capture_param_k, self.trap_density.data, integ, grp_slope, slope)
                if is_subarray:
    ...
    def predict_capture(self, capture_param_k, trap_density, integ, grp_slope, slope):
        ...
        if any_saturated:
            ...
            filled[mask] = self.predict_saturation_capture(
                capture_param_k, trap_density[mask], ramp_traps_filled[mask], sattime[mask], sat_count[mask], ngroups
            )
        del sat_count, sattime, ramp_traps_filled, mask

        # Traps that were filled due to cosmic-ray jumps.
        cr_filled = self.delta_fcn_capture(capture_param_k, trap_density, integ, grp_slope, ngroups, t_group)
        filled += cr_filled
    ...
    def predict_ramp_capture(self, capture_param_k, trap_density, slope, dt):
        ...
        traps_filled = (
            trap_density
            * slope**2
            * (dt**2 * (par0 + par2) / 2.0 + par0 * (dt * tau + tau**2) * np.exp(-dt / tau) - par0 * tau**2)
        )

        traps_filled *= SCALEFACTOR
        return traps_filled

    def predict_saturation_capture(self, capture_param_k, trap_density, incoming_filled_traps, sattime, sat_count, ngroups):
    ...
    def delta_fcn_capture(self, capture_param_k, trap_density, integ, grp_slope, ngroups, t_group):
        ...
        for group in range(1, ngroups):
            ...
            if len(cr_flag[0]) > 0:
                ...
                jump = (data[z, cr_flag[0], cr_flag[1]] - data[z_prev, cr_flag[0], cr_flag[1]]) - grp_slope[cr_flag]
                jump = np.where(jump < 0.0, 0.0, jump)
                cr_filled[cr_flag] += trap_density[cr_flag] * jump * (par0 * (1.0 - math.exp(par1 * delta_t)) + par2)
