...
class DataSet:
    ...
    def calc_nirspec(self, ftab, area_fname):
        ...
        # IFU and MSA exposures use one set of flux cal data
            ...
            # IFU data
                ...
                if not self.inverse:
                    sens2d_fac = sens2d
                    sens2d_sq_fac = sens2d * sens2d
                else:
                    sens2d_fac = 1 / sens2d
                    sens2d_sq_fac = 1 / (sens2d * sens2d)

                self.input.data *= sens2d_fac
                self.input.err *= sens2d_fac
                self.input.var_poisson *= sens2d_sq_fac
                self.input.var_rnoise *= sens2d_sq_fac

                if self.input.var_flat is not None and np.size(self.input.var_flat) > 0:
                    self.input.var_flat *= sens2d_sq_fac
    ...
    def calc_niriss(self, ftab):
        ...
        if isinstance(self.input, datamodels.MultiSlitModel):
            ...
            raise DataModelTypeError(f"Unexpected input data model type for NIRISS: {str(self.input)}")
