...
class DataSet:
    ...
    def __init__(self, model, inverse=False, source_type=None, mrs_time_correction=False, correction_pars=None):
        """
        Short Summary
        -------------
        ...
        if self.detector == "MIRIMAGE":
            ...
            try:
                ...
                data = np.array(
                    ...
                    dtype=[("filter", "O"), ("subarray", "O"), ("photmjsr", "<f4"), ("uncertainty", "<f4")],
                )
    ...
    def photom_io(self, tabdata, order=None):
        ...
        if isinstance(self.input, datamodels.MultiSlitModel):
            ...
            self.input.slits[self.slitnum].meta.photometry.conversion_microjanskys = conversion * MJSR_TO_UJA2
        elif isinstance(self.input, datamodels.MultiSpecModel):
            ...
            if isinstance(self.input, datamodels.MultiSlitModel):
                ...
                if self.exptype.upper() == "NRS_FIXEDSLIT" and slit.source_type.upper() == "POINT":
                    ...
                    conversion_2d_uniform, no_cal = self.create_2d_conversion(
                        slit, self.exptype, conversion_uniform, waves, relresps, order, use_wavecorr=False
                    )
                    ...
                    conversion, no_cal = self.create_2d_conversion(
                        slit, self.exptype, conversion, waves, relresps, order, include_dispersion=True
                    )

                else:
                    conversion, no_cal = self.create_2d_conversion(slit, self.exptype, conversion, waves, relresps, order)

            elif isinstance(self.input, datamodels.MultiSpecModel):
                # This input does not require a 2d conversion, but a 1d interpolation on the
                # input wavelength vector to find the relresponse.
                conversion, no_cal = self.create_1d_conversion(self.input.spec[self.specnum], conversion, waves, relresps)
            else:
                # NRC_TSGRISM data produces a SpecModel, which is handled here
                if self.exptype in ["NRC_WFSS", "NRC_TSGRISM"]:
                    log.info("Including spectral dispersion in 2-d flux calibration")
                    conversion, no_cal = self.create_2d_conversion(
                        self.input, self.exptype, conversion, waves, relresps, order, include_dispersion=True
                    )

                else:
                    conversion, no_cal = self.create_2d_conversion(self.input, self.exptype, conversion, waves, relresps, order)
    ...
    def create_2d_conversion(
        self, model, exptype, conversion, waves, relresps, order, use_wavecorr=None, include_dispersion=False
    ):
        """
        ...
        if exp_type == "NRS_MSASPEC":
            ...
            for slit in self.input.slits:
                ...
                if n_matches != 1:
                    ...
                    slit.meta.photometry.pixelarea_steradians = slit.meta.photometry.pixelarea_arcsecsq * A2_TO_SR
            if n_failures > 0:
                log.warning(
                    "%d failures out of %d, matching MSA data to " "area reference file", n_failures, len(self.input.slits)
                )
            ...
            for k in range(nrows):
                if slit_id[k] == slit_name:
                    ...
                    self.input.meta.photometry.pixelarea_steradians = self.input.meta.photometry.pixelarea_arcsecsq * A2_TO_SR
                    break
            if not foundit:
            ...
            log.warning(
                "EXP_TYPE of NIRSpec data is %s, which is not an " "expected value; pixel area keywords will be set to 1.",
                exp_type,
            )
            self.input.meta.photometry.pixelarea_arcsecsq = 1.0
            self.input.meta.photometry.pixelarea_steradians = 1.0
    ...
    def apply_photom(self, photom_fname, area_fname):
        ...
        if self.instrument == "NIRISS":
            ...
            raise RuntimeError("Instrument {} is not recognized".format(self.instrument))
