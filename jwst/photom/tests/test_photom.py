...
def create_input(instrument, detector, exptype, filter=None, pupil=None, grating=None, band=None):
...
def create_photom_nrs_fs(min_wl=1.0, max_wl=5.0, min_r=8.0, max_r=9.0):
    ...
    reftab = np.array(
        list(zip(filter, grating, slit, photmj, uncertainty, nelem, wavelength, relresponse, reluncertainty)), dtype=dtype
    )
...
def create_photom_nrs_msa(min_wl=1.0, max_wl=5.0, min_r=8.0, max_r=9.0):
    ...
    reftab = np.array(
        list(zip(filter, grating, photmj, uncertainty, nelem, wavelength, relresponse, reluncertainty)), dtype=dtype
    )
...
def create_photom_niriss_wfss(min_wl=1.0, max_wl=5.0, min_r=8.0, max_r=9.0):
    ...
    filter = ["GR150C", "GR150C", "GR150C", "GR150C", "GR150R", "GR150R", "GR150R", "GR150R"]
    ...
    reftab = np.array(
        list(zip(filter, pupil, order, photmjsr, uncertainty, nelem, wavelength, relresponse, reluncertainty)), dtype=dtype
    )
...
def create_photom_niriss_soss(min_r=8.0, max_r=9.0):
    ...
    reftab = np.array(
        list(zip(filter, pupil, order, photmj, uncertainty, nelem, wavelength, relresponse, reluncertainty)), dtype=dtype
    )
...
def create_photom_niriss_image(min_r=8.0, max_r=9.0):
    ...
    dtype = np.dtype([("filter", "S12"), ("pupil", "S12"), ("photmjsr", "<f4"), ("uncertainty", "<f4")])
...
def create_photom_miri_lrs(min_wl=5.0, max_wl=10.0, min_r=8.0, max_r=9.0):
    ...
    reftab = np.array(
        list(zip(filter, subarray, photmjsr, uncertainty, nelem, wavelength, relresponse, reluncertainty)), dtype=dtype
    )
...
def create_photom_miri_image(min_wl=16.5, max_wl=19.5, min_r=8.0, max_r=9.0):
    ...
    dtype = np.dtype([("filter", "S12"), ("subarray", "S15"), ("photmjsr", "<f4"), ("uncertainty", "<f4")])
    ...
    reftabc = np.array(list(zip(timecoeff_amp, timecoeff_tau, timecoeff_t0)), dtype=dtypec)
...
def create_photom_nircam_wfss(min_wl=2.4, max_wl=5.0, min_r=8.0, max_r=9.0):
    ...
    reftab = np.array(list(zip(filter, pupil, order, photmjsr, uncertainty, nelem, wavelength, relresponse, relunc)), dtype=dtype)
...
def create_msa_pixel_area_ref(quadrant, shutter_x, shutter_y, pixarea):
    ...
    dtype = np.dtype([("quadrant", "<i2"), ("shutter_x", "<i2"), ("shutter_y", "<i2"), ("pixarea", "<f4")])
...
def test_nirspec_fs():
    ...
    input_model = create_input("NIRSPEC", "NRS1", "NRS_FIXEDSLIT", filter="F170LP", grating="G235M")
    ...
    for k, slit in enumerate(save_input.slits):
        ...
        rownum = find_row_in_ftab(save_input, ftab, ["filter", "grating"], slitname, order=None)
        ...
        ratio_var_p = np.sqrt(ds.input.slits[k].var_poisson[iy, ix] / slit.var_poisson[iy, ix])
        result.append(np.allclose(ratio_var_p, compare, rtol=1.0e-7))
        ratio_var_r = np.sqrt(ds.input.slits[k].var_rnoise[iy, ix] / slit.var_rnoise[iy, ix])
        result.append(np.allclose(ratio_var_r, compare, rtol=1.0e-7))
        ratio_var_f = np.sqrt(ds.input.slits[k].var_flat[iy, ix] / slit.var_flat[iy, ix])
...
def test_nirspec_bright():
    ...
    input_model = create_input("NIRSPEC", "NRS1", "NRS_BRIGHTOBJ", filter="F170LP", grating="G235M")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
    ...
    result.append(np.allclose(ratio_err, compare, rtol=1.0e-7))
    ratio_var_p = np.sqrt(ds.input.var_poisson[:, iy, ix] / save_input.var_poisson[:, iy, ix])
    result.append(np.allclose(ratio_var_p, compare, rtol=1.0e-7))
    ratio_var_r = np.sqrt(ds.input.var_rnoise[:, iy, ix] / save_input.var_rnoise[:, iy, ix])
    result.append(np.allclose(ratio_var_r, compare, rtol=1.0e-7))
...
def test_nirspec_msa():
    ...
    input_model = create_input("NIRSPEC", "NRS1", "NRS_MSASPEC", filter="F170LP", grating="G235M")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
    ...
    rownum = find_row_in_ftab(save_input, ftab, ["filter", "grating"], slitname=None, order=None)
    photmj = ftab.phot_table["photmj"][rownum]
    nelem = ftab.phot_table["nelem"][rownum]
    wavelength = ftab.phot_table["wavelength"][rownum][0:nelem]
    relresponse = ftab.phot_table["relresponse"][rownum][0:nelem]
...
def test_niriss_wfss():
    ...
    input_model = create_input("NIRISS", "NIS", "NIS_WFSS", filter="GR150R", pupil="F140M")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
...
def test_niriss_soss():
    ...
    input_model = create_input("NIRISS", "NIS", "NIS_SOSS", filter="CLEAR", pupil="GR700XD")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
...
def test_niriss_image():
    ...
    input_model = create_input("NIRISS", "NIS", "NIS_IMAGE", filter="CLEAR", pupil="F140M")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
...
def test_miri_mrs():
    ...
    input_model = create_input("MIRI", "MIRIFULONG", "MIR_MRS", filter="F1500W", band="LONG")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
    value = 1.436
    pixel_area = 0.0436
    ...
    ftab = create_photom_miri_mrs(shape, value=value, pixel_area=pixel_area, photmjsr=photmjsr)
    ds.calc_miri(ftab)
    ...
    result.append(math.isclose(photmjsr, ds.input.meta.photometry.conversion_megajanskys, rel_tol=1.0e-12))
    result.append(math.isclose(photmjsr * MJSR_TO_UJA2, ds.input.meta.photometry.conversion_microjanskys, rel_tol=1.0e-12))
...
def test_nircam_image():
    ...
    input_model = create_input("NIRCAM", "NRCA3", "NRC_IMAGE", filter="F150W", pupil="CLEAR")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
...
def test_nircam_spec():
    ...
    input_model = create_input("NIRCAM", "NRCALONG", "NRC_WFSS", filter="F356W", pupil="GRISMR")
    save_input = input_model.copy()
    ds = photom.DataSet(input_model)
...
def test_apply_photom_1():
    ...
    assert math.isclose(output_model.meta.photometry.pixelarea_steradians, area_ster, rel_tol=1.0e-7)
    assert math.isclose(output_model.meta.photometry.pixelarea_arcsecsq, area_a2, rel_tol=1.0e-7)
