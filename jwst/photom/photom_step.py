...
class PhotomStep(Step):
    ...
    def process(self, input):
        ...
        if model_type not in ("CubeModel", "ImageModel", "SlitModel", "IFUImageModel", "MultiSlitModel", "MultiSpecModel"):
            self.log.warning(
        ...
        try:
            # Do the correction
            phot = photom.DataSet(input_model, self.inverse, self.source_type, self.mrs_time_correction, correction_pars)
            ...
            self.log.error(
                f"Unexpected data model type {model_type} for "
