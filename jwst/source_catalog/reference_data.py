...
class ReferenceData:
    ...
    def _aperture_ee_table(self):
        ...
        if len(ee_table) == 0:
            raise RuntimeError("APCORR reference file data is missing for " f"{selector}.")
    ...
    def _get_ee_table_row(self, aperture_ee):
        ...
        if len(ee_row) > 1:
            raise RuntimeError("More than one matching row was found in " "the APCORR reference file " f"{self.apcorr_filename}")
        return ee_row
    ...
    def aperture_params(self):
        ...
        if self.apcorr_filename is None:
            log.warning(
                "APCorrModel reference file was not input. Using " "fallback aperture sizes without any aperture " "corrections."
            )
        ...
        if len(skyins) != 1 or len(skyouts) != 1:
            raise RuntimeError(
                "Expected to find only one value for skyin " "and skyout in the APCORR reference file for " "a given selector."
            )
    ...
    def abvega_offset(self):
        ...
        if self.abvegaoffset_filename is None:
            log.warning("ABVEGAOFFSET reference file was not input. " "Catalog Vega magnitudes are not correct.")
            return 0.0
        ...
        try:
            ...
            raise KeyError(f"{badkey} not found in ABVEGAOFFSET reference " f"file {self.abvegaoffset_filename}")

        row = offsets_table[np.logical_and.reduce(mask_idx)]

        if len(row) == 0:
            raise RuntimeError("Did not find matching row in ABVEGAOFFSET " f"reference file {self.abvegaoffset_filename}")
        if len(row) > 1:
            raise RuntimeError(
                "Found more than one matching row in " "ABVEGAOFFSET reference file " f"{self.abvegaoffset_filename}"
            )
