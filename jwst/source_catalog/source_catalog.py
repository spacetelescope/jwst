...
class JWSTSourceCatalog:
    ...
    *all* sources of error, including the Poisson error of the sources,
    ...
    def __init__(self, model, segment_img, convolved_data, kernel_fwhm, aperture_params, abvega_offset, ci_star_thresholds):
    ...
    def segment_colnames(self):
        ...
        desc["ellipticity"] = "1 minus the ratio of the 1-sigma lengths of " "the semimajor and semiminor axes"
        desc["orientation"] = (
        ...
        desc["sky_bbox_ll"] = "Sky coordinate of the lower-left vertex of " "the minimal bounding box of the source"
        desc["sky_bbox_ul"] = "Sky coordinate of the upper-left vertex of " "the minimal bounding box of the source"
        desc["sky_bbox_lr"] = "Sky coordinate of the lower-right vertex of " "the minimal bounding box of the source"
        desc["sky_bbox_ur"] = "Sky coordinate of the upper-right vertex of " "the minimal bounding box of the source"
    ...
    def set_segment_properties(self):
        ...
        segm_cat = SourceCatalog(
            self.model.data, self.segment_img, convolved_data=self.convolved_data << u.Jy, error=self.model.err, wcs=self.wcs
        )
    ...
    def sky_orientation(self):
        ...
        skycoord = self.wcs.pixel_to_world(self.model.meta.wcsinfo.crpix1 - 1, self.model.meta.wcsinfo.crpix2 - 1)
    ...
    def aperture_colnames(self):
        ...
        desc["aper_bkg_flux_err"] = "The standard error of the " "sigma-clipped median background value"
    ...
    def set_aperture_properties(self):
        ...
        The values are set as dynamic attributes.
        """
        apertures = [CircularAperture(self._xypos_finite, radius) for radius in self.aperture_params["aperture_radii"]]
    ...
    def ci_colname_descriptions(self):
        ...
        """
        return [
            "Concentration index calculated as " f"({self.aperture_flux_colnames[2*j]} / " f"{self.aperture_flux_colnames[2*i]})"
    ...
    def concentration_indices(self):
        ...
        """
        fluxes = [(self.aperture_flux_colnames[2 * j], self.aperture_flux_colnames[2 * i]) for (i, j) in self._ci_ee_indices]
    ...
    def _daofind_convolved_data(self):
        ...
        """
        return ndimage.convolve(self.model.data.value, self._daofind_kernel, mode="constant", cval=0.0)
    ...
    def _daofind_cutout(self):
        ...
        for xcen, ycen in zip(*np.transpose(self._xypos_finite)):
            try:
                cutout_ = extract_array(self.model.data, self._daofind_kernel.shape, (ycen, xcen), fill_value=0.0)
    ...
    def _daofind_cutout_conv(self):
        ...
        for xcen, ycen in zip(*np.transpose(self._xypos_finite)):
            try:
                cutout_ = extract_array(self._daofind_convolved_data, self._daofind_kernel.shape, (ycen, xcen), fill_value=0.0)
            except NoOverlapError:
                cutout_ = np.zeros(self._daofind_kernel.shape)
            cutout.append(cutout_)
    ...
    def roundness(self):
        ...
        axis = (1, 2)
        sum2 = -quad1.sum(axis=axis) + quad2.sum(axis=axis) - quad3.sum(axis=axis) + quad4.sum(axis=axis)
        sum2[sum2 == 0] = 0.0
    ...
    def aper_total_flux(self):
        ...
        flux = self.aperture_params["aperture_corrections"][idx] * getattr(self, self.aperture_flux_colnames[idx * 2])
        return flux
    ...
    def aper_total_flux_err(self):
        ...
        """
        idx = self.n_aper - 1  # apcorr for the largest EE (largest radius)
        flux_err = self.aperture_params["aperture_corrections"][idx] * getattr(self, self.aperture_flux_colnames[idx * 2 + 1])
        return flux_err
    ...
    def format_columns(catalog):
        ...
        for colname in catalog.colnames:
            ...
            if "abmag" in colname or "vegamag" in colname or colname in ("nn_dist", "sharpness", "roundness"):
                catalog[colname].info.format = ".6f"
            if colname in ("semimajor_sigma", "semiminor_sigma", "ellipticity", "orientation", "sky_orientation"):
                catalog[colname].info.format = ".6f"
