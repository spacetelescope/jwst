    ...
    def _get_reffile_paths(self, model):
        ...
        for reffile_type in self.reference_file_types:
            try:
                ...
                self.log.info(f"Using {reffile_type.upper()} reference file: " f"{filepath}")
    ...
    def process(self, input_model):
        with datamodels.open(input_model) as model:
            ...
            catobj = JWSTSourceCatalog(
                model, segment_img, convolved_data, self.kernel_fwhm, aperture_params, abvega_offset, ci_star_thresholds
            )
