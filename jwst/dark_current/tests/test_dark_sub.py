...
def _params():
    ...
    for readpatt, values in readpatterns.items():
        params.append(
            (readpatt, ngroups, values["nframes"], values["nskip"], nrows, ncols)
        )
...
def test_frame_averaging(
    setup_nrc_cube, readpatt, ngroups, nframes, groupgap, nrows, ncols
):
    ...
    data, dark_model = setup_nrc_cube(
        readpatt, ngroups, nframes, groupgap, nrows, ncols
    )
    ...
    for newgp, gstart, gend in zip(range(ngroups), gstrt_ind, gend_ind):
        ...
        manual_errs[newgp] = np.sqrt(np.sum(dark.err[gstart:gend, 10, 10] ** 2)) / (
            gend - gstart
        )
