...
class ResidualFringeStep(Step):
    ...
    def process(self, input):
        ...
        self.log.info("Using FRINGEFREQ reference file:{}".format(self.residual_fringe_filename))

        # set up regions reference file
        self.regions_filename = self.get_reference_file(input, "regions")
        self.log.info("Using MRS regions reference file: {}".format(self.regions_filename))
        ...
        rfc = residual_fringe.ResidualFringeCorrection(
            input, self.residual_fringe_filename, self.regions_filename, self.ignore_regions, **pars
        )
