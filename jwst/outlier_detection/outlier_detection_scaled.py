...
class OutlierDetectionScaled(OutlierDetection):
    ...
    def do_detection(self):
        ...
        catalog = tso_aperture_photometry(
            self.inputs, xcenter, ycenter, radius, radius_inner, radius_outer
        )
        ...
        for image in self.input_models:
            image.wht = resample_utils.build_driz_weight(
                image, weight_type="ivm", good_bits=pars["good_bits"]
            )
        ...
        aper2 = CircularAnnulus(
            (xcenter, ycenter), r_in=radius_inner, r_out=radius_outer
        )

        tbl1 = aperture_photometry(
            median_model.data, apertures, error=median_model.data * 0.0 + 1.0
        )
        tbl2 = aperture_photometry(
            median_model.data, aper2, error=median_model.data * 0.0 + 1.0
        )
