...
def test_shift_subtract():
    ...
    truth = np.array([ 0.65830479,  1.38243832,  0.46556956, 1.24234379,
                       0.17542511, -4.85323936, -0., -0.,
                      -3.06869101, -3.70808286, -0.03657461, 2.61462573,
                       2.73619353,  2.81983023,  2.56214928])
...
def test_align_array():
    ...
    targ[1,0,1] += 0.7; targ[1,2,1] += 0.2; targ[1,2,3] += 0.8; targ[1,2,2] -= 1.8
    targ[2,1,1] += 0.9; targ[2,2,2] -= 0.5; targ[2,1,0] += 0.8; targ[2,1,2] += 0.8

    ref = temp.copy()
    ref[0,0] += 0.5 ; ref[1,2] -=0.4 ; ref[2,2] -=1.4 ; ref[0,2] +=1.3
    ref[1,4] -= .6

    mask = temp * 0 + 1; mask[1,1]= 0 ; mask[1,2]= 0
...
def test_KLT():
    ...
    refs[1,0,1] += 0.7; refs[1,2,1] += 0.2; refs[1,2,3] += 0.8; refs[1,2,2] -= 1.8
    refs[2,1,1] += 0.9; refs[2,2,2] -= 0.5; refs[2,1,0] += 0.8; refs[2,1,2] += 0.8
    ...
    truth_klvect = np.array([
       [0.42288619,  0.34884115,  0.30246786,  0.25254087,  0.18204953,  0.10593695,
        0.03757055,  0.01225229, -0.05878713, -0.1189963,  -0.17920546, -0.25776279,
       -0.2541062,  -0.37564538, -0.42004212],
       [-0.16042986, 0.07486249, -0.12636643,  0.11188599, -0.09230299,  0.04286651,
        -0.05806697, 0.65210377, -0.02417611, -0.00714439,  0.00988733, -0.22042976,
        -0.59112697, 0.3104232,   0.0780142 ],
       [-0.08789825,-0.3672306,  -0.08482407, -0.02768044, -0.08174988,  0.33098098,
         0.35055338, 0.47863215, -0.07560151, -0.07406442, -0.07252733, -0.22908779,
         0.39578645,-0.3889097,  -0.06637896]]
      )
