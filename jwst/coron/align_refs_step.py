...
class AlignRefsStep(Step):
    ...
    def process(self, target, psf):
        ...
        psf_model = self.prepare_output(psf)
        ...
        if self.mask_name == "N/A":
            ...
            psf_model.meta.cal_step.align_psfs = "SKIPPED"
        ...
        # Result is a new model: close the input if it was opened here
        if target_model is not target:
            target_model.close()
        if psf_model is not psf:
            psf_model.close()
