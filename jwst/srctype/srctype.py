    ...
    if exptype in [
        ...
        try:
            ...
            if user_type is None:
                log.warning(
                    "SRCTYAPT keyword not found in input; using SRCTYPE instead"
                )
        ...
        if source_type is not None:
        ...
        elif (patttype is not None) and (
            ("NOD" in patttype) or ("POINT-SOURCE" in patttype)
        ):
            ...
            log.info(
                f"Input source type is unknown; setting default SRCTYPE = {src_type}"
            )
