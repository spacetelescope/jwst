...
def test_invalid_user_spec():
    """ Try to overwrite source_type with invalid user-specified value
    """
    input = datamodels.ImageModel((10, 10))
    ...
    # User sets to invalid value
    output = srctype.set_source_type(input, "GARBAGE")

    # Result should be match "UNKNOWN"
    assert output.meta.target.source_type == 'UNKNOWN'
