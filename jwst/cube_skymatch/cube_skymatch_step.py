    ...
    def process(self, input1, input2):
        ...
        for cm in cube_models:
            ...
            if dqbits is not None:
                dq = bitfield_to_boolean_mask(cm.dq, self._dqbits, good_mask_value=0, dtype=bool)
                weights[dq] = 0.0
        ...
        if nsubspace > 1:
            self.log.warning("Not all cubes have been sky matched as " "some of them do not overlap.")
        ...
        for c in skymatch_cubes:
            ...
            model2d, channel = _find_associated_2d_image(c.meta["original_cube_model"], models2d)
