...
class SkyCube:
    ...
    * other useful properties.
    ...
    def __init__(
        self, data, wcs=None, wcsinfo=None, weights=None, cube_weight=1.0, bkg_deg=0, bkg_center=None, id=None, meta=None
    ):
        ...
        wcsinfo_is_None = wcsinfo is None or wcsinfo.crval1 is None or wcsinfo.crval2 is None or wcsinfo.crval3 is None

        if wcsinfo_is_None and wcs is not None:
            raise ValueError("'wcsinfo' cannot be None or have its 'crvaln' " "set to None when 'wcs' is not None.")
        ...
        if hasattr(bkg_deg, "__iter__"):
            if len(bkg_deg) != 3:
                raise ValueError("When 'bkg_deg' is an iterable it must have " "three elements.")
    ...
    def weights(self, weights):
        if weights is None:
            ...
            if weights.shape != self.data.shape:
                raise ValueError("Weight map must have the same shape as " "the image cube")
    ...
    def _calc_meshgrid(self):
        ...
        v = np.polynomial.polynomial.polyvander3d(self._mgx, self._mgy, self._mgz, self._bkg_degree)
    ...
    def calc_bkg_cube(self, mx=None, my=None, mz=None):
        ...
        if mx is None and my is None and mz is None:
            ...
            raise ValueError("All mesh grids must simultaneously be either " "None or not None.")
