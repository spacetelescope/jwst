...
class PixelReplacement:
    ...
    def __init__(self, input_model, **pars):
        ...
        try:
            ...
            log.critical(f"Algorithm name {self.pars['algorithm']} provided does " "not match an implemented algorithm!")
            raise Exception
            ...
            if self.input.meta.exposure.type == "MIR_MRS":
                if self.pars["algorithm"] == "mingrad":
                    ...
                    for i, beta in enumerate(unique_beta):
                        ...
                        self.output.var_poisson = np.where(trace_mask, trace_model.var_poisson, self.output.var_poisson)
                        self.output.var_rnoise = np.where(trace_mask, trace_model.var_rnoise, self.output.var_rnoise)
                        self.output.var_flat = np.where(trace_mask, trace_model.var_flat, self.output.var_flat)
    ...
    def fit_profile(self, model):
        ...
        if np.any(0 in np.shape(good_pixels)):
            log.warning("No good pixels in at least one dimension of " "data array - skipping pixel replacement.")
            return model
        ...
        for i, ind in enumerate(profiles_to_replace):
            ...
            cleaned_current = np.where(model.dq[current_condition] & self.DO_NOT_USE, np.nan, current_profile)[
                range(*profile_cut)
            ]
            ...
            # TODO: check on signs here - absolute max sometimes picks up
            #  large negative outliers
            norm_scale = minimize(self.profile_mse, x0=np.abs(np.nanmax(norm_current)), args=(min_median, norm_current)).x
            ...
            replaced_current = np.where(replace_condition, median_profile * norm_scale * scale, cleaned_current)

            # Change the dq bits where old flag was DO_NOT_USE and new value is not nan
            replaced_dq = np.where(
                replace_condition & ~(np.isnan(replaced_current)), current_dq ^ self.DO_NOT_USE ^ self.FLUX_ESTIMATED, current_dq
            )
            ...
            replaced_err = np.where(replace_condition, norm_errors["err"] * norm_scale * scale, current_err)
            ...
            replaced_var = np.where(replace_condition, (norm_errors["var_poisson"] * norm_scale * scale) ** 2, current_var)
            ...
            replaced_var = np.where(replace_condition, (norm_errors["var_rnoise"] * norm_scale * scale) ** 2, current_var)
            ...
            replaced_var = np.where(replace_condition, (norm_errors["var_flat"] * norm_scale * scale) ** 2, current_var)
    ...
    def mingrad(self, model):
        ...
        indx = np.where((~np.isfinite(indata)) & (basey > pad) & (basey < ysize - pad) & (basex > pad) & (basex < xsize - pad))
        # X and Y indices
        ...
        for ii in range(0, len(xindx)):
            ...
            interp_var_p = np.array([(left_var_p + right_var_p) / 2.0, (top_var_p + bottom_var_p) / 2.0])
            interp_var_r = np.array([(left_var_r + right_var_r) / 2.0, (top_var_r + bottom_var_r) / 2.0])
            interp_var_f = np.array([(left_var_f + right_var_f) / 2.0, (top_var_f + bottom_var_f) / 2.0])
            ...
            try:
                ...
                # the DO_NOT_USE flag
                if (indq[yindx[ii], xindx[ii]] & self.DO_NOT_USE) and not (indq[yindx[ii], xindx[ii]] & self.NON_SCIENCE):
