...
@pytest.mark.parametrize("input_model_function", [nirspec_tso, nirspec_fs_slitmodel, miri_lrs, miri_mrs])
...
def test_pixel_replace_no_container(input_model_function, algorithm):
    ...
    assert result.dq[bad_idx] == (input_model.dq[bad_idx] - flags["DO_NOT_USE"] + flags["FLUX_ESTIMATED"])
...
def test_pixel_replace_multislit(input_model_function, algorithm):
    ...
    # The DQ plane for the bad pixel is updated to remove do-not-use
    # and add flux-estimated. The non-science edges are unchanged.
    assert result.slits[0].dq[bad_idx] == (input_model.slits[0].dq[bad_idx] - flags["DO_NOT_USE"] + flags["FLUX_ESTIMATED"])
