...
def test_create_shutter_elements():
    ...
    assert np.allclose(shutter_elements["closed_closed"], 0.01 * np.ones((501, 101), dtype=np.float64), atol=1.0e-10)
...
def test_create_first():
    ...
    assert np.allclose(shadow, shadow1x1[:501, :], atol=1.0e-10)
...
def test_create_open_open():
    ...
    assert np.allclose(shadow, shadow1x3[:501, :], atol=1.0e-10)
...
def test_create_open_closed():
    ...
    assert np.allclose(shadow, shadow1x1[500:, :], atol=1.0e-10)
...
def test_create_closed_closed():
    ...
    assert np.allclose(shadow, 0.01 * np.ones((501, 101), dtype=np.float64), atol=1.0e-10)
...
def test_create_shadow():
    ...
    # closed_closed
    assert np.allclose(shadow[1501:2000, :], 0.01, atol=1.0e-10)
    # closed_open
...
def test_add_next_shutter():
    ...
    assert np.allclose(shadow[first_row, :], (dummy_value + shadow_element[0, :]) / 2.0, atol=1.0e-9)
