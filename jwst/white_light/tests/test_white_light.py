...
def make_datamodel():
    ...
    spec_dtype = (
        datamodels.SpecModel().spec_table.dtype
    ...
    integrations = [
        (1, 58627.53891071, 58627.53896565, 58627.5390206, 0.0, 0.0, 0.0),
        (2, 58627.5390206, 58627.53907555, 58627.5391305, 0.0, 0.0, 0.0),
        (3, 58627.5391305, 58627.53918544, 58627.53924039, 0.0, 0.0, 0.0),
    ]
...
def test_white_light_with_expstart(make_datamodel):
    ...
    dt = data.meta.exposure.group_time * (data.meta.exposure.ngroups + 1)

    # We know there is only one table, so set we are hardcoding.
    ntables_current = 1
    dt_arr[0:1] = np.arange(1, 1 + ntables_current) * dt - (dt / 2.0)
    int_dt = TimeDelta(dt_arr, format="sec")

    int_times = Time(data.meta.exposure.start_time, format="mjd") + int_dt
    # Sum the fluxes
    fluxsums = data.spec[0].spec_table["FLUX"].sum()
