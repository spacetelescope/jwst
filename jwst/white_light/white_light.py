...
def white_light(input, min_wave=None, max_wave=None):
    ...
    log.debug("norders = %d, sporders = %s, ntables_order = %s", norders, str(sporders), str(ntables_order))
    ...
    for i in range(ntables):
        wave_mask = np.where(
            (input.spec[i].spec_table["WAVELENGTH"] >= low_cutoff) & (input.spec[i].spec_table["WAVELENGTH"] <= high_cutoff)
        )[0]
