...
def mock_miri_model():
    ...
    # Mock only the transforms needed for these tests
    def mock_get_transform(frame1, frame2):
        if frame2 == "coordinates":
            # for detector -> coordinates: just return input
            return Identity(2)
        else:
            # for coordinates/detector -> alpha beta, just return input + one extra output
            return Mapping((0, 1, 1))

    input_model.meta.wcs.get_transform = mock_get_transform
    return input_model
