...
It has been replaced by a different technique. The code was left  in the jwst
pipeline repo for in case future enhancements are found to make this a useful
tool for flagging outliers.


Main function for Python: blot_wrapper

Python signature:
            result = blot_wrapper(roi_det, blot_xsize, blot_ysize, xstart,
xsize2, xcenter, ycenter, x_cube, y_cube, flux_cube)



Each file is mapped to the sky to create a single type
IFU cube. Using the set of single IFUs a median IFU is determined. This median
image is then mapped (blotted) back to the detector plane. This routine
determines the overlap between the median blotted back image and the detector
plane.
...
xsize2 : int
   If MIRI xsize2 = x size of the detector side the medina image is being
blotted to If NIRSpec, xsize2 = xsize xcenter : double array x center pixel
values of the detector values. Size xsize2. ycenter : double array y center
pixel values of the detector values. Size ysize x_cube : double array x
coordinate of median cube mapped back to detector y_cube : double array y
coordinate of median cube mapped back to detector flux_cube : double array flux
of median cube
...
int
blot_overlap(
    double roi, int xsize_det, int ysize_det, int xstart, int xsize2, int ncube, double *xcenter,
...
blot_wrapper(PyObject *module, PyObject *args)
{
    ...
    int free_xcenter = 0, free_ycenter = 0, free_xcube = 0, free_ycube = 0, free_flux = 0,
        status = 0;
    ...
    if (!PyArg_ParseTuple(
            args, "diiiiOOOOO:blot_wrapper", &roi, &xsize_det, &ysize_det, &xstart, &xsize2,
            &xcentero, &ycentero, &x_cubeo, &y_cubeo, &flux_cubeo)) {
        return NULL;
    }
    ...
    status = blot_overlap(
        ...
        (double *)PyArray_DATA(y_cube), (double *)PyArray_DATA(flux_cube), &blot_flux,
        &blot_weight);
    ...
    if (status) {
        ...
        blot_weight_arr =
            (PyArrayObject *)PyArray_SimpleNewFromData(1, &npt, NPY_DOUBLE, blot_weight);
        if (!blot_weight_arr) {
            goto fail;
        }
        blot_weight = NULL;
...
static PyMethodDef blot_methods[] = {
    {"blot_wrapper", blot_wrapper, METH_VARARGS, "blot_wrapper()"},
