...
int mem_alloc(int nelem, dbl_type **fluxv, dbl_type **weightv,  int **ifluxv, dbl_type **varv) {
    int  *i;
    dbl_type *f, *w, *v;
    ...
    // weight:
    w = (dbl_type*) realloc(*weightv, nelem * sizeof(dbl_type));
    if (w) {
        *weightv = w;
    } else {
        PyErr_SetString(PyExc_MemoryError, msg);
    ...
    // variance:
    v = (dbl_type*) realloc(*varv, nelem * sizeof(dbl_type));
    if (v) {
        *varv = v;
    } else {
        PyErr_SetString(PyExc_MemoryError, msg);
...
static PyObject * point_emsm(PyObject *module, PyObject *args) {
  ...
  PyObject *rois_pixelo, *roiw_pixelo, *scalerad_pixelo;
  
  double cdelt1, cdelt2, zcdelt3;
  ...
  if (!PyArg_ParseTuple(args, "OOOOOOOOOOOiiiiddd:point_emsm",
    ...
    status = match_point_emsm((dbl_type *) PyArray_DATA(xc),
	...
	spaxel_var_arr = (PyArrayObject*) PyArray_SimpleNewFromData(
        ...
        spaxel_iflux_arr = (PyArrayObject*) PyArray_SimpleNewFromData(
            1, &npy_ncube, NPY_INT, spaxel_iflux
        );
        if (!spaxel_iflux_arr) goto fail;
        spaxel_iflux = NULL;
