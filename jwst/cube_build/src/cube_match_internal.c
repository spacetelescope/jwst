...
int match_detector_cube(int instrument, int naxis1, int naxis2, int nz, int npt, int ncube, int na,
                        double crval_along, double cdelt_along, double crval3, double cdelt3, double *a1,
                        double *a2, double *a3, double *a4, double *lam1, double *lam2, double *lam3,
                        double *lam4, double *acoord, double *zcoord, int ss, double *pixel_flux,
                        double *pixel_err, double **spaxel_flux, double **spaxel_weight, double **spaxel_var,
                        double **spaxel_iflux)
{
    ...
    */
    ...
    double along_corner[4], wave_corner[4], along_min, wave_min, along_max, wave_max, Area, MinW, MaxW,
        zcenter, acenter, area_overlap, AreaRatio, err;
    ...
    for (ipixel = 0; ipixel < npt; ipixel++)
    {
        ...
        for (zz = iz1; zz < iz2 + 1; zz++)
        {
            ...
            for (aa = ia1; aa < ia2 + 1; aa++)
            {
                ...
                acenter = acoord[aa];
                area_overlap =
                    sh_find_overlap(acenter, zcenter, cdelt_along, cdelt3, along_corner, wave_corner);
...
static PyObject *cube_wrapper_internal(PyObject *module, PyObject *args)
{
    ...
    int free_a1 = 0, free_a2 = 0, free_a3 = 0, free_a4 = 0, free_lam1 = 0, free_lam2 = 0, free_lam3 = 0,
        free_lam4 = 0;
    ...
    if (ncube == 0)
    {
        ...
        result =
            Py_BuildValue("(NNNN)", spaxel_flux_arr, spaxel_weight_arr, spaxel_var_arr, spaxel_iflux_arr);
    ...
    status = match_detector_cube(
        ...
        (double *)PyArray_DATA(zcoord), ss, (double *)PyArray_DATA(flux), (double *)PyArray_DATA(err),
