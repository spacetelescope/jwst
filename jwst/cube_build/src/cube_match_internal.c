/*
This module creates the IFU cube in the "slicer plane". This method of creating
cubes is for an engineering mode. The detector pixels are mapped to the along
slice coordinate and wavelength. The slice number represents the across slice
coordinate. The pixels from each slice are assumed to not overlap on the output
plane with pixels from other slices. This breaks the 3-D representation of the
pixel's coordinates in the output plane to 2D. Standard drizzling type routines
are used to calculate the percentage of each pixel's overlap with the output
spaxels. The percentage of the pixel area  overlapped onto an output spaxel is
used to weight the pixel flux for determining the spaxel flux.
...
int
match_detector_cube(
    int instrument, int naxis1, int naxis2, int nz, int npt, int ncube, int na, double crval_along,
    double cdelt_along, double crval3, double cdelt3, double *a1, double *a2, double *a3,
    double *a4, double *lam1, double *lam2, double *lam3, double *lam4, double *acoord,
    double *zcoord, int ss, double *pixel_flux, double *pixel_err, double **spaxel_flux,
    double **spaxel_weight, double **spaxel_var, double **spaxel_iflux)
{
    /* Find the overlap between the detector pixels and spaxels of the IFU cube
       when the IFU cube is of internal cal type.

       The overlap is based on a 2-D drizzling type of algorithm  which finds
       the contribution of each detector flux to the IFU cube spaxel

       A description of each variable is given at the top of this c routine.
    */

    double *fluxv = NULL, *weightv = NULL, *varv = NULL,
           *ifluxv = NULL;  // vectors for spaxel
    double along_corner[4], wave_corner[4], along_min, wave_min, along_max, wave_max, Area, MinW,
        MaxW, zcenter, acenter, area_overlap, AreaRatio, err;
    ...
    for (ipixel = 0; ipixel < npt; ipixel++) {
        ...
        wave_max = wave_min;
        for (j = 1; j < 4; j++) {
            if (along_corner[j] < along_min) {
                along_min = along_corner[j];
            }
            if (along_corner[j] > along_max) {
                along_max = along_corner[j];
            }
            if (wave_corner[j] < wave_min) {
                wave_min = wave_corner[j];
            }
            if (wave_corner[j] > wave_max) {
                wave_max = wave_corner[j];
            }
        }
        ...
        // find the min and max values in the cube appropriate xcoord,ycoord or
        // zcoord
        ...
        for (zz = iz1; zz < iz2 + 1; zz++) {
            ...
            for (aa = ia1; aa < ia2 + 1; aa++) {
                ...
                area_overlap = sh_find_overlap(
                    acenter, zcenter, cdelt_along, cdelt3, along_corner, wave_corner);
...
static PyObject *
cube_wrapper_internal(PyObject *module, PyObject *args)
{
    PyObject *result = NULL, *a1o, *a2o, *a3o, *a4o, *lam1o, *lam2o, *lam3o, *lam4o, *fluxo, *erro,
             *acoordo,
    ...
    int free_a1 = 0, free_a2 = 0, free_a3 = 0, free_a4 = 0, free_lam1 = 0, free_lam2 = 0,
        free_lam3 = 0, free_lam4 = 0;
    ...
    PyArrayObject *lam3 = NULL, *lam4 = NULL, *flux = NULL, *err = NULL, *acoord = NULL,
                  *zcoord = NULL;
    ...
    if (!PyArg_ParseTuple(
            args, "iiiddddOOOOOOOOOOiOO:cube_wrapper_internal", &instrument, &naxis1, &naxis2,
            &crval_along, &cdelt_along, &crval3, &cdelt3, &a1o, &a2o, &a3o, &a4o, &lam1o, &lam2o,
            &lam3o, &lam4o, &acoordo, &zcoordo, &ss, &fluxo, &erro)) {  // codespell:ignore erro
        return NULL;
    }
    ...
    if ((cdelt3 < 0) || (cdelt_along < 0)) {
        PyErr_SetString(
            PyExc_ValueError, "'cdelt3' and 'cdelt_along' must be a strictly positive number.");
        return NULL;
    }

    // ensure we are working with numpy arrays and avoid creating new ones
    // if possible:
    if ((!(a1 = ensure_array(a1o, &free_a1))) || (!(a2 = ensure_array(a2o, &free_a2))) ||
        (!(a3 = ensure_array(a3o, &free_a3))) || (!(a4 = ensure_array(a4o, &free_a4))) ||
    ...
    if (ncube == 0) {
        ...
        result = Py_BuildValue(
            "(NNNN)", spaxel_flux_arr, spaxel_weight_arr, spaxel_var_arr, spaxel_iflux_arr);
    ...
    //______________________________________________________________________
    status = match_detector_cube(
        instrument, naxis1, naxis2, nz, npt, ncube, na, crval_along, cdelt_along, crval3, cdelt3,
        (double *)PyArray_DATA(a1), (double *)PyArray_DATA(a2), (double *)PyArray_DATA(a3),
        (double *)PyArray_DATA(a4), (double *)PyArray_DATA(lam1), (double *)PyArray_DATA(lam2),
        (double *)PyArray_DATA(lam3), (double *)PyArray_DATA(lam4), (double *)PyArray_DATA(acoord),
        (double *)PyArray_DATA(zcoord), ss, (double *)PyArray_DATA(flux),
        (double *)PyArray_DATA(err), &spaxel_flux, &spaxel_weight, &spaxel_var, &spaxel_iflux);
...
static PyMethodDef cube_methods[] = {
    {"cube_wrapper_internal", cube_wrapper_internal, METH_VARARGS,
