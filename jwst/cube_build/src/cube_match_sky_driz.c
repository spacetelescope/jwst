...
extern double sh_find_overlap(
    double xcenter, double ycenter, double xlength, double ylength, double xPixelCorner[], double yPixelCorner[]);
...
cube_wrapper_driz(PyObject *module, PyObject *args)
{
    ...
    int free_xi1 = 0, free_eta1 = 0, free_xi2 = 0, free_eta2 = 0, free_xi3 = 0, free_eta3 = 0, free_xi4 = 0,
        free_eta4 = 0;
    int free_dwave = 0;
    ...
    if (!PyArg_ParseTuple(
            args, "iiiiiiOOOOOOOOOOOOOOOOOOOdddiOOl:cube_wrapper_driz", &instrument, &flag_dq_plane, &start_region,
            &end_region, &overlap_partial, &overlap_full, &xco, &yco, &zco, &coord1o, &coord2o, &waveo, &fluxo, &erro,
            &slicenoo,  // codespell:ignore erro
            &xi1o, &eta1o, &xi2o, &eta2o, &xi3o, &eta3o, &xi4o, &eta4o, &dwaveo, &cdelt3o, &cdelt1, &cdelt2,
            &cdelt3_mean, &linear, &x_deto, &y_deto, &debug_cube_index)) {
        return NULL;
    }
    ...
    if ((!(xc = ensure_array(xco, &free_xc))) || (!(yc = ensure_array(yco, &free_yc))) ||
        (!(zc = ensure_array(zco, &free_zc))) || (!(coord1 = ensure_array(coord1o, &free_coord1))) ||
        (!(coord2 = ensure_array(coord2o, &free_coord2))) || (!(wave = ensure_array(waveo, &free_wave))) ||
        (!(flux = ensure_array(fluxo, &free_flux))) || (!(x_det = ensure_array(x_deto, &free_x_det))) ||
        (!(y_det = ensure_array(y_deto, &free_y_det))) ||
        (!(err = ensure_array(erro, &free_err))) ||  // codespell:ignore erro
        (!(sliceno = ensure_array(slicenoo, &free_sliceno))) || (!(xi1 = ensure_array(xi1o, &free_xi1))) ||
        (!(eta1 = ensure_array(eta1o, &free_eta1))) || (!(xi2 = ensure_array(xi2o, &free_xi2))) ||
    ...
    if (ncube == 0) {
        ...
        result = Py_BuildValue(
            "(NNNNN)", spaxel_flux_arr, spaxel_weight_arr, spaxel_var_arr, spaxel_iflux_arr, spaxel_dq_arr);
    ...
    if (flag_dq_plane) {
        if (instrument == 0) {
            status1 = dq_miri(
                start_region, end_region, overlap_partial, overlap_full, nxx, nyy, nwave, cdelt1, cdelt2, cdelt3_mean,
            ...
            status1 = dq_nirspec(
                overlap_partial, nxx, nyy, nwave, cdelt1, cdelt2, cdelt3_mean, (double *)PyArray_DATA(xc),
                (double *)PyArray_DATA(yc), (double *)PyArray_DATA(zc), (double *)PyArray_DATA(coord1),
                (double *)PyArray_DATA(coord2), (double *)PyArray_DATA(wave), (double *)PyArray_DATA(sliceno), ncube,
                npt, &spaxel_dq);
        }
    ...
    status = match_driz(
        ...
        (double *)PyArray_DATA(cdelt3), (double *)PyArray_DATA(x_det), (double *)PyArray_DATA(y_det), cdelt1, cdelt2,
        nxx, nyy, nwave, ncube, npt, linear, debug_cube_index, &spaxel_flux, &spaxel_weight, &spaxel_var,
        &spaxel_iflux);
