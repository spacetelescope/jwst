...
addpoint(double x, double y, double xnew[], double ynew[], int *nVertices2)
{
    /*
      ...
      x : double
          X coordinate to add to clipped polygon that falls inside the cube
      spaxel y : double Y coordinate to add to clipped polygon that falls inside
      the cube spaxel xnew : double array Array containing the x vertices of
      clipped polygon ynew : double array Array containing the y vertices of
      clipped polygon
    */
    xnew[*nVertices2] = x;
    ynew[*nVertices2] = y;
...
int
insideWindow(int edge, double x, double y, double left, double right, double top, double bottom)
{
    /*
      Support function for sh_find_overlap. Determine where a point is in
      relationship to the clipped polygon.

      edge : int
         Integer value which defines which edge of the polygon we are working
      with. 0 = left, 1 = right, 2 = top, 3 = bottom. x : double X coordinate
      that is being tested if it falls inside the polygon. y : double Y
      coordinate that is being tested if it falls inside the polygon. left :
      double Value defining the left side of the clipped polygon right : double
...
int
calcCondition(
    int edge, double x1, double y1, double x2, double y2, double left, double right, double top,
    double bottom)
{
    /* A support function for sh_find_overlap. Is a point in the polygon ?
       ...
       x2,y2 from pixel j+1 is inside the clipped polygon. Based on this
      information the vertices of the clipped polygon are updated.

       edge : int
         Integer value which defines which edge of the polygon we are working
      with. 0 = left, 1 = right, 2 = top, 3 = bottom. x1 : double X value of a
      pixel[j] y1 : double Y value of a pixel[j] x2 : double X value of a
      pixel[j+1] y2 : double Y value of a pixel[j+1] left : double Value
      defining the left side of the clipped polygon right : double Value
      defining the right side of the clipped polygon top : double Value defining
      the top edge of the clipped polygon bottom : double Value defining the
      bottom edge of the clipped polygon
     */
...
void
solveIntersection(
    int edge, double x1, double y1, double x2, double y2, double *x, double *y, double left,
    double right, double top, double bottom)
{
    /*
      A support function for sh_find_overlap. Find the intersection of a polygon
      and a cube spaxel. The cube spaxel is defined on an evenly spaced regular
      grid.

       edge : int
         Integer value that defines which edge of the polygon we are working
      with. 0 = left, 1 = right, 2 = top, 3 = bottom. x1 : double X value of a
      pixel[j] y1 : double Y value of a pixel[j] x2 : double X value of a
      pixel[j+1] y2 : double Y value of a pixel[j+1] x : double The return x
      value of the x vertice of the clipped polygon y : double The return y
      value of the x vertice of the clipped polygon left : double Value defining
      the left side of the clipped polygon right : double Value defining the
      right side of the clipped polygon top : double Value defining the top edge
      of the clipped polygon bottom : double Value defining the bottom edge of
      the clipped polygon
...
{
    /* Find the area of an quadrilateral between clipped polygon and cube
      spaxel.
...
find_area_poly(int nVertices, double xPixel[], double yPixel[])
{
    ...
    for (i = 0; i < nVertices - 1; i++) {
        area = (xPixel[i] - xmin) * (yPixel[i + 1] - ymin) -
               (xPixel[i + 1] - xmin) * (yPixel[i] - ymin);
    ...
    double xPixelCorner[], double yPixelCorner[])
{
    ...
    xPixelCorner : ndarray
        X corners of the polygon that is the formed by overlap between detector
    pixel and cube spaxel yPixelCorner : ndarray Y corners of the polygon that
    is the formed by overlap between detector pixel and cube spaxel
    ...
    */
    ...
    // xPixel,yPixel is become the clipped polygon vertices inside the cube
    // pixel
    for (i = 0; i < 4; i++) {
