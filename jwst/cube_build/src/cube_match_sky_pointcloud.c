...
The IFU cube is represented by a 3-D regular grid. This module finds the point cloud members
contained in a region centered on the center of the cube spaxel. The size of the spaxel in spatial
coordinates is cdetl1 and cdelt2, while the wavelength size is zcdelt3.
This module uses the modified Shepard weighting method (emsm if weight_type =0 or msm if weight_type
=1) to determine how to  weight each point cloud member in the spaxel.

Main function for Python: cube_wrapper

Python signature: result = cube_wrapper(instrument, flag_dq_plane, weight_type, start_region,
end_region, overlap_partial, overlap_full, xcoord, ycoord, zcoord, coord1, coord2, wave, flux, err,
...
spaxel_iflux, spaxel_dq) example output
    ...
    double **spaxel_flux, double **spaxel_weight, double **spaxel_var, double **spaxel_iflux)
{
    ...
    for (k = 0; k < npt; k++) {
        ...
        if (done_search_x == 1 && done_search_y == 1 && done_search_w == 1) {
            // The search above for x,y  was a crude search - now narrow the search using the
            // distance between the spaxel center and point cloud
...
cube_wrapper(PyObject *module, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &zco, &coord1o, &coord2o, &waveo, &fluxo, &erro, &slicenoo, // codespell:ignore erro
    ...
    if (weight_type == 0) {
        status = match_point_emsm(
            ...
            (double *) PyArray_DATA(wave), (double *) PyArray_DATA(flux),
            (double *) PyArray_DATA(err), (double *) PyArray_DATA(rois_pixel),
        ...
        status = match_point_msm(
            ...
            (double *) PyArray_DATA(softrad_pixel), (double *) PyArray_DATA(zcdelt3), nxx, nyy,
