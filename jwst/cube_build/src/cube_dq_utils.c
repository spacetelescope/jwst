...
extern double sh_find_overlap(const double xcenter, const double ycenter, const double xlength,
                              const double ylength, double xPixelCorner[], double yPixelCorner[]);
...
int corner_wave_plane_miri(int w, int start_region, int end_region, double roiw_ave, double *zc,
                           double *coord1, double *coord2, double *wave, double *sliceno, long ncube,
                           long npt, double *corner1, double *corner2, double *corner3, double *corner4)
{
    /*
    ...
    // Make sure the 2 extreme slices are found on the FOV. Not finding both can occur for edge wavelength
    // planes or empty wavelength planes between channels
    ...
    {
        // Find the length in the start and end regions for c1 and c2. This will help define which coordinates
        // to use to set corners. Because we do not know the orientation on the sky pick the  longest length
        // to set how to pick corners.
...
int overlap_fov_with_spaxels(int overlap_partial, int overlap_full, double cdelt1, double cdelt2, int naxis1,
                             int naxis2, double xcenters[], double ycenters[], double xi_corner[],
        ...
        if (x1 > ximin && x2 < ximax)
        {
            for (iy = 0; iy < naxis2; iy++)
            {
                ...
                if (y1 > etamin && y2 < etamax)
                {
                    ixy = iy * naxis1 + ix;
                    area_overlap =
                        sh_find_overlap(xcenters[ix], ycenters[iy], cdelt1, cdelt2, xi_corner, eta_corner);
...
long match_wave_plane_nirspec(double wave_plane, double roiw_ave, double coord1[], double coord2[],
                              double wave[], double sliceno[], long npt, double *c1_min, double *c2_min,
                              double *c1_max, double *c2_max, int *match_slice)
{
...
int overlap_slice_with_spaxels(int overlap_partial, double cdelt1, double cdelt2, int naxis1, int naxis2,
                               double xstart, double ystart, double xi_min, double eta_min, double xi_max,
                               double eta_max, int wave_slice_dq[])
{
...
int dq_miri(int start_region, int end_region, int overlap_partial, int overlap_full, int nx, int ny, int nz,
            ...
            double *coord2, double *wave, double *sliceno, long ncube, long npt, int **spaxel_dq)
{
    ...
    */
    ...
    // for each wavelength plane find the 2 extreme slices to set FOV. Use these two extreme slices to set up
    // the corner of the FOV for each wavelength
    ...
    for (w = 0; w < nz; w++)
    {
        ...
        status_wave = 0;
        status_wave = corner_wave_plane_miri(w, start_region, end_region, roiw_ave, zc, coord1, coord2, wave,
                                             sliceno, ncube, npt, corner1, corner2, corner3, corner4);
        if (status_wave == 0)
        { // found min and max slice on wavelength plane
            ...
            status = overlap_fov_with_spaxels(overlap_partial, overlap_full, cdelt1, cdelt2, nx, ny, xc, yc,
                                              xi_corner, eta_corner, wave_slice_dq);
        }
        istart = nxy * w;
        iend = istart + nxy;
...
int dq_nirspec(int overlap_partial, int nx, int ny, int nz, double cdelt1, double cdelt2, double roiw_ave,
               ...
               double *sliceno, long ncube, long npt, int **spaxel_dq)
{
    ...
    */
    ...
    for (w = 0; w < nz; w++)
    {
        ...
        imatch = match_wave_plane_nirspec(zc[w], roiw_ave, coord1, coord2, wave, sliceno, npt, c1_min, c2_min,
                                          c1_max, c2_max, match_slice);
        ...
        if (imatch > 0)
        { // some matches were found on the wavelength slice
            for (islice = 0; islice < 30; islice++)
            {
                ...
                if (match_slice[islice] == 1)
                {
                    ...
                    float xstart = xc[0];
                    float ystart = yc[0];
                    status = overlap_slice_with_spaxels(overlap_partial, cdelt1, cdelt2, nx, ny, xstart,
                                                        ystart, slice_c1_min, slice_c2_min, slice_c1_max,
                                                        slice_c2_max, wave_slice_dq);
