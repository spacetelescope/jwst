    ...
    */
    ...
    for (w = 0; w < nz; w++) {
        ...
        status_wave = corner_wave_plane_miri(
            w, start_region, end_region, roiw_ave, zc, coord1, coord2, wave, sliceno, ncube, npt, corner1, corner2,
            corner3, corner4);
        if (status_wave == 0) {  // found min and max slice on wavelength plane
            ...
            status = overlap_fov_with_spaxels(
                overlap_partial, overlap_full, cdelt1, cdelt2, nx, ny, xc, yc, xi_corner, eta_corner, wave_slice_dq);
        }
        istart = nxy * w;
        iend = istart + nxy;
    ...
    */
    ...
    for (w = 0; w < nz; w++) {
        ...
        imatch = match_wave_plane_nirspec(
            zc[w], roiw_ave, coord1, coord2, wave, sliceno, npt, c1_min, c2_min, c1_max, c2_max, match_slice);
            ...
            for (islice = 0; islice < 30; islice++) {
                ...
                if (match_slice[islice] == 1) {
                    ...
                    status = overlap_slice_with_spaxels(
                        overlap_partial, cdelt1, cdelt2, nx, ny, xstart, ystart, slice_c1_min, slice_c2_min,
                        slice_c1_max, slice_c2_max, wave_slice_dq);
