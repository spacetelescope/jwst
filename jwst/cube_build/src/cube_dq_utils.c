    ...
    double *corner2, double *corner3, double *corner4)
{
    /*
       ...
       Use the 2 extreme slices set by start_region and end_region to define the FOV of the
       wavelength plane. Using the min and max coordinates for the extent of the slice on the sky
       for these two slice,  set the corners of the FOV.
    ...
    // Make sure the 2 extreme slices are found on the FOV. Not finding both can occur for edge
    // wavelength planes or empty wavelength planes between channels

    if (ic1_start_min == -1 || ic1_start_max == -1 || ic1_end_min == -1 || ic1_end_max == -1) {
        status = 1;
        return status;
    } else {
        // Find the length in the start and end regions for c1 and c2. This will help define which
        // coordinates to use to set corners. Because we do not know the orientation on the sky pick
        // the  longest length to set how to pick corners.
