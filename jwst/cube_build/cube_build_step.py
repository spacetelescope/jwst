...
class CubeBuildStep(Step):
    ...
    def process(self, input):
        ...
        if self.wavemin is not None:
            self.log.info(
                f"Setting minimum wavelength of spectral cube to: {self.wavemin}"
            )
        if self.wavemax is not None:
            self.log.info(
                f"Setting maximum wavelength of spectral cube to: {self.wavemax}"
            )
        ...
        input_table = data_types.DataTypes(
            input, self.single, self.output_file, self.output_dir
        )
        ...
        cubeinfo = cube_build.CubeData(
            self.input_models, self.input_filenames, par_filename, **pars
        )
        # ________________________________________________________________________________
        # cubeinfo.setup:
        ...
        for i in range(num_cubes):
            ...
            if self.single:
                ...
                self.log.info(
                    "Number of Single IFUCube models returned %i ", len(cube_container)
                )
