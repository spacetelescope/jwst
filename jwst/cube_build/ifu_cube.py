...
class IFUCubeData:
        ...
        if self.coord_system == "internal_cal":
            if num_files > 1:
                raise IncorrectInput(
                    "Cubes built in internal_cal coordinate system" + " are built from a single file, not multiple exposures"
                )
        ...
        for ib in range(number_bands):
            ...
            for input in self.master_table.FileMap[self.instrument][this_par1][this_par2]:
                ...
                if self.interpolation in ["pointcloud", "drizzle"]:
                    pixelresult = self.map_detector_to_outputframe(this_par1, subtract_background, input_model)
    ...
    def determine_cube_parameters(self):
        ...
        # if all bands have the same spectral size then linear_wavelength
            ...
            if self.instrument == "MIRI":
                ...
                (table_wavelength, table_sroi, table_wroi, table_power, table_softrad, table_scalerad) = table
        ...
        if self.rois == 0.0:
            ...
            if self.output_type == "single" or self.num_files < 4:
                ...
                if self.weighting.lower() != "emsm":
                    ...
                    log.info("Increasing spatial region of interest " f"default value set for 4 dithers {self.rois}")
    ...
    def map_detector_to_outputframe(self, this_par1, subtract_background, input_model):
        ...
        if not_mapped_low > 0:
            log.info(
                "# of detector pixels not mapped to output plane: " f"{not_mapped_low} with wavelength below {min_wave_tolerance}"
            )
    ...
    def map_miri_pixel_to_sky(self, input_model, this_par1, subtract_background):
        ...
        if self.interpolation == "drizzle":
            ...
            ra1, dec1, _ = input_model.meta.wcs.transform(
                "alpha_beta", input_model.meta.wcs.output_frame, alpha1, beta - dbeta * pixfrac / 2.0, wave
            )
            ra2, dec2, _ = input_model.meta.wcs.transform(
                "alpha_beta", input_model.meta.wcs.output_frame, alpha1, beta + dbeta * pixfrac / 2.0, wave
            )
            ra3, dec3, _ = input_model.meta.wcs.transform(
                "alpha_beta", input_model.meta.wcs.output_frame, alpha2, beta + dbeta * pixfrac / 2.0, wave
            )
    ...
    def set_final_dq_flags(self):
        ...
        self.spaxel_dq[non_science] = np.bitwise_or(self.overlap_no_coverage, dqflags.pixel["DO_NOT_USE"])
        ...
        for i in range(len(index[0])):
            ...
            if yrem == 0 or yrem == (self.naxis2 - 1) or xrem == 0 or xrem == (self.naxis1 - 1):
                spaxel_dq_temp[index[0][i]] = np.bitwise_or(self.overlap_no_coverage, dqflags.pixel["DO_NOT_USE"])
                found = 1
            ...
            while (ij < 4) and (found == 0):
                if xcheck[ij] > 0 and xcheck[ij] < self.naxis1 and ycheck[ij] > 0 and ycheck[ij] < self.naxis2:
                    ...
                    # pixels all in FOV.
                    check = np.bitwise_and(self.spaxel_dq[index_check], self.overlap_no_coverage) == self.overlap_no_coverage
                    if check:
                        spaxel_dq_temp[index[0][i]] = np.bitwise_or(self.overlap_no_coverage, dqflags.pixel["DO_NOT_USE"])
                        found = 1
                ij = ij + 1
    ...
    def setup_final_ifucube_model(self, model_ref):
        ...
        if self.linear_wavelength:
            ...
            ifucube_model = datamodels.IFUCubeModel(data=flux, dq=dq, err=var, weightmap=wmap, wavetable=alldata)
        ...
        ifucube_model.meta.photometry.pixelarea_steradians = ifucube_model.meta.photometry.pixelarea_arcsecsq * 2.3504e-11
        if self.linear_wavelength:
        ...
        ifucube_model.meta.wcs.bounding_box = ((0, self.naxis1 - 1), (0, self.naxis2 - 1), (0, self.naxis3 - 1))
    ...
    def blend_output_metadata(self, IFUCube):
        ...
        blendmeta.blendmodels(IFUCube, inputs=self.input_models_this_cube, output=output_file)
