...
class CubeData:
    ...
    def setup(self):
        ...
        * cube parameter reference file
        ...
        return {"instrument": self.instrument, "instrument_info": self.instrument_info, "master_table": self.master_table}
    ...
    def determine_band_coverage(self, master_table):
        ...
        if self.instrument == "MIRI":
                ...
                for j in range(nsubchannels):
                    ...
                    if nfiles > 0:
                        ...
                        # both parameters set
                            ...
                            if valid_channel[i] in self.channel and valid_subchannel[j] in self.subchannel:
                                self.all_channel.append(valid_channel[i])
                                self.all_subchannel.append(valid_subchannel[j])
            ...
            if number_channels == 0:
                raise ErrorNoChannels("The cube  does not cover any channels, change channel parameter")
            if number_subchannels == 0:
                raise ErrorNoSubchannels("The cube does not cover any subchannels, change band parameter")
    ...
    def number_cubes(self):
        ...
        if self.instrument == "MIRI":
            ...
            if self.output_type == "user" or self.output_type == "single" or self.output_type == "multi":
                if self.output_type == "multi":
                    log.info("Output IFUcube are constructed from all the data ")
                if self.single:
                    log.info("Single = true, creating a set of single exposures mapped" + " to output IFUCube coordinate system")
