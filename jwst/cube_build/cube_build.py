...
class CubeData:
    ...
    def __init__(self, input_models, input_filenames, par_filename, **pars):
    ...
    def setup(self):
        ...
        * cube parameter reference file
        ...
        instrument = master_table.set_file_table(
            self.input_models, self.input_filenames
        )
    ...
    def determine_band_coverage(self, master_table):
        ...
        if self.instrument == "MIRI":
            ...
            for i in range(nchannels):
                for j in range(nsubchannels):
                    nfiles = len(
                        master_table.FileMap["MIRI"][valid_channel[i]][
                            valid_subchannel[j]
                        ]
                    )
                    if nfiles > 0:
            ...
            log.info(
                "The desired cubes cover the MIRI subchannels: %s", self.all_subchannel
            )
        ...
        if self.instrument == "NIRSPEC":
            ...
            for i in range(nbands):
                nfiles = len(
                    master_table.FileMap["NIRSPEC"][valid_gwa[i]][valid_fwa[i]]
                )
                if nfiles > 0:
    ...
    def number_cubes(self):
        ...
        if self.instrument == "MIRI":
            ...
            if self.output_type == "band":
                log.info(
                    "Output Cubes are single channel, single sub-channel IFU Cubes"
                )
