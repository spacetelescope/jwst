...
def correction_skip_groups(input_model, group_skip):
    ...
    output.groupdq[1:, 0:group_skip:, :] = np.bitwise_or(output.groupdq[1:, 0:group_skip, :, :], dqflags.group["DO_NOT_USE"])
...
def correction_decay_function(input_model, param):
    ...
    b1_even = param["even"]["ascale"] * (
        param["even"]["illum_zp"] + param["even"]["illum_slope"] * sci_ngroups + param["even"]["illum2"] * ngroups2
    )

    b1_odd = param["odd"]["ascale"] * (
        param["odd"]["illum_zp"] + param["odd"]["illum_slope"] * sci_ngroups + param["odd"]["illum2"] * ngroups2
    )
    ...
    for i in range(1, sci_nints):
        ...
        sat, dn_last23, dn_lastfit = get_DNaccumulated_last_int(input_model, i, sci_ngroups)
