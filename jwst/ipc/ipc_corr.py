...
NumRefPixels = namedtuple("NumRefPixels", ["bottom_rows", "top_rows", "left_columns", "right_columns"])
...
def do_correction(input_model, ipc_model):
    ...
    log.debug(
        "IPC corr using nints=%d, ngroups=%d, nframes=%d, groupgap=%d" % (sci_nints, sci_ngroups, sci_nframes, sci_groupgap)
    )
...
def ipc_correction(input_model, ipc_model):
    ...
    log.debug(
        ...
        " %d, %d, %d, %d" % (nref.bottom_rows, nref.top_rows, nref.left_columns, nref.right_columns)
    )
...
def get_num_ref_pixels(input_model):
    ...
    if input_model.meta.instrument.name == "MIRI":
        nref = NumRefPixels(
            bottom_rows=0, top_rows=0, left_columns=max(0, 4 - xstart), right_columns=max(0, xstart + xsize - 1028)
        )
    else:
...
def ipc_convolve(output_data, kernel, nref):
    ...
    temp[b_b : b_b + ny, l_b : l_b + nx] = output_data[yoff : yoff + ny, xoff : xoff + nx].copy()
    ...
    if len(kshape) == 2:
        ...
        for j in range(kshape[0]):
            ...
            for i in range(kshape[1]):
                ...
                istart = kshape[1] - i - 1
                output_data[yoff : yoff + ny, xoff : xoff + nx] += part[jstart : jstart + ny, istart : istart + nx]
        # The middle pixel of the IPC kernel is expected to be the largest,
        # so add that last.
        part = kernel[middle_j, middle_i] * temp
        output_data[yoff : yoff + ny, xoff : xoff + nx] += part[middle_j : middle_j + ny, middle_i : middle_i + nx]
        ...
        k_temp[:, :, b_b : b_b + ny, l_b : l_b + nx] = kernel[:, :, yoff : yoff + ny, xoff : xoff + nx]
        ...
        for j in range(kshape[0]):
            ...
            for i in range(kshape[1]):
                ...
                part = k_temp[j, i, b_b : b_b + ny, l_b : l_b + nx] * temp[jstart : jstart + ny, istart : istart + nx]
                output_data[yoff : yoff + ny, xoff : xoff + nx] += part
        # Add the product for the middle pixel last.
        part = (
            k_temp[middle_j, middle_i, b_b : b_b + ny, l_b : l_b + nx] * temp[middle_j : middle_j + ny, middle_i : middle_i + nx]
        )
        output_data[yoff : yoff + ny, xoff : xoff + nx] += part
