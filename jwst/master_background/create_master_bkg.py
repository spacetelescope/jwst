...
def create_background(wavelength, surf_bright):
    ...
    if len(wl_shape) > 1:
        bad = True
        log.error("The wavelength array has shape {}; expected " "a 1-D array".format(wl_shape))
    if len(sb_shape) > 1:
        bad = True
        log.error("The background surf_bright array has shape {}; expected " "a 1-D array".format(sb_shape))
    if bad:
        return None

    if wl_shape[0] != sb_shape[0]:
        log.error(
            "wavelength array has length {}, " "but background surf_bright array has length {}.".format(wl_shape[0], sb_shape[0])
        )
