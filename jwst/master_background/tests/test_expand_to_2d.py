...
def user_bkg_spec_a():
    ...
    surf_bright = np.linspace(
        13.0, 25.0, num=25, endpoint=True, retstep=False, dtype=np.float64
    )
...
def user_bkg_spec_c():
    ...
    data = np.array(
        list(zip(wavelength, flux, error, surf_bright, sb_error, dq, weight, n_input)),
