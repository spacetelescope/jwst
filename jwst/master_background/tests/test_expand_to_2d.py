...
def slit_data_a():
    ...
    temp_wl = np.linspace(1.3, 4.8, num=data_shape[1], endpoint=True, retstep=False, dtype=np.float32)
...
def user_bkg_spec_b():
    ...
    error = np.ones_like(wavelength)
    var_dummy = error.copy()
    surf_bright = np.linspace(13.0, 25.0, num=25, endpoint=True, retstep=False, dtype=np.float64)[::-1]
    sb_error = np.ones_like(wavelength)
    dq = np.zeros(wavelength.shape, dtype=np.uint32)
    background = np.ones_like(wavelength)
    berror = np.ones_like(wavelength)
...
def user_bkg_spec_c():
    ...
    data = np.array(list(zip(wavelength, flux, error, surf_bright, sb_error, dq, weight, n_input)), dtype=spec_table_dtype)
