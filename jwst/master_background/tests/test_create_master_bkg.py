...
def test_create_master_bkg_array_mismatch(caplog):
    wl = np.arange(37, dtype=np.float64) * 0.1 + 3.0
    surf_bright = np.ones(37, dtype=np.float64) * 7
...
def test_create_master_bkg_wrong_wavelength_shape(caplog):
    surf_bright = np.ones(37, dtype=np.float64) * 7
...
def test_create_master_bkg_wrong_surf_bright_shape(caplog):
    wl = np.arange(37, dtype=np.float64) * 0.1 + 3.0
