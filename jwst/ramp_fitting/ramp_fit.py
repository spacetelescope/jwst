...
def ols_ramp_fit( model, buffsize, save_opt, readnoise_model, gain_model,
    ...
    # Save original shapes for writing to log file, as these change for MIRI
    orig_nreads = nreads
    orig_cubeshape = cubeshape 
        
    # For MIRI datasets having >1 reads, resize the input model arrays to
    # exclude the final group. 
    ...
    if (ngroups == 1):
        log.warn('Dataset ramps have only a single good read, so count rates for')
        log.warn('each integration will be calculated as the value of that 1')
        log.warn('group divided by the group exposure time.')
...
def gls_ramp_fit(model,
    ...
    # Output integration-specific arrays if there are more than 1 integration
    if n_int > 1:
...
def fit_lines( data, mask_2d, end_heads, end_st, start, rn_sect, gain_sect,
    ...
    if weighting.lower() == 'optim': # do the fits using optimal weighting 
        ...
        sumx, sumxx, sumxy, sumy, nreads_wtd, xvalues =\
           calc_opt_sums( rn_sect, gain_sect, data_masked, mask_2d, xvalues ) 
