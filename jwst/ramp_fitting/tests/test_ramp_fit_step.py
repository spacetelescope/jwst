...
def test_ramp_fit_step_subarray(generate_miri_reffiles, setup_inputs):
    ...
    """
    override_gain, override_readnoise = generate_miri_reffiles
    model, gdq, rnModel, pixdq, err, gain = setup_subarray_inputs(
        ngroups=5, subxstart=10, subystart=20, subxsize=5, subysize=15, readnoise=50)

    model.meta.exposure.ngroups = 11
    model.data[0, 0, 12, 1] = 10.0
    model.data[0, 1, 12, 1] = 15.0
    model.data[0, 2, 12, 1] = 25.0
    model.data[0, 3, 12, 1] = 33.0
    model.data[0, 4, 12, 1] = 60.0
