...
def test_one_group_small_buffer_fit_ols():
    ...
    new_info, int_info, opt_model, gls_opt_model = ramp_fit(
        model1, 512, True, rnModel, gain, 'OLS', 'optimal', 'none')

    assert new_info is not None

    slopes_data = new_info[0]
    np.testing.assert_allclose(slopes_data[50, 50], 10.0, 1e-6)
...
@pytest.mark.skip(reason="GLS code does not [yet] handle single group integrations.  And GLS code is now out of date due to refactor.")
def test_one_group_small_buffer_fit_gls():
    model1, gdq, rnModel, pixdq, err, gain = setup_inputs(ngroups=1, gain=1, readnoise=10)
    model1.data[0, 0, 50, 50] = 10.0
    new_info, int_info, opt_model, gls_opt_model = ramp_fit(
        model1, 512, True, rnModel, gain, 'GLS', 'optimal', 'none')
...
def test_one_group_two_ints_fit_ols():
    ...
    slopes_data = new_info[0]
    np.testing.assert_allclose(slopes_data[50, 50], 11.0, 1e-6)


@pytest.mark.skip(reason="GLS does not correctly combine the slopes for integrations into the exposure slope.  And GLS code is now out of date due to refactor.  And GLS code is now out of date due to refactor.")
def test_gls_vs_ols_two_ints_ols():
    ...
    # TODO: the outputs are changing.  Data models are no longer being
    # returned, so this will fail  now.
...
@pytest.mark.skip(reason="Jenkins environment does not correctly handle multi-processing.  And GLS code is now out of date due to refactor.")
...
@pytest.mark.xfail(reason="GLS code does not [yet] handle single group integrations.  And GLS code is now out of date due to refactor.")
...
class TestMethods:
    ...
    def test_nocrs_noflux(self, method):
        ...
        new_info, int_info, opt_model, gls_opt_model = ramp_fit(
            model1, 60000, False, rnModel, gain, method, 'optimal', 'none')

        assert(new_info is not None)

        slopes_data = new_info[0]

        assert(0 == np.max(slopes_data))
        assert(0 == np.min(slopes_data))
    ...
    def test_nocrs_noflux_firstrows_are_nan(self, method):
        ...
        new_info, int_info, opt_model, gls_opt_model = ramp_fit(
            model1, 60000, False, rnModel, gain, 'OLS', 'optimal', 'none')
    ...
    def test_error_when_frame_time_not_set(self, method):
        ...
        new_info, int_info, opt_model, gls_opt_model = ramp_fit(
            model1, 64000, False, rnModel, gain, 'OLS', 'optimal', 'none')
    ...
    def test_bad_gain_values(self, method):
        ...
        slopes_data = new_info[0]
        slopes_dq = new_info[1]

        assert(0 == np.max(slopes_data))
        assert(0 == np.min(slopes_data))
        assert slopes_dq[10, 10] == 524288 + 1
        assert slopes_dq[20, 20] == 524288 + 1
    ...
    def test_simple_ramp(self, method):
        ...
        new_info, int_info, opt_model, gls_opt_model = ramp_fit(
            model1, 64000, True, rnModel, gain, 'OLS', 'optimal', 'none')
        ...
        np.testing.assert_allclose(slopes_data[50, 50], (20.0 / 3), 1e-6)
    ...
    def test_read_noise_only_fit(self, method):
        ...
        np.testing.assert_allclose(slopes_data[50, 50], coeff[0], 1e-6)
    ...
    def test_photon_noise_only_fit(self, method):
        ...
        np.testing.assert_allclose(slopes_data[50, 50], cds_slope, 1e-2)
    ...
    def test_photon_noise_only_bad_last_frame_two_groups(self, method):
        ...
        np.testing.assert_allclose(slopes_data[50, 50], cds_slope, 1e-6)


    def test_photon_noise_with_unweighted_fit(self, method):
        # Should probably be skipped since 'optimal' is hardcoded.
        model1, gdq, rnModel, pixdq, err, gain = setup_inputs(ngroups=5, gain=1000, readnoise=1)
        model1.data[0, 0, 50, 50] = 10.0
        model1.data[0, 1, 50, 50] = 15.0
        model1.data[0, 2, 50, 50] = 25.0
        ...
        new_info, int_info, opt_model, gls_opt_model = ramp_fit(
            model1, 1024 * 30000., True, rnModel, gain, 'OLS', 'unweighted', 'none')
    ...
    def test_two_groups_fit(self, method):
        ...
        # expect SATURATED
        assert slopes_dq[50, 51] == SATURATED
        # expect SATURATED and DO_NOT_USE, because 1st group is Saturated
        assert slopes_dq[50, 52] == SATURATED + DO_NOT_USE
    ...
    def test_four_groups_four_CRs(self, method):
        ...
        np.testing.assert_allclose(slopes_data[50, 50], 0, 1e-6)
    ...
    def test_four_groups_three_CRs_at_end(self, method):
        ...
        np.testing.assert_allclose(slopes_data[50, 50], expected_slope, 1e-6)


    def test_four_groups_CR_causes_orphan_1st_group(self, method):
        model1, gdq, rnModel, pixdq, err, gain = setup_inputs(
            ngroups=4, gain=.01, readnoise=10000)
        model1.data[0, 0, 50, 50] = 10.0
    ...
    def test_two_groups_unc(self, method):
        ...
        slopes_data = new_info[0]
        slopes_dq = new_info[1]
        slopes_var_poisson = new_info[2]
        slopes_var_rnoise = new_info[3]
        slopes_err = new_info[4]
        # delta_electrons = deltaDN * ingain
        single_sample_readnoise = inreadnoise / np.sqrt(2)

        np.testing.assert_allclose(
            slopes_var_poisson[50, 50],
            ((deltaDN / ingain) / grouptime**2),
            1e-6)
        np.testing.assert_allclose(
            slopes_var_rnoise[50, 50],
    ...
    def test_oneCR_10_groups_combination(self, method):
        ...
        np.testing.assert_allclose(sdata[50, 50], 2.5, rtol=1e-5)
    ...
    def test_oneCR_10_groups_combination_noisy2ndSegment(self, method):
        ...
        slopes, int_model, opt_info, gls_opt_model = ramp_fit(
            model1, 1024 * 30000., True, rnModel, gain, 'OLS', 'optimal', 'none')
