...
def test_one_group_small_buffer_fit_ols():
    ...
    slopes, cube, optional, gls_dummy = ramp_fit(model1, 512, True, rnoise, gain, "OLS", "optimal", "none", dqflags.pixel)
...
def test_readnoise_variance():
    ...
    model1, gdq_4d, rnoise, pixdq, err, gain = setup_inputs(
        ngroups=10, nints=3, nrows=3, ncols=4, nframes=1, gain=1, readnoise=0.7071, grouptime=group_time
    )
    ...
    true_var_r2 = np.array(
        [
            [0.0000000e00, 1.7979382e-05, 0.0000000e00, 7.4164942e-04],
    ...
    true_var_r3_0 = np.array(
        [
            [0.0000000e00, 5.3938144e-05, 0.0000000e00, 2.2249483e-03],
    ...
    true_var_r4_0_0 = np.array(
        [
            [0.0000000e00, 5.3938144e-05, 0.0000000e00, 2.2249483e-03],
            [0.0000000e00, 5.3938144e-05, 2.5427982e-04, 1.5892487e-04],
...
def test_one_group_two_ints_fit_ols():
    model1, gdq, rnoise, pixdq, err, gain = setup_inputs(ngroups=1, gain=1.0, readnoise=10.0, nints=2)
    model1.data[0, 0, 50, 50] = 10.0
    model1.data[1, 0, 50, 50] = 12.0

    slopes, cube, optional, gls_dummy = ramp_fit(
        model1, 1024 * 30000.0, False, rnoise, gain, "OLS", "optimal", "none", dqflags.pixel
    )
...
def test_multiprocessing():
    ...
    slopes, int_model, opt_model, gls_opt_model = ramp_fit(
        model1, 1024 * 30000.0, False, rnoise, gain, algo, "optimal", "none", dqflags.pixel
    )

    slopes_multi, int_model_multi, opt_model_multi, gls_opt_model_multi = ramp_fit(
        model2, 1024 * 30000.0, False, rnoise2, gain2, algo, "optimal", "all", dqflags.pixel
    )
...
class TestMethods:
    def test_nocrs_noflux(self, method):
        ...
        slopes, cube, optional, gls_dummy = ramp_fit(model1, 60000, False, rnoise, gain, method, "optimal", "none", dqflags.pixel)
    ...
    def test_nocrs_noflux_firstrows_are_nan(self, method):
        ...
        slopes, cube, optional, gls_dummy = ramp_fit(model1, 60000, False, rnoise, gain, "OLS", "optimal", "none", dqflags.pixel)
    ...
    def test_error_when_frame_time_not_set(self, method):
        ...
        slopes, cube, optional, gls_dummy = ramp_fit(model1, 64000, False, rnoise, gain, "OLS", "optimal", "none", dqflags.pixel)
    ...
    def test_five_groups_two_ints_Poisson_noise_only(self, method):
        ...
        slopes, cube, optional, gls_dummy = ramp_fit(
            model1, 1024 * 30000.0, True, rnoise, gain, "OLS", "optimal", "none", dqflags.pixel
        )
    ...
    def test_simple_ramp(self, method):
        ...
        slopes, cube, optional, gls_dummy = ramp_fit(model1, 64000, True, rnoise, gain, "OLS", "optimal", "none", dqflags.pixel)
    ...
    @pytest.mark.xfail(
        reason="Fails, bad last frame yields only one good one. \
    ...
    def test_photon_noise_with_unweighted_fit(self, method):
        ...
        slopes, cube, optional, gls_dummy = ramp_fit(
            model1, 1024 * 30000.0, True, rnoise, gain, "OLS", "unweighted", "none", dqflags.pixel
        )
    ...
    def test_four_groups_four_CRs(self, method):
        ...
        model1, gdq, rnoise, pixdq, err, gain = setup_inputs(ngroups=4, nrows=1, ncols=2, gain=1.0, readnoise=10.0)
        row, col = 0, 1
    ...
    def test_four_groups_CR_causes_orphan_1st_group(self, method):
        model1, gdq, rnoise, pixdq, err, gain = setup_inputs(ngroups=4, nrows=2, ncols=2, gain=1.05, readnoise=12.5)
    ...
    def test_two_groups_unc(self, method):
        ...
        np.testing.assert_allclose(
            var_rnoise[50, 50], (12 * single_sample_readnoise**2 / (ngroups * (ngroups**2 - 1) * grouptime**2)), 1e-6
        )
        np.testing.assert_allclose(
            err[50, 50], (np.sqrt((deltaDN / ingain) / grouptime**2 + (inreadnoise**2 / grouptime**2))), 1e-6
        )
    ...
    def test_five_groups_unc(self, method):
        ...
        np.testing.assert_allclose(var_poisson[50, 50], ((median_slope) / (ingain * delta_time)), 1e-6)
        np.testing.assert_allclose(
            var_rnoise[50, 50], (12 * single_sample_readnoise**2 / (ngroups * (ngroups**2 - 1) * grouptime**2)), 1e-6
        )
        np.testing.assert_allclose(err[50, 50], np.sqrt(var_poisson[50, 50] + var_rnoise[50, 50]), 1e-6)
    ...
    def test_oneCR_10_groups_combination(self, method):
        ...
        np.testing.assert_allclose(
            ...
            (12.0 * single_sample_readnoise**2 / (segment_groups * (segment_groups**2 - 1) * grouptime**2)),
            rtol=1e-6,
        )
...
def test_twenty_groups_two_segments():
    ...
    model1, gdq, rnoise, pixdq, err, gain = setup_small_cube(ngroups, nints, nrows, ncols, deltatime)
    ...
    (oslope, sigslope, var_poisson, var_rnoise, oyint, sigyint, opedestal, weights, crmag) = opt_info
...
def test_miri_first_last():
    ...
    model1.data[0, :, 0, 0] = np.array([-200.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0, -500.0], dtype=np.float32)
    model1.data[0, :, 0, 1] = np.array([-300.0, 80.0, 90.0, 100.0, 110.0, 120.0, 130.0, 140.0, 150.0, -600.0], dtype=np.float32)
    model1.data[0, :, 1, 0] = np.array([-200.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 110.0, 900.0], dtype=np.float32)
    model1.data[0, :, 1, 1] = np.array([-600.0, 140.0, 150.0, 160.0, 170.0, 180.0, 190.0, 200.0, 210.0, -400.0], dtype=np.float32)
...
def setup_small_cube(ngroups=10, nints=1, nrows=2, ncols=2, deltatime=10.0, gain=1.0, readnoise=10.0):
    """
...
def setup_inputs(ngroups=10, readnoise=10.0, nints=1, nrows=103, ncols=102, nframes=1, grouptime=1.0, gain=1.0, deltatime=1.0):
    data = np.zeros(shape=(nints, ngroups, nrows, ncols), dtype=np.float32)
