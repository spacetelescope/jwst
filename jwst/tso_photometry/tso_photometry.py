...
def tso_aperture_photometry(datamodel, xcenter, ycenter, radius, radius_inner, radius_outer):
    ...
    if sub64p_wlp8:
        info = "Photometry measured as the sum of all values in the " "subarray.  No background subtraction was performed."
        ...
        info = (
            "Photometry measured in a circular aperture of r={0} "
            "pixels.  Background calculated as the mean in a "
            "circular annulus with r_inner={1} pixels and "
            "r_outer={2} pixels.".format(radius, radius_inner, radius_outer)
        )
        for i in np.arange(nimg):
            aper_sum, aper_sum_err = phot_aper.do_photometry(datamodel.data[i, :, :], error=datamodel.err[i, :, :])
            ann_sum, ann_sum_err = bkg_aper.do_photometry(datamodel.data[i, :, :], error=datamodel.err[i, :, :])
    ...
    if nrows > 0:
        ...
        if offset < 0:
            log.warning("Range of integration numbers in science data extends " "outside the range in INT_TIMES table.")
    ...
    if nrows == 0:
        ...
        n_dt = datamodel.meta.exposure.integration_end - datamodel.meta.exposure.integration_start + 1
    ...
    if not sub64p_wlp8:
        ...
        colnames = ["annulus_sum", "annulus_sum_err", "annulus_mean", "annulus_mean_err", "aperture_bkg", "aperture_bkg_err"]
        for col in colnames:
