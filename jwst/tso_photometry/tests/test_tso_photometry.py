...
def set_meta(datamodel, sub64p=False):
    ...
    datamodel.meta.exposure.integration_end = datamodel.meta.exposure.integration_start + datamodel.meta.exposure.nints - 1
...
def include_int_times(datamodel):
    ...
    otab = np.array(list(zip(integration_number, dummy, time_arr, dummy, dummy, dummy, dummy)), dtype=it_dtype)
...
def test_tso_phot_1():
    ...
    catalog = tso_aperture_photometry(datamodel, xcenter, ycenter, radius + 1.0, radius_inner, radius_outer)
...
def test_tso_phot_2():
    ...
    catalog = tso_aperture_photometry(datamodel, xcenter, ycenter, radius, radius_inner, radius_outer)
    ...
    assert np.allclose(catalog["aperture_sum"].value, (value + background) * shape[-1] * shape[-2], rtol=1.0e-6)
    assert np.allclose(catalog["aperture_sum_err"].value, 0.0, atol=1.0e-7)
...
def test_tso_phot_4():
    ...
    int_times = np.array([58704.62853, 58704.628655, 58704.62878, 58704.62890, 58704.6290, 58704.6291511, 58704.629275])
...
def test_tso_phot_5():
    ...
    with pytest.raises(ValueError):
        tso_aperture_photometry(datamodel, xcenter, ycenter, radius + 1.0, radius_inner, radius_outer)
