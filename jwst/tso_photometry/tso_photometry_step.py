...
class TSOPhotometryStep(Step):
    ...
    def process(self, input_data):
        ...
        if output_model.meta.wcsinfo.siaf_xref_sci is None:
            raise ValueError("XREF_SCI is missing.")
        if output_model.meta.wcsinfo.siaf_yref_sci is None:
            raise ValueError("YREF_SCI is missing.")
        if output_model.meta.bunit_data is None:
            raise ValueError("BUNIT for data array is missing.")
        if output_model.meta.bunit_err is None:
            raise ValueError("BUNIT for error array is missing.")

        xcenter = output_model.meta.wcsinfo.siaf_xref_sci - 1  # 1-based origin
        ycenter = output_model.meta.wcsinfo.siaf_yref_sci - 1  # 1-based origin
        ...
        if reffile_utils.ref_matches_sci(output_model, gain_m):
            gain_2d = gain_m.data
        else:
            log.info("Extracting gain subarray to match science data")
            gain_2d = reffile_utils.get_subarray_model(output_model, gain_m).data
        ...
        convert_data_units(output_model, gain_2d)
        ...
        # Since the datamodel is not returned, close it if it was opened here.
