...
from .tso_photometry import convert_data_units, tso_aperture_photometry, tso_source_centroid
...
class TSOPhotometryStep(Step):
    ...
    def process(self, input_data):
        ...
        with CubeModel(input_data) as model:
            ...
            self.log.debug(f"initial xcenter = {xcenter}")
            self.log.debug(f"initial ycenter = {ycenter}")
            ...
            if self.centroid_source:
                ...
                if np.all(np.isnan(centroid_x)) or np.all(np.isnan(centroid_y)):
                    ...
                    xc = xcenter
                    yc = ycenter
                    self.log.info(f"Using planned center x,y = {xc:.2f},{yc:.2f}")
