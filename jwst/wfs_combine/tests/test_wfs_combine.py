...
def gaussian(x, mu, sig):
    return 1.0 / (np.sqrt(2 * np.pi) * sig) * np.exp(-np.power((x - mu) / sig, 2) / 2)
...
def wfs_association(tmp_path_factory):
    ...
    im1.meta.observation = {"exposure_number": "1", "date": "2021-10-25", "time": "16:58:27.258"}
    im1.meta.exposure = {"type": "NRC_IMAGE"}

    im1 = AssignWcsStep.call(im1, sip_approx=False)

    im2 = im1.copy()
    im2.meta.observation = {"exposure_number": "2", "date": "2021-10-25", "time": "17:58:27.258"}
    ...
    asn = asn_from_list(["image1_cal.fits", "image2_cal.fits"], product_name="jw00024-a3001_t001_nircam_nrca4_{suffix}")
...
def test_create_combined(tmp_cwd, wfs_association, data1, data2, dq1, dq2, result_data, result_dq):
    ...
    result = WfsCombineStep.call(path_asn, do_refine=False, flip_dithers=False, psf_size=50, blur_size=10, n_size=2)
...
def test_shift_order_no_refine_no_flip(wfs_association):
    ...
    wfs = wfs_combine.DataSet(
        path1, path2, "outfile.fits", do_refine=False, flip_dithers=False, psf_size=50, blur_size=10, n_size=2
    )
    wfs.do_all()
...
def test_shift_order_no_refine_with_flip(wfs_association):
    ...
    with datamodels.open(path2) as im2:
        ...
        im2.save(path2)
    wfs = wfs_combine.DataSet(
        path1, path2, "outfile.fits", do_refine=False, flip_dithers=True, psf_size=50, blur_size=10, n_size=2
    )
    wfs.do_all()
    wfs.input_1.close()
    wfs.input_2.close()
...
def test_refine_no_error(wfs_association, xshift, yshift, xerror, yerror, flip_dithers):
    ...
    with datamodels.open(path2) as im2:
        ...
        im2.save(path2)
    wfs = wfs_combine.DataSet(
        path1, path2, "outfile.fits", do_refine=True, flip_dithers=flip_dithers, psf_size=50, blur_size=10, n_size=2
    )
    wfs.do_all()
...
def test_refine_with_error(wfs_association):
    ...
    with datamodels.open(path2) as im2:
        ...
        im2.save(path2)
    wfs = wfs_combine.DataSet(
        path1, path2, "outfile.fits", do_refine=True, flip_dithers=True, psf_size=50, blur_size=10, n_size=2
    )
    wfs.do_all()
    assert wfs.input_1.meta.observation.exposure_number == "1"
    assert wfs.input_2.meta.observation.exposure_number == "2"
