...
def wfs_association(tmp_path_factory):
    ...
    asn = asn_from_list(
        [path1, path2], product_name="jw00024-a3001_t001_nircam_nrca4_{suffix}"
    )
...
def test_create_combined(
    _jail, wfs_association, data1, data2, dq1, dq2, result_data, result_dq
):
...
def test_refine_with_error(wfs_association):
    ...
    with datamodels.open(path2) as im2:
        ...
        im2.data = np.zeros(shape=(data_size, data_size), dtype=np.float32)
        im2.data = add_point_source(
            im2.data, 200, 100 + delta_pixel + shift_error, 100, 4, 4
        )
        im2.save(path2)
    wfs = wfs_combine.DataSet(
        path1,
