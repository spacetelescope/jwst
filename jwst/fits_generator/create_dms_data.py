...
def get_subarray_name(instrument, colstart, colstop, rowstart, rowstop):
    for subarray in subarrays[instrument].values():
        if (
            rowstart == subarray[2]
            and rowstop == subarray[3]
            and colstart == subarray[0]
    ...
    if sca in [493, 494, 495]:
        ...
        if sca == 493:
            if header["EXP_TYPE"] == "":
                ...
                if miri_filter == "P750L":
                    subarray_name = get_subarray_name(
                        instrument, colstart, colstop, rowstart, rowstop
                    )
        ...
        if input_file_types.is_nirspec_ips(input_hdulist):
            ...
            try:
                ...
                print(
                    (
                        "Detector row start = %d, from keyword A1_ROW_C"
                        % detector_row_start
                    )
                )
            except KeyError:
                try:
                    ...
                    print(
                        (
                            "Detector row start = %d, from keyword A1WINVSA"
                            % detector_row_start
                        )
                    )
                except KeyError:
                    ...
                    detector_row_start = (
                        0  # corresponds to 1 in detector/IRAF coordinates
                    )
            try:
                detector_column_start = int(float(header["A1_COL_C"]))
                print(
                    (
                        "Detector column start = %d, from keyword A1_COL_C"
                ...
                try:
                    ...
                    print(
                        (
                            "Detector column start = %d, from keyword A1WINHSA"
                            % detector_column_start
                        )
                    )
                except KeyError:
        ...
        if input_file_types.is_nirspec_ips(input_hdulist):
            ...
            try:
                ...
                print(
                    (
                        "Detector row start = %d from keyword A2_ROW_C"
                        % detector_row_start
                    )
                )
            except KeyError:
            ...
            try:
                ...
                print(
                    (
                        "Detector columns start = %d from keyword A2_COL_C"
                        % detector_column_start
                    )
                )
            except KeyError:
...
def create_single_subarray(input_hdulist, subarray):
    ...
    if instrument == "MIRI":
        ...
        refout_subarray = continuous_refout[
            :, (ystart - 1) : ystop, (xstart - 1) // 4 : (xstop + 1) // 4
        ]
...
def create_dms(base_file, level="1b", parfile=None, subarray=None, exp_type="UNKNOWN"):
    ...
    if subarray is not None:
        subarray_file = create_single_subarray(base_hdulist, subarray)
        print("Parfile = %s" % parfile)
        if parfile is None:

            parfile = "".join(
                (base_root, "_", subarray[4].lower(), "_observation_identifiers.dat")
            )
            print(parfile)
        ...
        hdulist = main.generate([base_hdulist, parfile], level=level)
        filename = main.guess_filename(hdulist)
        hdulist[0].header["FILENAME"] = filename
        hdulist.writeto(filename, output_verify="silentfix")
