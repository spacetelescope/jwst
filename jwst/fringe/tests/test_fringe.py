...
def test_data_correction(setup_inputs):
    ...
    assert_array_equal(output_model.data[good_pix],
                       (input_model.data * FRINGE_CONSTANT)[good_pix])
    assert_array_equal(output_model.err[good_pix],
                       (input_model.err * FRINGE_CONSTANT)[good_pix])
    assert_array_equal(output_model.var_poisson[good_pix],
                       (input_model.var_poisson * FRINGE_CONSTANT_SQ)[good_pix])
    assert_array_equal(output_model.var_rnoise[good_pix],
                       (input_model.var_rnoise * FRINGE_CONSTANT_SQ)[good_pix])
    assert_array_equal(output_model.var_flat[good_pix],
                       (input_model.var_flat * FRINGE_CONSTANT_SQ)[good_pix])
