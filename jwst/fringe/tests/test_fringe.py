...
def test_data_correction(setup_inputs):
    ...
    assert (output_model.data[good_pix] == (input_model.data * FRINGE_CONSTANT)[good_pix]).all()
    assert (output_model.err[good_pix] == (input_model.err * FRINGE_CONSTANT)[good_pix]).all()
...
def setup_inputs():
    ...
    def _setup(shape=(2, 2)):
        input_data = (np.ones(shape[0] * shape[1])).reshape(shape) * 6.0
