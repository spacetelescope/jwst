...
xart_wrapper(PyObject *module, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "iiiiOOOOOOOO:xart_wrapper", &imin, &imax, &xsize_det, &ysize_det, &xveco, &fimgo, &gammao, &lor_ampo,
            &g_stdo, &g_dxo, &g1_ampo, &g2_ampo)) {
        return NULL;
    }
    ...
    status = xart_model(
        imin, imax, xsize_det, ysize_det, (double *)PyArray_DATA(xvec), (double *)PyArray_DATA(fimg),
        (double *)PyArray_DATA(gamma), (double *)PyArray_DATA(lor_amp), (double *)PyArray_DATA(g_std),
        (double *)PyArray_DATA(g_dx), (double *)PyArray_DATA(g1_amp), (double *)PyArray_DATA(g2_amp), &xart_flux);
...
static PyMethodDef xart_methods[] = {
    {"xart_wrapper", xart_wrapper, METH_VARARGS, "xart_wrapper()"}, {NULL, NULL, 0, NULL} /* sentinel */
};
