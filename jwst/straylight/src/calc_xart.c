...
int
alloc_xart_arrays(int nelem, double **fluxv)
{
...
int
xart_model(
    int imin, int imax, int xsize_det, int ysize_det, double *xvec, double *fimg, double *gamma,
    ...
    for (j = 0; j < ysize_det; j++) {
        for (i = imin; i < imax; i++) {
            for (k = 0; k < xsize_det; k++) {
                fluxv[xsize_det * j + k] +=
                    (fimg[j * xsize_det + i] * lor_amp[j] * gamma[j] * gamma[j]) /
                    (gamma[j] * gamma[j] + (xvec[k] - i) * (xvec[k] - i));
                fluxv[xsize_det * j + k] +=
                    (fimg[j * xsize_det + i] * g1_amp[j] *
                     exp(-((xvec[k] - i - g_dx[j]) * (xvec[k] - i - g_dx[j])) /
                         (2 * g_std[j] * g_std[j])));
                fluxv[xsize_det * j + k] +=
                    (fimg[j * xsize_det + i] * g1_amp[j] *
                     exp(-((xvec[k] - i + g_dx[j]) * (xvec[k] - i + g_dx[j])) /
...
static PyObject *
xart_wrapper(PyObject *module, PyObject *args)
{
    PyObject *result = NULL, *xveco, *fimgo, *gammao, *lor_ampo, *g_stdo, *g_dxo, *g1_ampo,
             *g2_ampo;
    ...
    if (!PyArg_ParseTuple(
            args, "iiiiOOOOOOOO:xart_wrapper", &imin, &imax, &xsize_det, &ysize_det, &xveco, &fimgo,
    ...
    status = xart_model(
        imin, imax, xsize_det, ysize_det, (double *) PyArray_DATA(xvec),
...
static PyMethodDef xart_methods[] = {
    {"xart_wrapper", xart_wrapper, METH_VARARGS, "xart_wrapper()"},
