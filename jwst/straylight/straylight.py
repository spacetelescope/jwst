...
def makemodel_ccode(
    fimg, xvec, imin, imax, lor_fwhm, lor_amp, g_fwhm, g_dx, g1_amp, g2_amp
):
    ...
    for yy in range(0, 1024):
        for ii in range(imin, imax):
            ...
            model1d[1032 * yy : 1032 * (yy + 1)] += (
                ...
                * g1_amp[yy]
                * np.exp(
                    -((xvec - ii - g_dx[yy]) * (xvec - ii - g_dx[yy]))
                    / (2 * gstd[yy] * gstd[yy])
                )
            )
            model1d[1032 * yy : 1032 * (yy + 1)] += (
...
def correct_xartifact(input_model, modelpars):
    ...
    if left == "N/A" or right == "N/A":
        log.info(
            "Warning: no parameters found for channel = "
            + str(channel)
            + " band = "
            + str(band)
        )
    ...
    try:
        ...
        log.info(
            "Found parameters for left detector half, applying Cross-Artifact correction."
        )
        istart, istop = 0, 516
        ...
        left_model[:, :] = 0
        log.info(
            "No parameters for left detector half, not applying Cross-Artifact correction."
        )
    ...
    try:
        ...
        log.info(
            "Found parameters for right detector half, applying Cross-Artifact correction."
        )
        ...
        right_model[:, :] = 0
        log.info(
            "No parameters for right detector half, not applying Cross-Artifact correction."
        )
