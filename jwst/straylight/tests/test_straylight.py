...
from jwst.straylight.straylight import makemodel_ccode
import numpy as np
...
def test_correct_mrs_xartifact():
    ...
    result = makemodel_ccode(image,xvec,istart,istop,lorfwhm,lorscale,gaufwhm,gauxoff,gauscale1,gauscale2)
    cutout = result[500,270:330]
    ...
    assert(np.allclose(compare, cutout, rtol=1e-6))
