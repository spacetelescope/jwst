    ...
    def process(self, mnemonics):
        ...
        for mnemonic in mnemonics:
            try:
                ...
                self.log.info("Cannot retrieve info for {}".format(mnemonic))
                continue

            if len(values) < 1:
                self.log.info("{} has no entries in time range {}:{}".format(mnemonic, stime, etime))
                continue

            if verbosity == "initial":
                result[mnemonic] = values[0]
                self.log.info("{}[{}:{}] = {}".format(mnemonic, stime, etime, values[0]))
            elif verbosity == "all":
                result[mnemonic] = values
                self.log.info("{}[{}:{}] = {}".format(mnemonic, stime, etime, values))
                next
