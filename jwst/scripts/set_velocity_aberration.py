...
def parse_args(args):
    ...
    parser.add_argument(
        "filename", nargs="+", help="FITS file(s) to which to add the velocity aberration correction information."
    )
    parser.add_argument(
        "-f",
        "--force_level1bmodel",
...
def deprecated_name():
    ...
    warnings.warn(f"usage of `{filename.name}` is deprecated; use `{filename.stem}` instead")
