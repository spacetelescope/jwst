...
def main():
    ...
    parser.add_argument('--ignore_hdus',
                        dest='ignore_hdus', action='store', default=['ASDF'],
                        help='HDU names to ignore when comparing two FITS files.')
    ignore_keywords = ['DATE', 'CAL_VER', 'CAL_VCS', 'CRDS_VER', 'CRDS_CTX',
                       'NAXIS1', 'TFORM*']
    parser.add_argument('--ignore_keywords',
                        dest='ignore_keywords', action='store', default=ignore_keywords,
                        help='Header keywords to ignore when comparing two headers.')
    parser.add_argument('--ignore_comments',
                        dest='ignore_comments', action='store', default=ignore_keywords,
                        help='A list of header keywords whose comments should be ignored in the comparison.')
    parser.add_argument('--ignore_fields',
                        dest='ignore_fields', action='store', default=[],
                        help='The (case-insensitive) names of any table columns to ignore, if any table data'
                             ' is to be compared.')
    parser.add_argument('--numdiffs',
                        dest='numdiffs', action='store', default=10,
    ...
    parser.add_argument('--rtol',
                        dest='rtol', action='store', default=1e-5,
    ...
    parser.add_argument('--atol',
                        dest='atol', action='store', default=1e-7,
                        help='The allowed absolute difference.')
    parser.add_argument('--ignore_blanks',
                        dest='ignore_blanks', action='store_false', default=True,
                        help='Ignore extra whitespace at the end of string values either in headers or data.')
    parser.add_argument('--ignore_blank_cards',
                        dest='ignore_blank_cards', action='store_false', default=True,
                        help='Ignore all cards that are blank, i.e. they only contain whitespace.')
    parser.add_argument('--report_pixel_loc_diffs',
                        dest='report_pixel_loc_diffs', action='store_true', default=False,
                        help='Report the numdiffs pixel locations where differences are found.')
    parser.add_argument('--extension_tolerances',
                        dest='extension_tolerances', action='store', default={},
                        help="Provide a different relative and absolute tolerance for the given extensions, e.g."
    ...
    stfitsdiff_default_kwargs = {
        ...
        'extension_tolerances': args.extension_tolerances
    }
