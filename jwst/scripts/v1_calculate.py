...
REDUCE_FUNCS_MAPPING = {"all": stp.all_pointings, "first": stp.first_pointing, "average": stp.pointing_from_average}
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Either a list of JWST data files to retrieve the timing from"
    ...
    parser.add_argument(
        ...
        default="average",
        help=(
            "Which pointing(s) to use within the specified time interval." f" Choices: {REDUCE_FUNCS}" " (default: %(default)s)"
        ),
    )
    parser.add_argument(
        "--method",
    ...
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="Increase verbosity. Specifying multiple times adds more output."
    )
    parser.add_argument("--override-transforms", type=str, default=None, help="Transform matrices to use instead of calculated")
    parser.add_argument(
        "--engdb-url",
        type=str,
        default=None,
