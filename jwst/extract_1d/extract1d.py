...
def extract1d(
    ...
    if not (independent_var.startswith("pixel") or independent_var.startswith("wavelength")):
    ...
    for i in range(n_srclim):
        ...
        if diff.min() < 0.0:
            if diff.max() < 0.0:
                log.error("Lower and upper source extraction limits" " appear to be swapped.")
                raise ValueError("Lower and upper source extraction limits" " appear to be swapped.")
            else:
                log.error("Lower and upper source extraction limits" " cross each other.")
                raise ValueError("Lower and upper source extraction limits" " cross each other.")
        del diff
    ...
    for i in range(nbkglim):
        ...
        if diff.min() < 0.0:
            if diff.max() < 0.0:
                log.error("Lower and upper background extraction limits" " appear to be swapped.")
                raise ValueError("Lower and upper background extraction limits" " appear to be swapped.")
            else:
                log.error("Lower and upper background extraction limits" " cross each other.")
                raise ValueError("Lower and upper background extraction limits" " cross each other.")
        del diff
        if np.any(lower < -0.5) or np.any(upper < -0.5):
    ...
    for j in range(nl):
        ...
        if nbkglim > 0:
            ...
            (bkg_model, b_var_poisson_model, b_var_rnoise_model, b_var_flat_model, bkg_npts) = _fit_background_model(
                temp_image, var_poisson, var_rnoise, var_flat, x, j, bkglim, bkg_fit, bkg_order
            )

            if bkg_npts == 0:
                bkg_model = None
                log.debug(f"Not enough valid pixels to determine background " f"for lambda={lam:.6f} (column {x:d})")
        ...
        (
            ...
            srclim,
            weights=weights,
            bkgmodels=[bkg_model, b_var_poisson_model, b_var_rnoise_model, b_var_flat_model],
        )
        if nbkglim > 0:
    ...
    return (temp_flux, f_var_poisson, f_var_rnoise, f_var_flat, background, b_var_poisson, b_var_rnoise, b_var_flat, npixels)
...
def _extract_src_flux(image, var_poisson, var_rnoise, var_flat, x, j, lam, srclim, weights, bkgmodels):
    ...
    return (total_flux, f_var_poisson, f_var_rnoise, f_var_flat, bkg_flux, b_var_poisson, b_var_rnoise, b_var_flat, tarea, twht)


def _fit_background_model(image, var_poisson, var_rnoise, var_flat, x, j, bkglim, bkg_fit, bkg_order):
    ...
    if npts <= 1 or not np.any(good):
        return (models.Polynomial1D(0), models.Polynomial1D(0), models.Polynomial1D(0), models.Polynomial1D(0), 0)
    ...
    if bkg_fit == "poly":
        ...
        bkg_model = lsqfitter(models.Polynomial1D(min(bkg_order, npts - 1)), y, val, weights=wht)
        b_var_poisson_model = lsqfitter(models.Polynomial1D(min(bkg_order, npts - 1)), y, var_poisson_val, weights=wht)
        b_var_rnoise_model = lsqfitter(models.Polynomial1D(min(bkg_order, npts - 1)), y, var_rnoise_val, weights=wht)
        b_var_flat_model = lsqfitter(models.Polynomial1D(min(bkg_order, npts - 1)), y, var_flat_val, weights=wht)
...
def _coalesce_bounds(segments):
    ...
    if all([isinstance(x, list) for x in intervals]):
        ...
        for x in intervals:
            if len(x) != 2:
                raise ValueError("Each list in the 'segments' list must have " "exactly two elements")
            try:
                map(float, x)
            except TypeError:
                raise TypeError("Each segment in 'segments' must be a list of " "two numbers")
    else:
        # we have a single "interval" (or "segment"):
        if len(intervals) != 2:
            raise ValueError(
                "'segments' list must be a list of lists of two " "numbers or 'segments' must be a list of exactly " "two numbers"
            )
        try:
            map(float, intervals)
        except TypeError:
