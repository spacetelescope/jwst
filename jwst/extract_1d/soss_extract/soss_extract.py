...
def get_ref_file_args(ref_files, transform):
    ...
    specprofile_o1 = transform_profile(
        transform, specprofile_ref.profile[0].data, ovs, pad, norm=False
    )
    specprofile_o2 = transform_profile(
        transform, specprofile_ref.profile[1].data, ovs, pad, norm=False
    )

    # The throughput curves for order 1 and 2.
    spectrace_ref = ref_files["spectrace"]

    throughput_o1 = ThroughputSOSS(
        spectrace_ref.trace[0].data["WAVELENGTH"],
        spectrace_ref.trace[0].data["THROUGHPUT"],
    )
    throughput_o2 = ThroughputSOSS(
        spectrace_ref.trace[1].data["WAVELENGTH"],
        spectrace_ref.trace[1].data["THROUGHPUT"],
    )
    ...
    # Get kernels
    kernels_o1 = WebbKernel(
        speckernel_ref.wavelengths, speckernel_ref.kernels, centroid[1], ovs, n_pix
    )
    kernels_o2 = WebbKernel(
        speckernel_ref.wavelengths, speckernel_ref.kernels, centroid[2], ovs, n_pix
    )
    ...
    valid_wavemap = (speckernel_wv_range[0] <= wavemap_o1) & (
        wavemap_o1 <= speckernel_wv_range[1]
    )
    wavemap_o1 = np.where(valid_wavemap, wavemap_o1, 0.0)
    valid_wavemap = (speckernel_wv_range[0] <= wavemap_o2) & (
        wavemap_o2 <= speckernel_wv_range[1]
    )
...
def estim_flux_first_order(scidata_bkg, scierr, scimask, ref_files, threshold):
    ...
    ref_file_args = [wave_maps[0]], [spat_pros[0]], [thrpts[0]], [np.array([1.0])]
    kwargs = {
        "wave_grid": wave_grid,
        "orders": [1],
        "global_mask": mask,
...
def _mask_wv_map_centroid_outside(
    wave_maps, ref_files, transform, y_max, orders=(1, 2)
):
...
def model_image(
    ...
    engine = ExtractionEngine(
        *ref_file_args, n_os=n_os, threshold=threshold, c_kwargs=c_kwargs
    )
    ...
    if tikfac is None:
        ...
        try:
            estimate = estim_flux_first_order(
                scidata_bkg, scierr, scimask, ref_file_args, threshold
            )
            # Initial pass 8 orders of magnitude with 10 grid points.
            factors = engine.estimate_tikho_factors(
                estimate, log_range=[-4, 4], n_points=10
            )
        except Exception as e:
            log.warning(
                f"Error caught in first order flux estimation: {e}\n"
                f"Using pre-defined array for flux factor estimate."
            )
            factors = np.logspace(-19, -10, 10)
        # Find the tikhonov factor.
        tiktests = engine.get_tikho_tests(
            factors, data=scidata_bkg, error=scierr, mask=scimask
        )
    ...
    f_k = engine.__call__(
        data=scidata_bkg, error=scierr, mask=scimask, tikhonov=True, factor=tikfac
    )
...
def extract_image(
    ...
    for order_integer in order_list:
        ...
        xtrace, ytrace, wavelengths[order] = get_trace_1d(
            ref_files, transform, order_integer
        )
        ...
        if bad_pix == "model":
            ...
            try:
                ...
                scierr_ord = estim_error_nearest_data(
                    scierr, decont, pix_to_estim, valid_pix
                )
                ...
                scierr_ord = scierr
                log.warning(
                    f"Bad pixels in {order} will be masked instead of modeled: trace model unavailable."
                )
        else:
            # Mask pixels
            scimask_ord = scimask
...
def run_extract1d(
    ...
    if isinstance(input_model, datamodels.ImageModel):
        ...
        scidata_bkg, col_bkg, npix_bkg = soss_background(
            scidata, scimask, bkg_mask=bkg_mask
        )
        ...
        if transform is None:
            ...
            if subarray == "SUBSTRIP96" or soss_filter == "F277W":
                ...
                transform = solve_transform(
                    scidata_bkg, scimask, xref_o1, yref_o1, soss_filter=soss_filter
                )
            else:
                transform = solve_transform(
                    scidata_bkg,
        ...
        log.info(
            "Measured to Reference trace position transform: theta={:.4f}, dx={:.4f}, dy={:.4f}".format(
        ...
        if subarray != "SUBSTRIP96":
            _mask_wv_map_centroid_outside(
                ref_file_args[0], ref_files, transform, scidata_bkg.shape[0]
            )
        ...
        if soss_filter == "CLEAR":
            ...
            result = model_image(
                scidata_bkg, scierr, scimask, refmask, ref_file_args, **kwargs
            )
        ...
        for i in range(nimages):
            ...
            scimask = np.bitwise_and(
                input_model.dq[i], dqflags.pixel["DO_NOT_USE"]
            ).astype(bool)
            refmask = bitfield_to_boolean_mask(
                input_model.dq[i],
                ignore_flags=dqflags.pixel["REFERENCE_PIXEL"],
            ...
            for order in wavelengths.keys():
                ...
                spec.int_num = (
                    i + 1
                )  # integration number starts at 1, not 0 like python
