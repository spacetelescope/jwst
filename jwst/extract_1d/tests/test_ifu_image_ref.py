...
x_center = 23.0
y_center = 26.0  # offset +2 from image center
radius = 11.5
inner_bkg = 11.5
outer_bkg = 16.5
method = "exact"
...
def test_ifu_3d():
    ...
    ref_image_3d = make_ref_image(
        data_shape,  # 3-D ref image
        x_center=x_center,
        y_center=y_center,
        radius=radius,
        inner_bkg=inner_bkg,
    ...
    ref_dict_2d = {"ref_file_type": extract.FILE_TYPE_IMAGE, "ref_model": ref_image_2d}
    truth = extract.do_extract1d(input, ref_dict_2d, smoothing_length=0, bkg_order=0, log_increment=50, subtract_background=True)

    ref_dict_3d = {"ref_file_type": extract.FILE_TYPE_IMAGE, "ref_model": ref_image_3d}
    output = extract.do_extract1d(input, ref_dict_3d, smoothing_length=0, bkg_order=0, log_increment=50, subtract_background=True)
    ...
    assert np.allclose(wavelength, true_wl, rtol=1.0e-14, atol=1.0e-14)

    assert np.allclose(flux, true_flux, atol=1.0e-14)

    assert np.allclose(background, true_bkg, atol=1.0e-14)
...
def make_ifu_cube(
    data_shape, source=None, background=None, x_center=None, y_center=None, radius=None, inner_bkg=None, outer_bkg=None
):
    ...
    def mock_wcs(x, y, z):
        ...
        wavelength = np.linspace(0.5975, 5.2975, 941, endpoint=True, retstep=False, dtype=np.float64)
...
def make_ref_image(shape, x_center=None, y_center=None, radius=None, inner_bkg=None, outer_bkg=None):
