...
def test_fit_background_model(inputs_constant):
    (bkg_model, b_var_poisson_model, b_var_rnoise_model, b_var_flat_model, npts) = extract1d._fit_background_model(
        *inputs_constant
    )

    assert math.isclose(bkg_model(0.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(bkg_model(8.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(b_var_poisson_model(0.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(b_var_poisson_model(8.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(b_var_rnoise_model(0.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(b_var_rnoise_model(8.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(b_var_flat_model(0.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(b_var_flat_model(8.0), 22.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
...
def test_fit_background_mean(inputs_constant):
    ...
    (bkg_model, b_var_poisson_model, b_var_rnoise_model, b_var_flat_model, npts) = extract1d._fit_background_model(
        image, var_poisson, var_rnoise, var_rflat, x, j, bkglim, bkg_fit, bkg_order
    )
...
def test_handles_nan(inputs_constant):
    ...
    assert math.isclose(bkg_model(0.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(bkg_model(8.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(b_var_poisson_model(0.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(b_var_poisson_model(8.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(b_var_rnoise_model(0.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(b_var_rnoise_model(8.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(b_var_flat_model(0.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    assert math.isclose(b_var_flat_model(8.0), 0.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
