...
def inputs_constant():
    ...
    lower = np.zeros(shape[1], dtype=np.float64) + 3.0  # middle of pixel 3
    upper = np.zeros(shape[1], dtype=np.float64) + 7.0  # middle of pixel 7
    srclim = [[lower, upper]]

    return (image, var_poisson, var_rnoise, var_rflat, x, j, lam, srclim, weights, bkgmodels)


def test_extract_src_flux(inputs_constant):
    (image, var_poisson, var_rnoise, var_rflat, x, j, lam, srclim, weights, bkgmodels) = inputs_constant

    (total_flux, f_var_poisson, f_var_rnoise, f_var_flat, bkg_flux, b_var_poisson, b_var_rnoise, b_var_flat, tarea, twht) = (
        extract1d._extract_src_flux(image, var_poisson, var_rnoise, var_rflat, x, j, lam, srclim, weights, bkgmodels)
    )

    # 0.5 * 17. + 22. + 27. + 32. + 0.5 * 37.
    assert math.isclose(total_flux, 108.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert bkg_flux == 0.0

    assert math.isclose(tarea, 4.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
    ...
    # 0.5 * 17. + 22. + 32. + 0.5 * 37.
    assert math.isclose(total_flux, 81.0, rel_tol=1.0e-8, abs_tol=1.0e-8)

    assert math.isclose(tarea, 3.0, rel_tol=1.0e-8, abs_tol=1.0e-8)
