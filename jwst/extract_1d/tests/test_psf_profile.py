...
def test_psf_profile_model_nod_no_trace(mock_miri_lrs_fs, psf_reference_file, log_watcher):
    ...
    watcher = log_watcher(
        "stpipe.jwst.extract_1d", message="Cannot model a negative nod without position"
    )
    profiles, lower, upper = pp.psf_profile(
        model, trace, wl_array, psf_reference_file, optimize_shifts=False, model_nod_pair=True
    )
...
def test_psf_profile_model_nod_not_subtracted(mock_miri_lrs_fs, psf_reference_file, log_watcher):
    ...
    watcher = log_watcher("stpipe.jwst.extract_1d", message="data was not nod-subtracted")
    profiles, lower, upper = pp.psf_profile(
        model, trace, wl_array, psf_reference_file, optimize_shifts=False, model_nod_pair=True
    )
    watcher.assert_seen()
...
def test_psf_profile_model_nod_wrong_pattern(mock_miri_lrs_fs, psf_reference_file, log_watcher):
    ...
    watcher = log_watcher("stpipe.jwst.extract_1d", message="data was not a two-point nod")
    profiles, lower, upper = pp.psf_profile(
        model, trace, wl_array, psf_reference_file, optimize_shifts=False, model_nod_pair=True
    )
    watcher.assert_seen()
...
def test_psf_profile_model_nod_bad_position(mock_miri_lrs_fs, psf_reference_file, log_watcher):
    ...
    watcher = log_watcher("stpipe.jwst.extract_1d", message="Nod center could not be estimated")
    profiles, lower, upper = pp.psf_profile(
        model, trace, wl_array, psf_reference_file, optimize_shifts=False, model_nod_pair=True
    )
    watcher.assert_seen()
...
def test_psf_profile_optimize(
    ...
    watcher = log_watcher("stpipe.jwst.extract_1d", message="Centering profile on spectrum at 24.5")
    profiles, lower, upper = pp.psf_profile(
        model,
        trace,
        wl_array,
...
def test_psf_profile_optimize_with_nod(
    ...
    watcher = log_watcher(
        "stpipe.jwst.extract_1d", message="Also modeling a negative trace at 39.50"
    )
    profiles, lower, upper = pp.psf_profile(
        model,
        trace,
