            ...
            if self.location is not None:
                if isinstance(self.model, MultiSlitModel):
                    ...
                    scale_degrees = compute_scale(
                        ...
                        self.location,
                        disp_axis=self.model.slits[
                            idx
                        ].meta.wcsinfo.dispersion_direction,
                    )
...
class ApCorrPhase(ApCorrBase):
    ...
    *args, **kwargs :
    ...
    def approximate(self):
        ...
        def _approx_func(
            wavelength: float, size: float, pixel_phase: float
        ) -> interp2d:
            ...
            apcorr_pixphase_func = interp1d(
                self.reference["pixphase"], self.reference["apcorr"]
            )
            size_wl_func = interp1d(
                self.reference["wavelength"], self.reference["size"]
            )

            apcorr_pixphase = apcorr_pixphase_func(pixel_phase)
            size_wl = size_wl_func(wavelength)

            pixphase_size_func = interp2d(
                self.reference["wavelength"], size_wl, apcorr_pixphase
            )
    ...
    def apply(self, spec_table: fits.FITS_rec):
        ...
        for row in spec_table:
            try:
                correction = self.apcorr_func(
                    row["wavelength"], row["npixels"], self.phase
                )
            except ValueError:
...
class ApCorrRadial(ApCorrBase):
    ...
    def match_wavelengths(self, wavelength_ifu):
        ...
        for i in range(dim):
            ...
            appi = self.apcorr[i, :]
            fap = interp1d(
                self.wavelength, appi, bounds_error=False, fill_value="extrapolate"
            )
...
def select_apcorr(
    input_model: DataModel,
) -> Union[Type[ApCorr], Type[ApCorrPhase], Type[ApCorrRadial]]:
