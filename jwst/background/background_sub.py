...
def average_background(bkg_list, sigma, maxiters):
    ...
    for i, bkg_file in enumerate(bkg_list):
        ...
        if bkg_dim == 3:
            ...
            sc_bkg_data = sigma_clip(
                bkg_model.data, sigma=sigma, maxiters=maxiters, axis=0
            )
            sc_bkg_err = sigma_clip(
                bkg_model.err * bkg_model.err, sigma=sigma, maxiters=maxiters, axis=0
            )
...
def subtract_wfss_bkg(input_model, bkg_filename, wl_range_name, mmag_extract=None):
    ...
    log.debug(
        "mean of [{}, {}] percentile grism image = {}".format(lowlim, highlim, sci_mean)
    )
    log.debug(
    ...
    if bkg_mean != 0.0:
        ...
        log.info(
            f"Average of background image subtracted = {subtract_this.mean(dtype=float)}"
        )
    else:
