    ...
    def process(self, input, bkg_list):
        ...
        with datamodels.open(input) as input_model:
            if input_model.meta.exposure.type in ["NIS_WFSS", "NRC_WFSS"]:
                ...
                result = background_sub.subtract_wfss_bkg(input_model, bkg_name, wlrange_name, self.wfss_mmag_extract)
                if result is None:
                    result = input_model
                ...
                if input_model.meta.instrument.name in ["NIRSPEC"]:
                    ...
                    for bkg_file in bkg_list:
                        with datamodels.open(bkg_file) as bkg_model:
                            ...
                            if np.allclose((input_xtilt, input_ytilt), (bkg_xtilt, bkg_ytilt), atol=tolerance, rtol=0):
                                pass
                            else:
                                do_sub = False
                                break
                ...
                if do_sub:
                    bkg_model, result = background_sub.background_sub(input_model, bkg_list, self.sigma, self.maxiters)
                    result.meta.cal_step.back_sub = "COMPLETE"
                    ...
                    self.log.warning("GWA_XTIL and GWA_YTIL source values " "are not the same as bkg values")
