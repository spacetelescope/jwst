...
def frame_from_model(wcsinfo):
    ...
    if spectral_axes:
        spec = cf.SpectralFrame(
            name="spectral", axes_order=tuple(spectral_axes), unit=cunit[spectral_axes], axes_names=("wavelength",)
        )
        frames.append(spec)
    if other:
        ...
        spatial = cf.Frame2D(name=name, axes_order=tuple(other), unit=cunit[other], axes_names=axes_names)
