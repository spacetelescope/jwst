...
def add_configuration_records(filename):
    """
    Add in some test configuration records to the MSA metadata file.

    Parameters
    ----------
    filename : str
        The name of the MSA metadata file to add the configuration records to.
    """
    ...
    base = np.array(
        [
            (12, 2, 4, 251, 22, 1, "Y", "OPEN", np.nan, np.nan),
    ...
    tt = numpy.lib.recfunctions.stack_arrays(
        (base, test1, test2, test3, test4, test5), usemask=False
    )
    msa_conf[2].data = tt
...
if __name__ == "__main__":
    print("Call add_configuration_records(filename) directly.")  # noqa: T201
