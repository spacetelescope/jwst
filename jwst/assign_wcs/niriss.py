...
def niriss_soss_set_input(model, order_number):
    ...
    sky = cf.CelestialFrame(
        reference_frame=coord.ICRS(), axes_names=("ra", "dec"), axes_order=(0, 1), unit=(u.deg, u.deg), name="sky"
    )
...
def niriss_soss(input_model, reference_files):
    ...
    cm_order1 = (Mapping((0, 1, 1, 0)) | (Const1D(target_ra) & Const1D(target_dec) & wl1)).rename("Order1")
    cm_order2 = (Mapping((0, 1, 1, 0)) | (Const1D(target_ra) & Const1D(target_dec) & wl2)).rename("Order2")
    cm_order3 = (Mapping((0, 1, 1, 0)) | (Const1D(target_ra) & Const1D(target_dec) & wl3)).rename("Order3")
    ...
    if subarray2full is not None:
        ...
        bbox = ((-0.5, input_model.meta.subarray.ysize - 0.5), (-0.5, input_model.meta.subarray.xsize - 0.5))
...
def wfss(input_model, reference_files):
    ...
    gdetector = cf.Frame2D(name="grism_detector", axes_order=(0, 1), axes_names=("x_grism", "y_grism"), unit=(u.pix, u.pix))
    spec = cf.SpectralFrame(name="spectral", axes_order=(2,), unit=(u.micron,), axes_names=("wavelength",))
    ...
    if input_model.meta.instrument.filter.endswith("C"):
        det2det = NIRISSForwardRowGrismDispersion(orders, lmodels=displ, xmodels=dispx, ymodels=dispy, theta=fwcpos - fwcpos_ref)
        ...
        raise ValueError("FILTER keyword {} is not valid.".format(input_model.meta.instrument.filter))

    backward = NIRISSBackwardGrismDispersion(orders, lmodels=invdispl, xmodels=dispx, ymodels=dispy, theta=-(fwcpos - fwcpos_ref))
