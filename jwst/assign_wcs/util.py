...
def reproject(wcs1, wcs2):
    """
    Take in pixel coordinates in the first WCS and computes their location in the second one.
...
def compute_fiducial(wcslist, bounding_box=None):
    """
    Compute the 'fiducial point' of a list of WCS objects.
    ...
    bounding_box : tuple, None
        The bounding box of the output frame.

    Returns
    -------
    fiducial : np.ndarray
        The fiducial point. For a celestial footprint this is the center.
...
def is_fits(input_img):
    """
    Determine if the input is a FITS file.

    Parameters
    ----------
    input_img : str, `~astropy.io.fits.HDUList`
        The input image to be checked.

    Returns
    -------
    isfits : bool
        True if the input is a FITS file, False otherwise.
    fitstype : str
        The type of FITS file, one of 'waiver', 'mef', 'simple'; None if not a FITS file.
...
def get_num_msa_open_shutters(shutter_state):
    ...
    -------
    num : int
        The number of open shutters in the slitlet.
    """
