...
def create_nirspec_ifu_file(
    filter, grating, lamp="N/A", detector="NRS1", gwa_xtil=0.3318742513656616, gwa_ytil=0.1258982867002487, gwa_tilt=None
):
...
def test_msa_configuration_normal():
    ...
    dither_position = 1
    slitlet_info = nirspec.get_open_msa_slits(prog_id, msaconfl, msa_meta_id, dither_position, slit_y_range=[-0.5, 0.5])
    ref_slit = trmodels.Slit(
        55, 9376, 1, 251, 26, -5.6, 1.0, 4, 1, "1111x", "95065_1", "2122", 0.13, -0.31716078999999997, -0.18092266
    )
...
def test_msa_configuration_no_background():
    ...
    with pytest.raises(MSAFileError):
        nirspec.get_open_msa_slits(prog_id, msaconfl, msa_meta_id, dither_position, slit_y_range=[-0.5, 0.5])
...
def test_msa_configuration_all_background():
    ...
    dither_position = 1
    slitlet_info = nirspec.get_open_msa_slits(prog_id, msaconfl, msa_meta_id, dither_position, slit_y_range=[-0.5, 0.5])
    ref_slit = trmodels.Slit(57, 8281, 1, 251, 23, -2.15, 2.15, 4, 57, "1x1", "1234_BKG57", "BKG57", 0.0, 0.0, 0.0)
    _compare_slits(slitlet_info[0], ref_slit)
...
def test_msa_configuration_row_skipped():
    ...
    slitlet_info = nirspec.get_open_msa_slits(prog_id, msaconfl, msa_meta_id, dither_position, slit_y_range=[-0.5, 0.5])
    ref_slit = trmodels.Slit(
        58, 8646, 1, 251, 24, -3.3, 5.6, 4, 1, "11x1011", "95065_1", "2122", 0.130, -0.31716078999999997, -0.18092266
    )
    _compare_slits(slitlet_info[0], ref_slit)
...
def test_msa_fs_configuration():
    ...
    ref_slit = trmodels.Slit(
        "S200A1", 0, 1, 0, 0, -0.5, 0.5, 5, 3, "x", "95065_3", "3", 1.0, -0.161, -0.229, 53.139904, -27.805002
    )
...
def test_msa_fs_configuration_unsupported(tmp_path):
    ...
    with pytest.raises(MSAFileError, match="unsupported fixed slit"):
        nirspec.get_open_msa_slits(prog_id, bad_confl, msa_meta_id, dither_position, slit_y_range=[-0.5, 0.5])
...
def test_msa_missing_source(tmp_path):
    ...
    slitlet_info = nirspec.get_open_msa_slits(prog_id, bad_confl, msa_meta_id, dither_position, slit_y_range=[-0.5, 0.5])

    # MSA slit: virtual source name assigned
    ref_slit = trmodels.Slit(
        55, 9376, 1, 251, 26, -5.6, 1.0, 4, 1, "1111x", "1234_VRT55", "VRT55", 0.0, -0.31716078999999997, -0.18092266, 0.0, 0.0
    )
    _compare_slits(slitlet_info[0], ref_slit)

    # FS primary: S200A1, virtual source name assigned
    ref_slit = trmodels.Slit(
        "S200A1", 0, 1, 0, 0, -0.5, 0.5, 5, 3, "x", "1234_VRTS200A1", "VRTS200A1", 0.0, -0.161, -0.229, 0.0, 0.0
    )
    _compare_slits(slitlet_info[1], ref_slit)
...
def test_functional_ifu_prism():
    ...
    hdu1 = create_nirspec_ifu_file(grating="PRISM", filter="CLEAR", gwa_xtil=0.35986012, gwa_ytil=0.13448857, gwa_tilt=37.1)
