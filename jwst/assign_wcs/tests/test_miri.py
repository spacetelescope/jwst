...
def run_test(model):
    ...
    for ch in model.meta.instrument.channel:
        ...
        for i, s in enumerate(ref_data["s"]):
            sl = int(ch) * 100 + s
            x_back, y_back = ab_to_detector.set_input(sl)(
                alpha_back[i], beta_back[i], lam_back[i]
            )
