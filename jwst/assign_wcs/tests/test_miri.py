...
def create_miri_ifu_short(sml="SHORT"):
    image = create_hdul()
    image[0].header['DETECTOR'] = 'MIRIFUSHORT'
    image[0].header['CHANNEL'] = '12'
    image[0].header['BAND'] = sml
    return image


def create_miri_ifu_long(sml="SHORT"):
    image = create_hdul()
    image[0].header['DETECTOR'] = 'MIRIFULONG'
    image[0].header['CHANNEL'] = '34'
    image[0].header['BAND'] = sml
    return image
...
def test_miri_mrs_1A():
    ...
    for ch in im.meta.instrument.channel:
        ...
        for i, s in enumerate(ref_data['s']):
            sl = int(ch) * 100 + s
            detector_to_alpha_beta = wcsobj.get_transform('detector',
                                                          'alpha_beta')
            ...
            utils.assert_allclose(yin, ref_data['y'][i], atol=10**-5)


def test_miri_mrs_1B():

    f = create_miri_ifu_short("MEDIUM")
    im = ImageModel(f)
    im.meta.exposure.type = 'MIR_MRS'
    ref = create_reference_files(im)
