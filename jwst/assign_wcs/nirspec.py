...
def apply_slicemap(input_model, replace_wcs=True):
    ...
    for slit_id in range(nslice):
        ...
        # This is more efficient than fixing the slit name for the full det2slicer
        # transform, since it only needs to store one specific model per transform.
        gwa2slit_by_slice = _fix_slit_name(gwa2slit.models[slit_id], slit_id)
        slit2slicer_by_slice = _fix_slit_name(slit2slicer[0].models[slit_id], slit_id) & Identity(1)
