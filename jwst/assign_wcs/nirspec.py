...
def create_pipeline(input_model, reference_files, slit_y_range):
    ...
    if pipeline:
        log.info(
            "Created a NIRSPEC {0} pipeline with references {1}".format(
...
def imaging(input_model, reference_files):
    ...
    gwa2msa.inverse = (
        col.inverse | dircos2unitless.inverse | rotation.inverse | gwa_through
    )
...
def ifu(input_model, reference_files, slit_y_range=[-0.55, 0.55]):
    ...
    # GWA to SLIT
    gwa2slit = gwa_to_ifuslit(
        slits, input_model, disperser, reference_files, slit_y_range
    )
...
def get_open_slits(input_model, reference_files=None, slit_y_range=[-0.55, 0.55]):
    ...
    if exp_type in ["nrs_msaspec", "nrs_autoflat"] or (
        ...
        slits = get_open_msa_slits(
            msa_metadata_file, msa_metadata_id, dither_point, slit_y_range
        )
...
def get_open_msa_slits(
    msa_file, msa_metadata_id, dither_position, slit_y_range=[-0.55, 0.55]
):
    """
    ...
    for slitlet_id in slitlet_ids_unique:
        ...
        if n_main_shutter == 0:
            ...
            xcen = slitlets_sid[0][
                "shutter_row"
            ]  # grab the first as they are all the same
            source_xpos = 0.0
            source_ypos = 0.0
            source_id = _get_bkg_source_id(bkg_counter, max_source_id)
            log.info(
                f"Slitlet_id {slitlet_id} is background only; assigned source_id = {source_id}"
            )
            bkg_counter += 1
        ...
        # Not allowed....
            ...
            log.warning(message)
            message = (
                "MSA configuration file has more than 1 shutter with primary source"
            )
            log.warning(message)
            raise MSAFileError(message)
...
def get_spectral_order_wrange(input_model, wavelengthrange_file):
    ...
    try:
        ...
        log.info(
            "Combination {0} missing in wavelengthrange file, setting "
...
def gwa_to_ifuslit(slits, input_model, disperser, reference_files, slit_y_range):
    ...
    try:
        ...
        if velosys is not None:
            ...
            log.info(
                "Applied Barycentric velocity correction : {}".format(
    ...
    for slit in slits:
        ...
        msa2gwa = (
            ifuslicer_transform & Const1D(lam_cen) | ifupost_transform | collimator2gwa
        )
        gwa2slit = gwa_to_ymsa(
            msa2gwa, lam_cen=lam_cen, slit_y_range=slit_y_range
        ...
        msa2gwa_out = (
            ifuslicer_transform & Identity(1) | ifupost_transform | collimator2gwa
        )
        msa2bgwa = (
            Mapping((0, 1, 2, 2))
            | msa2gwa_out & Identity(1)
            | Mapping((3, 0, 1, 2))
            | agreq
        )
...
def gwa_to_slit(open_slits, input_model, disperser, reference_files):
    ...
    for quadrant in range(1, 6):
        slits_in_quadrant = [s for s in open_slits if s.quadrant == quadrant]
        log.info(
            "There are {0} open slits in quadrant {1}".format(
        ...
        if any(slits_in_quadrant):
            ...
            for slit in slits_in_quadrant:
                ...
                gwa2msa = gwa_to_ymsa(
                    msa2gwa, slit=slit, slit_y_range=(slit.ymin, slit.ymax)
                )  # TODO: Use model sets here
                bgwa2msa = (
                    Mapping((0, 1, 0, 1), n_inputs=3)
                    | Const1D(0) * Identity(1) & Const1D(-1) * Identity(1) & Identity(2)
...
def angle_from_disperser(disperser, input_model):
    ...
    if input_model.meta.instrument.grating.lower() != "prism":
        agreq = AngleFromGratingEquation(
...
def dms_to_sca(input_model):
    ...
    if detector == "NRS2":
        model = models.Shift(-2047) & models.Shift(-2047) | models.Scale(
            -1
        ) & models.Scale(-1)
    elif detector == "NRS1":
...
def compute_bounding_box(transform, wavelength_range, slit_ymin=-0.55, slit_ymax=0.55):
    ...
    def bbox_from_range(x_range, y_range):
        ...
        # add 10 px margin
        pad_x = (
            max(0, x_range.min() - 1 - 10) - 0.5,
            min(2047, x_range.max() - 1 + 10) + 0.5,
        )
        # add 2 px margin
        pad_y = (
            max(0, y_range.min() - 1 - 2) - 0.5,
            min(2047, y_range.max() - 1 + 2) + 0.5,
        )

        return pad_x, pad_y

    x_range_low, y_range_low = slit2detector(
        [0] * nsteps, [slit_ymin] * nsteps, lam_grid
    )
    x_range_high, y_range_high = slit2detector(
        [0] * nsteps, [slit_ymax] * nsteps, lam_grid
    )
...
def create_imaging_frames():
    ...
    msa = cf.Frame2D(
        name="msa", axes_order=(0, 1), unit=(u.m, u.m), axes_names=("x_msa", "y_msa")
    )
    v2v3 = cf.Frame2D(
        name="v2v3",
        axes_order=(0, 1),
    ...
    world = cf.CelestialFrame(
        name="world", axes_order=(0, 1), reference_frame=coord.ICRS()
    )
...
def get_slit_location_model(slitdata):
    ...
    model = models.Scale(xsize) & models.Scale(ysize) | models.Shift(
        xcenter
...
def gwa_to_ymsa(msa2gwa_model, lam_cen=None, slit=None, slit_y_range=None):
    ...
    tab = Tabular1D(
        points=(beta_in,), lookup_table=dy, bounds_error=False, name="tabular"
    )
    return tab
...
def validate_open_slits(input_model, open_slits, reference_files):
    ...
    col2det = (
        collimator2gwa & Identity(1) | Mapping((3, 0, 1, 2)) | agreq | gwa2det | det2dms
    )
