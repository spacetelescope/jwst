...
def tsgrism(input_model, reference_files):
    ...
    det2det = NIRCAMForwardRowGrismDispersion(
        orders, lmodels=displ, xmodels=dispx, ymodels=dispy, inv_lmodels=invdispl, inv_xmodels=invdispx
    )
    ...
    if sub_trans is not None:
        sub2direct = (
            sub_trans & Identity(1) | Mapping((0, 1, 0, 1, 2)) | (Identity(2) & xcenter & ycenter & Identity(1)) | det2det
        )
    else:
        sub2direct = Mapping((0, 1, 0, 1, 2)) | (Identity(2) & xcenter & ycenter & Identity(1)) | det2det
...
def wfss(input_model, reference_files):
    ...
    if "GRISMR" in input_model.meta.instrument.pupil:
        det2det = NIRCAMForwardRowGrismDispersion(
            orders, lmodels=displ, xmodels=dispx, ymodels=dispy, inv_lmodels=invdispl, inv_xmodels=invdispx, inv_ymodels=invdispy
        )
...
def create_coord_frames():
    ...
    detector = cf.Frame2D(name="full_detector", axes_order=(0, 1), axes_names=("dx", "dy"), unit=(u.pix, u.pix))
    v2v3_spatial = cf.Frame2D(name="v2v3_spatial", axes_order=(0, 1), axes_names=("v2", "v3"), unit=(u.deg, u.deg))
    v2v3vacorr_spatial = cf.Frame2D(
        name="v2v3vacorr_spatial", axes_order=(0, 1), axes_names=("v2", "v3"), unit=(u.arcsec, u.arcsec)
    )
