...
from .util import update_s_region_spectral, update_s_region_imaging, update_s_region_nrs_ifu, update_s_region_mrs
...
def load_wcs(input_model, reference_files={}, nrs_slit_y_range=None):
    ...
    if pipeline is None:
        ...
        if output_model.meta.exposure.type.lower() not in exclude_types:
            ...
            if output_model.meta.exposure.type.lower() in imaging_types:
                try:
                    ...
                    log.error("Unable to update S_REGION for type {}: {}".format(output_model.meta.exposure.type, exc))
                else:
                ...
                try:
                    ...
                    log.info("Unable to update S_REGION for type {}: {}".format(output_model.meta.exposure.type, exc))
        ...
        if output_model.meta.exposure.type.lower() == "mir_lrs-fixedslit":
            ...
            log.debug(
                f"Storing dithered pointing location information:"
