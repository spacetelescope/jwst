...
def create_pipeline(input_model, reference_files):
    ...
    if pipeline:
        log.info(
            "Created a MIRI {0} pipeline with references {1}".format(
...
def lrs(input_model, reference_files):
    ...
    spec = cf.SpectralFrame(
        name="spec", axes_order=(2,), unit=(u.micron,), axes_names=("lambda",)
    )
...
def lrs_distortion(input_model, reference_files):
    ...
    if input_model.meta.exposure.type.lower() == "mir_lrs-fixedslit":
        ...
        bb_sub = (
            (
                np.floor(x0.min() + zero_point[0]) - 0.5,
    ...
    slope = 1.0 / z[0]
    traceangle = np.arctan(slope) * 180.0 / np.pi  # trace angle in degrees
    ...
    xyrottov2v3 = (
        models.Shift(zero_point[0]) & models.Shift(zero_point[1]) | det_to_v2v3
    )
    ...
    wavemodel.inverse = models.Tabular1D(
        lookup_table=ygrid,
        points=wgrid,
        name="waverefinv",
        bounds_error=False,
...
def get_wavelength_range(input_model, path=None):
    ...
    if path is None and not os.path.exists(fname):
        raise IOError(
            "Reference file {0} not found. Please specify a path.".format(fname)
        )
    else:
