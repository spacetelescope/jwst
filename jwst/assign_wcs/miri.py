...
def create_pipeline(input_model, reference_files):
    ...
    if pipeline:
        log.info("Created a MIRI {0} pipeline with references {1}".format(exp_type, reference_files))
    return pipeline
...
def imaging(input_model, reference_files):
    ...
    v2v3vacorr = cf.Frame2D(name="v2v3vacorr", axes_order=(0, 1), axes_names=("v2", "v3"), unit=(u.arcsec, u.arcsec))
...
def lrs(input_model, reference_files):
    ...
    icrs = cf.CelestialFrame(
        name="icrs", reference_frame=coord.ICRS(), axes_order=(0, 1), unit=(u.deg, u.deg), axes_names=("RA", "DEC")
    )
    ...
    pipeline = [(detector, dettoabl), (miri_focal, abltov2v3l), (v2v3, va_corr), (v2v3vacorr, teltosky), (world, None)]
...
def lrs_xytoabl(input_model, reference_files):
    ...
    if input_model.meta.exposure.type.lower() == "mir_lrs-slitless":
        bb_sub = (
            (input_model.meta.subarray.xstart - 1 + 4 - 0.5, input_model.meta.subarray.xsize - 1 + 0.5),
            (np.floor(y2.min() + zero_point[1]) - 0.5, np.ceil(y0.max() + zero_point[1]) + 0.5),
        )
    ...
    dxmodel = models.Tabular1D(
        lookup_table=xshiftref, points=ycen_subarray, name="xshiftref", bounds_error=False, fill_value=np.nan
    )
    ...
    wavemodel = models.Tabular1D(
        lookup_table=wavereference, points=ycen_subarray, name="waveref", bounds_error=False, fill_value=np.nan
    )
    ...
    reftransform = models.Shift(-zero_point[0]) & models.Shift(-zero_point[1]) | models.Shift(+zero_point[0]) & models.Shift(
        +zero_point[1]
    )
...
def ifu(input_model, reference_files):
    ...
    spec_local = cf.SpectralFrame(name="alpha_beta_spectral", axes_order=(2,), unit=(u.micron,), axes_names=("lambda",))
    miri_focal = cf.CompositeFrame([alpha_beta, spec_local], name="alpha_beta")
    v23_spatial = cf.Frame2D(name="v2v3_spatial", axes_order=(0, 1), unit=(u.arcsec, u.arcsec), axes_names=("v2", "v3"))
    v2v3vacorr_spatial = cf.Frame2D(
        name="v2v3vacorr_spatial", axes_order=(0, 1), unit=(u.arcsec, u.arcsec), axes_names=("v2", "v3")
    )
    ...
    pipeline = [(detector, det2abl), (miri_focal, abl2v2v3l), (v2v3, va_corr), (v2v3vacorr, tel2sky), (world, None)]
    return pipeline
...
def detector_to_abl(input_model, reference_files):
    ...
    for c in channel:
        ...
        lm = selector.LabelMapper(inputs=("alpha", "beta", "lam"), mapper=mapper, inputs_mapping=models.Mapping((1,), n_inputs=3))
        ch_dict[tuple(wr[cb])] = lm

    alpha_beta_mapper = selector.LabelMapperRange(("alpha", "beta", "lam"), ch_dict, models.Mapping((2,)))
...
def abl_to_v2v3l(input_model, reference_files):
    ...
    for c in channels:
        ch = int(c[0])
        dict_mapper[tuple(wr[c])] = models.Mapping((2,), name="mapping_lam") | models.Const1D(ch, name="channel #")
