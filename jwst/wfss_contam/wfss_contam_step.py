...
class WfssContamStep(Step):
    ...
    def process(self, input_model):
        ...
        with datamodels.open(input_model) as dm:
            ...
            self.log.info("Using WAVELENGTHRANGE reference file %s", waverange_ref)
            ...
            self.log.info("Using PHOTOM reference file %s", photom_ref)
            ...
            if self.save_simulated_image:
                ...
                self.log.info('Full-frame simulated grism image saved to "%s"', simul_path)
            if self.save_contam_images:
                contam_path = self.save_model(contam, suffix="contam", force=True)
                self.log.info('Contamination estimates saved to "%s"', contam_path)
