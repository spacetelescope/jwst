...
def to_container(model):
    ...
    for plane in range(model.shape[0]):
        ...
        for attribute in ["data", "dq", "err", "zeroframe", "area", "var_poisson", "var_rnoise", "var_flat"]:
            try:
    ...
    def process(self, user_input):
        ...
        for target_file in targ_files:
            with datamodels.open(target_file) as target:
                ...
                self.save_model(psf_aligned, output_file=target_file, suffix="psfalign", acid=acid)
                ...
                self.save_model(psf_sub, output_file=target_file, suffix="psfsub", acid=acid)
