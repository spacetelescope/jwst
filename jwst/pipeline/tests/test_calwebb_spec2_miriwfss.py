...
from jwst.datamodels.utils.tests.wfss_helpers import wfss_spec2_multi
from jwst.extract_1d.tests.helpers import mock_miri_wfss_l2
...
@pytest.fixture
def mock_miri_spec2_wfss_l2():
    model = mock_miri_wfss_l2()
    yield model
    model.close()
...
@pytest.fixture
def spec2_wfss_asn(mock_miri_spec2_wfss_l2, tmp_cwd):
    model = mock_miri_spec2_wfss_l2
...
@pytest.fixture
def run_spec2_wfss(spec2_wfss_asn, monkeypatch, wfss_multispec):
    """
    Run the spec2 pipeline on a WFSS association.

    Extract_1d step are mocked.
