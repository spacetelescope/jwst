...
from ..lib.exposure_types import (
    is_nrs_ifu_flatlamp,
    is_nrs_ifu_linelamp,
        ...
        for product in asn["products"]:
            ...
            try:
                result = self.process_exposure_product(
                    product, asn["asn_pool"], asn.filename
                )
    ...
    def process_exposure_product(self, exp_product, pool_name=" ", asn_file=" "):
        ...
        with self.open_model(science_member) as science:
            ...
            if exp_type in WFSS_TYPES:
                try:
                    science.meta.source_catalog = os.path.basename(
                        members_by_type["sourcecat"][0]
                    )
                    self.log.info(
                        "Using sourcecat file {}".format(science.meta.source_catalog)
                    )
                    science.meta.segmentation_map = os.path.basename(
                        members_by_type["segmap"][0]
                    )
                    self.log.info(
                        "Using segmentation map {}".format(
                            science.meta.segmentation_map
                        )
                    )
                    science.meta.direct_image = os.path.basename(
                        members_by_type["direct_image"][0]
                    )
                    self.log.info(
                        "Using direct image {}".format(science.meta.direct_image)
                    )
                except IndexError:
        ...
        if exp_type == "NIS_SOSS":
            ...
            if x1d is not None:
                ...
                self.log.warning(
                    "Extract_1d did not return a DataModel - skipping photom."
                )
        else:
            x1d = self.extract_1d(resampled)
    ...
    def _step_verification(self, exp_type, members_by_type, multi_int):
        ...
        if not self.bkg_subtract.skip:
            if exp_type in WFSS_TYPES or len(members_by_type["background"]) > 0:
                ...
                if exp_type in WFSS_TYPES:
                    members_by_type[
                        "background"
                    ] = []  # will be overwritten by the step
                ...
                self.log.debug(
                    'Science data does not allow direct background subtraction. Skipping "bkg_subtract".'
                )
        ...
        if not self.imprint_subtract.skip:
            if len(imprint) > 0 and (
                ...
                self.log.debug(
                    'Science data does not allow imprint processing. Skipping "imprint_subtraction".'
                )
        ...
        if not self.msa_flagging.skip and exp_type not in [
            ...
            self.log.debug(
                'Science data does not allow MSA flagging. Skipping "msa_flagging".'
            )
        ...
        if not self.residual_fringe.skip and exp_type != "MIR_MRS":
            self.log.debug(
                'Science data does not allow residual fringe correction. Skipping "residual fringe".'
            )
        ...
        if not self.fringe.skip and exp_type != "MIR_MRS":
            self.log.debug(
                'Science data does not allow fringe correction. Skipping "fringe".'
            )
        ...
        if not self.pathloss.skip and exp_type not in [
            ...
            self.log.debug(
                'Science data does not allow pathloss correction. Skipping "pathloss".'
            )
        ...
        if not self.master_background_mos.skip and exp_type != "NRS_MSASPEC":
            self.log.debug(
                'Science data does not allow master background correction. Skipping "master_background_mos".'
            )
