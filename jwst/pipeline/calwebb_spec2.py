...
NRS_SLIT_TYPES = ["NRS_FIXEDSLIT", "NRS_BRIGHTOBJ", "NRS_MSASPEC", "NRS_LAMP", "NRS_AUTOWAVE", "NRS_AUTOFLAT"]
...
class Spec2Pipeline(Pipeline):
    ...
    def process_exposure_product(
        ...
        if len(science_member) != 1:
            self.log.warning("Wrong number of science exposures found in {}".format(exp_product["name"]))
        ...
        if exp_type in ["NRS_FIXEDSLIT", "NRS_MSASPEC", "MIR_LRS-FIXEDSLIT"] and not isinstance(calibrated, datamodels.CubeModel):
    ...
    def _step_verification(self, exp_type, input, members_by_type, multi_int):
        ...
        if not self.msa_flagging.skip and exp_type not in ["NRS_MSASPEC", "NRS_IFU", "NRS_LAMP", "NRS_AUTOFLAT", "NRS_AUTOWAVE"]:
        ...
        if not self.imprint_subtract.skip:
            if len(imprint) > 0 and (exp_type in ["NRS_MSASPEC", "NRS_IFU"] or is_nrs_ifu_flatlamp(input)):
