...
def guider_cds(model):
    ...
    imshape, n_int, effinttm, readpatt = get_dataset_info(model)

    if readpatt == 'ID': # force output to have single slice
        new_model = datamodels.GuiderCdsModel((1,)+imshape)
    else:
        new_model = datamodels.GuiderCdsModel()

    new_model.dq = model.pixeldq
    slope_int_cube = np.zeros(( n_int,) + imshape, dtype = np.float64)

    # loop over data integrations
    for num_int in range(0, n_int):
        data_sect = model.data[num_int, :,:,:]

        if readpatt == 'FINEGUIDE':
...
def get_dataset_info(model):
    ...
    readpatt: string
        readpattern ...number of groups per integration
    ...
    log.info('Readpatt: %s' % (readpatt))

    return imshape, n_int, effinttm, readpatt
