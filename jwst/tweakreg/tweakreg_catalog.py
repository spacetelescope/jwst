...
def _source_finder_wrapper(data, threshold, mask=None, **kwargs):
    """
    Find stars using `~photutils.segmentation.SourceFinder`.

    Wrapper function for photutils SourceFinder to make input
...
def _iraf_starfinder_wrapper(data, threshold, mask=None, **kwargs):
    """
    Find stars using IRAF star finder.
...
def _dao_starfinder_wrapper(data, threshold, mask=None, **kwargs):
    """
    Find stars using DAO star finder.
...
def make_tweakreg_catalog(
    model, snr_threshold, bkg_boxsize=400, starfinder="iraf", starfinder_kwargs=None
):
    """
    ...
    if starfinder.lower() in ["dao", "daostarfinder"]:
        ...
        starfinder = _source_finder_wrapper
    else:
