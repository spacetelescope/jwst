...
def _wcsinfo_from_wcs_transform(wcs):
    ...
    wcsinfo = {"v2_ref": 3600 * v2_ref, "v3_ref": -3600 * nv3_ref, "roll_ref": roll_ref, "ra_ref": -nra_ref, "dec_ref": dec_ref}
...
def adjust_wcs(wcs, delta_ra=0.0, delta_dec=0.0, delta_roll=0.0, scale_factor=1.0):
    ...
    for step in pipeline[::-1]:
        if step.frame.name and step.frame.name.startswith("v2v3") and step.transform.name == "v23tosky":
            s = step
            break
    else:
    ...
    if scale_factor != 1.0:
        ...
        corr = JWSTWCSCorrector(wcs, {"v2_ref": 3600.0 * v2, "v3_ref": 3600.0 * v3, "roll_ref": 0.0})
