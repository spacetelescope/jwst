...
def _make_gwcs_wcs(fits_hdr):
    ...
    sip_distortion = (
        ...
        | Mapping((0, 1, 0, 1))
        | (polynomial.Polynomial2D(a_order, **cx, c1_0=1) & polynomial.Polynomial2D(b_order, **cy, c0_1=1))
        | (Shift(fw.wcs.crpix[0]) & Shift(fw.wcs.crpix[1]))
    )
    ...
    c2tan = (Mapping((0, 1, 2), name="xyz") / Mapping((0, 0, 0), n_inputs=3, name="xxx")) | Mapping((1, 2), name="xtyt")
    c2tan.name = "Cartesian 3D to TAN"

    tan2c = Mapping((0, 0, 1), n_inputs=2, name="xtyt2xyz") | (Const1D(1, name="one") & Identity(2, name="I(2D)"))
    ...
    v2v3_frm = cf.Frame2D(name="v2v3", unit=(u.arcsec, u.arcsec), axes_names=("x", "y"), axes_order=(0, 1))
...
def test_multichip_jwst_alignment(monkeypatch):
    ...
    imcat1.meta["catalog"] = table.Table.read(data_file, format="ascii.ecsv", delimiter=" ", names=["x", "y"])
    ...
    imcat2.meta["catalog"] = table.Table.read(
        os.path.join(data_path, "wfc3_uvis2.ecsv"), format="ascii.ecsv", delimiter=" ", names=["x", "y"]
    )
    ...
    refcat = table.Table.read(os.path.join(data_path, "ref.ecsv"), format="ascii.ecsv", delimiter=" ", names=["RA", "DEC"])
...
def test_multichip_alignment_step(monkeypatch):
    ...
    imcat1 = table.Table.read(os.path.join(data_path, "wfc3_uvis1.ecsv"), format="ascii.ecsv", delimiter=" ", names=["x", "y"])
    imcat1["x"] += 1
    imcat1["y"] += 1
    ...
    imcat2 = table.Table.read(os.path.join(data_path, "wfc3_uvis2.ecsv"), format="ascii.ecsv", delimiter=" ", names=["x", "y"])
    imcat2["x"] += 1
    imcat2["y"] += 1
