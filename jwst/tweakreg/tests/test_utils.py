...
from jwst.tweakreg.utils import adjust_wcs, transfer_wcs_correction, _wcsinfo_from_wcs_transform
...
def test_adjust_wcs():
    ...
    assert np.allclose([crval1a - crval10, crval2a - crval20], [0.0135, 0.0208], rtol=0, atol=1e-13)
    ...
    # test roll:
    assert np.allclose(ca1.position_angle(ca0).degree - c01.position_angle(c0).degree, 25.7, rtol=1.0e-5, atol=0.0)
...
def test_transfer_wcs_correction(nircam_rate, tmp_path, input_type):
    ...
    assert np.allclose(np.linalg.norm(np.subtract(m2.meta.wcs(x, y), m1.meta.wcs(x, y)), axis=0), 0, rtol=0, atol=5e-11)
