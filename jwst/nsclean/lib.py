...
class NSClean:
    ...
    def __init__(
        ...
        self.P = np.where(self.mask, self.P, 0.0)  # Illuminated areas carry no weight
    ...
    def fit(self, data):
        ...
        for y in np.arange(self.ny)[4:-4]:
            ...
            # Fill outliers
            d = np.where(np.logical_and(_mu - self.sigrej * _sigma <= d, d <= _mu + self.sigrej * _sigma), d, _mu)
...
class NSCleanSubarray:
    ...
    def __init__(self, data, mask, fc=(1061, 1211, 49943, 49957), exclude_outliers=True, weights_kernel_sigma=None):
        """
        ...
        self.rfftfreq = np.array(np.fft.rfftfreq(self.n, self.tpix), dtype=np.float32)  # Fourier frequencies in clocking pattern
        ...
        if exclude_outliers is True:
            ...
            bdpx = np.array(np.where(np.logical_or(self.data < vmin, self.data > vmax), 1, 0), dtype=np.float32)
        ...
        self.apodizer[here] = 0.5 * (np.cos((np.pi / (self.fc[1] - self.fc[0])) * (self.rfftfreq[here] - self.fc[0])) + 1)
        # Cosine roll-on
        here = np.logical_and(self.fc[2] <= self.rfftfreq, self.rfftfreq < self.fc[3])
        self.apodizer[here] = 0.5 * (np.cos((np.pi / (self.fc[3] - self.fc[2])) * (self.rfftfreq[here] - self.fc[2]) + np.pi) + 1)
    ...
    def fit(self, return_fit=False, weight_fit=False):
        ...
        if weight_fit is True:
            rfft[self.apodizer > 0.0] = np.matmul(pinv_PB * P.reshape((1, -1)), self.data[self.mask])
        else:
            rfft[self.apodizer > 0.0] = np.matmul(pinvB, self.data[self.mask])
