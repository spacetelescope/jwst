...
from jwst.associations import Association, ProcessList, libpath
...
from jwst.associations.lib.product_utils import (
...
class DMS_Level3_Base(DMSBaseMixin, Association):
    ...
    def _dms_product_name(association):
        ...
        product_name = (
            "jw{program}-{acid}" "_{target}" "_{instrument}" "_{opt_elem}{subarray}"
        )
    ...
    def make_fixedslit_bkg(self):
        ...
        for product in self["products"]:
            ...
            for science_exp in science_exps:
                ...
                bkg_name = remove_suffix(splitext(split(science_exp["expname"])[1])[0])[
                    0
                ]
    ...
    def _add(self, item):
        ...
        if member["exposerr"] not in _EMPTY:
            logger.warning(
                'Member {} has exposure error "{}"'.format(
                    item["filename"], member["exposerr"]
                )
            )
    ...
    def __str__(self):
        result_list = []
        result_list.append(
            "{} with {} products".format(self.asn_name, len(self.data["products"]))
        )
...
class Utility:
    ...
    def rename_to_level2(
        level1b_name, exp_type=None, is_tso=False, member_exptype="science"
    ):
...
def dms_product_name_noopt(asn):
    ...
    product_name = "jw{}-{}_{}_{}".format(
        asn.data["program"], asn.acid.id, target, instrument
    )
    return product_name.lower()
...
def dms_product_name_sources(asn):
    ...
    product_name_format = (
        "jw{program}-{acid}" "_{source_id}" "_{instrument}" "_{opt_elem}{subarray}"
    )
