...
def test_wavecorr():
    ...
    im_ex2d.slits[0].meta.wcs.bounding_box = ((-0.5, 1432.5), (-0.5, 37.5))
...
def test_skipped():
    ...
    assert_allclose(
        (outw.slits[ind].source_xpos, outw.slits[ind].source_ypos), source_pos
    )
...
def test_wavecorr_fs():
    ...
    mean_correction = np.abs(
        src_result.slits[0].wavelength - result.slits[0].wavelength
    )
