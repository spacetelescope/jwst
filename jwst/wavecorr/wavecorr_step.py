    ...
    def process(self, step_input):
        ...
        with datamodels.open(step_input) as input_model:
            ...
            if isinstance(input_model, datamodels.SlitModel):
                if not (
                    hasattr(input_model.meta, "wcs")
                    and input_model.meta.wcs is not None
                ):
                    raise AttributeError(
                        "Input model does not have a WCS object; assign_wcs should "
                        "be run before wavecorr."
                    )
            else:
                if not (
                    hasattr(input_model.slits[0].meta, "wcs")
                    and input_model.slits[0].meta.wcs is not None
                ):
                    raise AttributeError(
                        "Input model does not have a WCS object; assign_wcs should "
                        "be run before wavecorr."
