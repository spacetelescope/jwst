...
def test_ad_floor_and_saturation_flagging(setup_nrc_cube):
    ...
    assert np.all(
        np.bitwise_and(
            output.groupdq[0, satindxs, 4:6, 4:6], dqflags.group["SATURATED"]
        )
        == dqflags.group["SATURATED"]
    )
...
def test_signal_fluctuation_flagging(setup_nrc_cube):
    ...
    assert np.all(
        np.bitwise_and(
            output.groupdq[0, satindex:, 4:6, 4:6], dqflags.group["SATURATED"]
        )
        == dqflags.group["SATURATED"]
    )
