...
def test_ad_floor_flagging(setup_nrc_cube):
    ...
    assert np.all(output.groupdq[0, satindxs, 5, 5] == dqflags.group["DO_NOT_USE"] | dqflags.group["AD_FLOOR"])
...
def test_ad_floor_and_saturation_flagging(setup_nrc_cube):
    ...
    assert np.all(output.groupdq[0, floorindxs, 5, 5] == dqflags.group["DO_NOT_USE"] | dqflags.group["AD_FLOOR"])
    # Check if the right frames are flagged as saturated
    assert np.all(np.bitwise_and(output.groupdq[0, satindxs, 4:6, 4:6], dqflags.group["SATURATED"]) == dqflags.group["SATURATED"])
...
def test_signal_fluctuation_flagging(setup_nrc_cube):
    ...
    assert np.all(
        np.bitwise_and(output.groupdq[0, satindex:, 4:6, 4:6], dqflags.group["SATURATED"]) == dqflags.group["SATURATED"]
    )
...
def test_all_groups_saturated(setup_nrc_cube):
    ...
    assert np.all(np.bitwise_and(output.groupdq[0, :, 4:6, 4:6], dqflags.group["SATURATED"]) == dqflags.group["SATURATED"])
